 ...
 * synthetic quantized weight (exact-representable, reusing ScalarJamTest's encoders) and a random
 * F32 activation are fed through BOTH the vector kernel and {@link ScalarJAM} (the complete
 * reference floor); the two must agree to within the int8-activation quantization the tiles use.
 * Kernels are invoked exactly as jinfer does — weight as a raw segment, activation + output through
 * the GLOBAL segment at their absolute addresses — which is uniform whether a kernel stores via
 * {@code o.set} or absolute {@code putFloat}.
 *
...
class VectorKernelTest {
    ...
    @Test
    void q8_0() {
        eachShape("Q8_0", VectorKernelTest::q8_0, Q8Kernel::gemm);
    }

    @Test
    void q4_0() {
        eachShape("Q4_0", VectorKernelTest::q4_0, Q4Kernel::gemm);
    }

    @Test
    void q4_k() {
        eachShape("Q4_K", VectorKernelTest::q4_k, withScratch(Q4KKernel::gemm));
    }

    @Test
    void q5_k() {
        eachShape("Q5_K", VectorKernelTest::q5_k, withScratch(Q5KKernel::gemm));
    }

    @Test
    void q6_k() {
        eachShape("Q6_K", VectorKernelTest::q6_k, withScratch(Q6KKernel::gemm));
    }

    @Test
    void mxfp4() {
        eachShape("MXFP4", VectorKernelTest::mxfp4, withScratch(Mxfp4Kernel::gemm));
    }
     ...
     */
    private static void eachShape(
            String name,
            java.util.function.BiFunction<Integer, Integer, Weight> encode,
            Gemm kernel) {
        ...
        for (int n : new int[] {8, 13, 16})
            check(name + " n=" + n, encode.apply(m, k), kernel, m, n, k);
    }
    ...
    // ---- weight: dtype tag, encoded segment, exact decoded values (encoders copied from
    // ScalarJamTest) ----
