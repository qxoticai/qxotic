 ...
 * blocks ({@code fp16 d; 32 int8 q}, value {@code d·q}) read from {@code w}; activation is F32 from
 * {@code (a, aBase)}; output F32 written by absolute address through {@link VectorSupport#putFloat} (so the
 * {@code o} segment is unused — {@code oBase} is the output's absolute byte address). The register-tile
 * shape is chosen once from the CPU width + the JIT in play ({@link VectorSupport#TILE_CODE}); override with
 * {@code -Djam.vector.tile}. Behaviour is identical to jinfer's {@code Q8_0FloatTensor.vectorGemm512F32} —
 * jinfer now delegates here.
 */
public final class Q8Kernel {

    private Q8Kernel() {}

    private static final int BLOCK = 32, TYPE = 34;   // Q8_0: 32 elems, 34 bytes/block (fp16 scale + 32 int8)

    /** The {@code -Djam.vector.tile} string this kernel resolved (e.g. {@code auto}). Diagnostics/tests. */
    public static String tile() { return VectorSupport.TILE; }
    /** The register-tile code this kernel dispatches on (see {@link VectorSupport#TILE_CODE}). */
    public static int tileCode() { return VectorSupport.TILE_CODE; }
    ...
    private static void gemm128Tile4x4F32(MemorySegment w, MemorySegment x, long xBase, long outAddr,
        ...
        private static void gemm512Tile2x8F32(MemorySegment w, MemorySegment x, long xBase, long outAddr,
            ...
            o += outStride; putFloat(outAddr + 4L * (o), c01.reduceLanes(VectorOperators.ADD));
            putFloat(outAddr + 4L * (o + 1), c11.reduceLanes(VectorOperators.ADD));
            o += outStride; putFloat(outAddr + 4L * (o), c02.reduceLanes(VectorOperators.ADD));
            putFloat(outAddr + 4L * (o + 1), c12.reduceLanes(VectorOperators.ADD));
            o += outStride; putFloat(outAddr + 4L * (o), c03.reduceLanes(VectorOperators.ADD));
            putFloat(outAddr + 4L * (o + 1), c13.reduceLanes(VectorOperators.ADD));
            o += outStride; putFloat(outAddr + 4L * (o), c04.reduceLanes(VectorOperators.ADD));
            putFloat(outAddr + 4L * (o + 1), c14.reduceLanes(VectorOperators.ADD));
            o += outStride; putFloat(outAddr + 4L * (o), c05.reduceLanes(VectorOperators.ADD));
            putFloat(outAddr + 4L * (o + 1), c15.reduceLanes(VectorOperators.ADD));
            o += outStride; putFloat(outAddr + 4L * (o), c06.reduceLanes(VectorOperators.ADD));
            putFloat(outAddr + 4L * (o + 1), c16.reduceLanes(VectorOperators.ADD));
            o += outStride; putFloat(outAddr + 4L * (o), c07.reduceLanes(VectorOperators.ADD));
        ...
        IntConsumer action = worker -> {
            ...
            for (int tileIndex = tileStart; tileIndex < tileEnd; tileIndex++) {
                ...
                switch (VectorSupport.TILE_CODE) {
                    case 1 -> { // 3x4
                        for (; row + 2 < rowEnd; row += 3) {
                            int s = s0;
                            for (; s + 3 < seqEnd; s += 4) {
                                gemm512Tile3x4F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 2, s);
                            }
                        }
                    }
                    case 2 -> { // 4x4
                        for (; row + 3 < rowEnd; row += 4) {
                            int s = s0;
                            for (; s + 3 < seqEnd; s += 4) {
                                gemm512Tile4x4F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 2, s);
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 3, s);
                            }
                        }
                    }
                    case 3 -> { // 2x8
                        for (; row + 1 < rowEnd; row += 2) {
                            int s = s0;
                            for (; s + 7 < seqEnd; s += 8) {
                                gemm512Tile2x8F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                    ...
                    case 4 -> { // 8x2
                        for (; row + 7 < rowEnd; row += 8) {
                            int s = s0;
                            for (; s + 1 < seqEnd; s += 2) {
                                gemm512Tile8x2F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                for (int r = 0; r < 8; r++) {
                                    gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + r, s);
                                }
                            }
                        }
                    }
                    case 5 -> { // 1x1 educational: no tiling, one output per call over the whole tile
                        for (; row < rowEnd; row++) {
                            for (int s = s0; s < seqEnd; s++) {
                                gemm512Tile1x1EduF32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                        }
                    }
                    case 6 -> { // avx256: 256-bit YMM kernels only (2x4 main + 256-bit 1x1 remainder)
                        for (; row + 1 < rowEnd; row += 2) {
                            int s = s0;
                            for (; s + 3 < seqEnd; s += 4) {
                                gemm256Tile2x4F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                            }
                        }
                        for (; row < rowEnd; row++) {
                            for (int s = s0; s < seqEnd; s++) {
                    ...
                    case 7 -> { // avx256 2x3
                        for (; row + 1 < rowEnd; row += 2) {
                            int s = s0;
                            for (; s + 2 < seqEnd; s += 3) {
                                gemm256Tile2x3F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                    ...
                    case 8 -> { // avx256 3x4
                        for (; row + 2 < rowEnd; row += 3) {
                            int s = s0;
                            for (; s + 3 < seqEnd; s += 4) {
                                gemm256Tile3x4F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 2, s);
                            }
                        }
                        for (; row < rowEnd; row++) {
                            for (int s = s0; s < seqEnd; s++) {
                    ...
                    case 9 -> { // avx256 4x3
                        for (; row + 3 < rowEnd; row += 4) {
                            int s = s0;
                            for (; s + 2 < seqEnd; s += 3) {
                                gemm256Tile4x3F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 2, s);
                                gemm256Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 3, s);
                            }
                        }
                        for (; row < rowEnd; row++) {
                            for (int s = s0; s < seqEnd; s++) {
                    ...
                    case 10 -> { // neon 4x4 (128-bit)
                        for (; row + 3 < rowEnd; row += 4) {
                            int s = s0;
                            for (; s + 3 < seqEnd; s += 4) {
                                gemm128Tile4x4F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm128Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm128Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                                gemm128Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 2, s);
                                gemm128Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 3, s);
                            }
                        }
                        for (; row < rowEnd; row++) {
                            for (int s = s0; s < seqEnd; s++) {
                    ...
                    case 11 -> { // neon 2x4 (128-bit)
                        for (; row + 1 < rowEnd; row += 2) {
                            int s = s0;
                            for (; s + 3 < seqEnd; s += 4) {
                                gemm128Tile2x4F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                            }
                            for (; s < seqEnd; s++) {
                                gemm128Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                                gemm128Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
                    ...
                    case 12 -> { // pure Java scalar (no Vector API): 4x1 tile + 1x1 remainder
                        for (; row + 3 < rowEnd; row += 4) {
                            for (int sq = s0; sq < seqEnd; sq++) {
                                gemmScalarTile4x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, sq);
                            }
                        }
                        for (; row < rowEnd; row++) {
                            for (int sq = s0; sq < seqEnd; sq++) {
                                gemmScalar1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, sq);
                            }
                        }
                    }
                ...
                for (; row + 2 < rowEnd; row += 3) {
                    int s = s0;
                    for (; s + 1 < seqEnd; s += 2) {
                        gemm512Tile3x2F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                    }
                    for (; s < seqEnd; s++) {
                        gemm512Tile3x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                    }
                }
                if (row + 1 < rowEnd) {
                    int s = s0;
                    for (; s + 1 < seqEnd; s += 2) {
                        gemm512Tile2x2F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                    }
                    for (; s < seqEnd; s++) {
                        gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row, s);
                        gemm512Tile1x1F32(w, a, aBase, outAddr, thatStride, outStride, dim1, thisOffset, row + 1, s);
