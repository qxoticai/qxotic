...
/**
 * Q4_0 register-tiled gemm, relocated from jinfer (segment-based). {@code C = W @ Aᵀ}: weights are
...
public final class Q4Kernel {
    ...
    // 2 weight rows x 4 activation columns; nibbles decoded once per block, shared by all 4
    // columns. 512-bit.
    private static void tile2x4(
            MemorySegment w,
            MemorySegment a,
            long aBase,
            MemorySegment o,
        ...
        for (int j = 0; j < k; j += BLOCK, b0 += TYPE, b1 += TYPE) {
            ...
            var w0b =
                    ByteVector.fromMemorySegment(
                            ByteVector.SPECIES_128, w, b0 + 2, ByteOrder.LITTLE_ENDIAN);
            var w1b =
                    ByteVector.fromMemorySegment(
                            ByteVector.SPECIES_128, w, b1 + 2, ByteOrder.LITTLE_ENDIAN);
            var w0lo =
                    ((FloatVector) w0b.and((byte) 0xF).sub((byte) 8).castShape(F_SPECIES, 0))
                            .mul(vd0);
            var w0hi =
                    ((FloatVector)
                                    w0b.lanewise(VectorOperators.LSHR, 4)
                                            .sub((byte) 8)
                                            .castShape(F_SPECIES, 0))
                            .mul(vd0);
            var w1lo =
                    ((FloatVector) w1b.and((byte) 0xF).sub((byte) 8).castShape(F_SPECIES, 0))
                            .mul(vd1);
            var w1hi =
                    ((FloatVector)
                                    w1b.lanewise(VectorOperators.LSHR, 4)
                                            .sub((byte) 8)
                                            .castShape(F_SPECIES, 0))
                            .mul(vd1);
            long xOff = 4L * j;
            ...
            aHi =
                    FloatVector.fromMemorySegment(
                            F_SPECIES, a, x0 + xOff + 64, ByteOrder.LITTLE_ENDIAN);
            ...
            aHi =
                    FloatVector.fromMemorySegment(
                            F_SPECIES, a, x1 + xOff + 64, ByteOrder.LITTLE_ENDIAN);
            ...
            aHi =
                    FloatVector.fromMemorySegment(
                            F_SPECIES, a, x2 + xOff + 64, ByteOrder.LITTLE_ENDIAN);
            ...
            aHi =
                    FloatVector.fromMemorySegment(
                            F_SPECIES, a, x3 + xOff + 64, ByteOrder.LITTLE_ENDIAN);
    ...
    /**
     * One-row vectorized dot, with scalar fallback for the unaligned head + tail (512/256/128
     * widths).
     */
    static float dot(MemorySegment w, long wOff, MemorySegment a, long aBase, long aOff, int size) {
        ...
        if (alignmentBound > 0) {
            result += scalarDot(w, wOff, a, aBase, aOff, alignmentBound);
            j += alignmentBound;
        }
        ...
        for (; j < upperBound; j += BLOCK, blockOffset += TYPE) {
            ...
            var wBytes =
                    ByteVector.fromMemorySegment(
                            ByteVector.SPECIES_128, w, blockOffset + 2, ByteOrder.LITTLE_ENDIAN);
            ...
            switch (F_SPECIES.vectorBitSize()) {
                ...
                case 128 -> {
                    for (int i = 0; i < 2; ++i) {
                        ...
                        var s0 =
                                av(a, aBase, aOff + j + (i * 4) * len)
                                        .mul(tmp.castShape(F_SPECIES, 0));
                        var s1 =
                                av(a, aBase, aOff + j + (i * 4 + 2) * len)
                                        .mul(tmp.castShape(F_SPECIES, 2));
                        s0 =
                                av(a, aBase, aOff + j + (i * 4 + 1) * len)
                                        .fma(tmp.castShape(F_SPECIES, 1), s0);
                        s1 =
                                av(a, aBase, aOff + j + (i * 4 + 3) * len)
    ...
    static float scalarDot(
            MemorySegment w, long wOff, MemorySegment a, long aBase, long aOff, int size) {
        double sum = 0;
