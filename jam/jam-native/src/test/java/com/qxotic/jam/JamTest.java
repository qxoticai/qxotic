...
class JamTest {
    ...
    /**
     * assert the bounds-checked mm() rejects this call in JAVA (IndexOutOfBoundsException), never
     * reaching native.
     */
    ...
    void fullStridedMatchesContiguous() { // the 7-arg shortcut == the 15-arg full form, contiguous
        try (Arena ar = Arena.ofConfined()) {
            ...
            MemorySegment w = ar.allocate((long) m * k * 4),
                    a = ar.allocate((long) n * k * 4),
                    c = ar.allocate((long) m * n * 4);
            MemorySegment.copy(W, 0, w, JAVA_FLOAT, 0, m * k);
            MemorySegment.copy(A, 0, a, JAVA_FLOAT, 0, n * k);
            assertEquals(
                    JAM.OK,
                    jam.mm(w, a, c, JAM.F32, m, n, k)); // shortcut: offsets 0, strides k/k/m, F32
            float[] out = new float[m * n];
            MemorySegment.copy(c, JAVA_FLOAT, 0, out, 0, m * n);
    ...
    void q8_0Matmul() { // quantize the weight to Q8_0 in Java, run, compare
        try (Arena ar = Arena.ofConfined()) {
            ...
            MemorySegment w =
                    ar.allocate((long) m * nb * 34); // block_q8_0 = { fp16 d; int8 qs[32] }
            float[] wdq = new float[m * k]; // dequantized weight (kernel's view)
            for (int i = 0; i < m; i++)
                for (int b = 0; b < nb; b++) {
                    float amax = 0;
                    for (int t = 0; t < 32; t++)
                        amax = Math.max(amax, Math.abs(Wf[i * k + b * 32 + t]));
                    ...
                    for (int t = 0; t < 32; t++) {
                        int q =
                                Math.max(
                                        -128,
                                        Math.min(127, Math.round(Wf[i * k + b * 32 + t] * id)));
            ...
            for (int i = 0; i < m * n; i++)
                assertEquals(
                        ref[i],
                        c.getAtIndex(JAVA_FLOAT, i),
                        0.05f + 0.05f * Math.abs(ref[i]),
                        "Q8_0 result within int8 activation-requant tolerance");
        }
    }

    @Test
    void denseHalfWeightsSupported() { // F16/BF16 dense weights run (AVX-512 tile, else float
        // floor)
        try (Arena ar = Arena.ofConfined()) {
            int m = 4, n = 2, k = 16;
    ...
    void rejectsHeapSegments() { // array-backed segments have no native address -> reject up front
        try (Arena ar = Arena.ofConfined()) {
            ...
            assertThrows(
                    ...
                    () -> jam.mm(heapW, a, c, JAM.F32, m, n, k),
                    "heap weight");
            MemorySegment w = ar.allocate((long) m * k * 4);
            assertThrows(
                    IllegalArgumentException.class,
                    () -> jam.mm(w, MemorySegment.ofArray(new float[n * k]), c, JAM.F32, m, n, k),
                    "heap activation");
        }
    }

    @Test
    void boundsRejectUndersizedOperands() { // each operand one byte short -> throws; exact-fit
        // -> OK
        try (Arena ar = Arena.ofConfined()) {
            int m = 4, n = 3, k = 64;
            ...
            assertEquals(
                    JAM.OK,
                    jam.mm(w, 0, JAM.F32, k, a, 0, JAM.F32, k, c, 0, JAM.F32, m, m, n, k),
                    "exact fit, no false positive");
            rejects(
                    "weight -1B",
                    () ->
                            jam.mm(
    ...
    void boundsRejectBadOffsets() { // a byte offset must be counted + must be non-negative
        try (Arena ar = Arena.ofConfined()) {
            ...
            MemorySegment w = ar.allocate((long) m * k * 4),
                    ...
                    c = ar.allocate((long) m * n * 4);
            rejects(
                    "weight offset overflow",
                    () -> jam.mm(w, 4, JAM.F32, k, a, 0, JAM.F32, k, c, 0, JAM.F32, m, m, n, k));
            rejects(
                    "act offset overflow",
                    () -> jam.mm(w, 0, JAM.F32, k, a, 4, JAM.F32, k, c, 0, JAM.F32, m, m, n, k));
            rejects(
                    "res offset overflow",
                    () -> jam.mm(w, 0, JAM.F32, k, a, 0, JAM.F32, k, c, 4, JAM.F32, m, m, n, k));
            rejects(
                    "negative offset",
                    () -> jam.mm(w, -4, JAM.F32, k, a, 0, JAM.F32, k, c, 0, JAM.F32, m, m, n, k));
        }
    ...
    void boundsHonorStrides() { // ldw/lda/ldr are ELEMENT strides; size = (rows-1)*stride + lastRow
        try (Arena ar = Arena.ofConfined()) {
            ...
            int ldw = k + pad;
            long wN = (long) (m - 1) * ldw * 4 + (long) k * 4;
            assertEquals(
                    JAM.OK,
                    jam.mm(
                            ar.allocate(wN),
            ...
            int lda = k + pad;
            long aN = (long) (n - 1) * lda * 4 + (long) k * 4;
            assertEquals(
                    JAM.OK,
                    jam.mm(
                            w,
    ...
    @Test
    void boundsBlockAwareAndDense() { // weight bytes are block-aware (quant) / 2-per-elem
        // (F16/BF16)
        try (Arena ar = Arena.ofConfined()) {
            int m = 4, n = 3, k = 256; // k%256 so Q4_K is legal
            MemorySegment a = ar.allocate((long) n * k * 4), c = ar.allocate((long) m * n * 4);
            long q8 = (long) m * (k / 32) * 34,
                    q4k = (long) m * (k / 256) * 144,
                    half = (long) m * k * 2;
            assertEquals(
                    JAM.OK,
                    jam.mm(
    ...
    @Test
    void nativeClassifiesUnsupportedAndDegenerate() { // safe mm must NOT throw where native
        // returns a clean status
        try (Arena ar = Arena.ofConfined()) {
            MemorySegment w = ar.allocate(8192),
                    a = ar.allocate(8192),
                    c = ar.allocate(8192); // generous; bounds never bite
            int m = 2, n = 2, k = 64;
            int Q2_K = 10; // a recognized-but-unsupported GGUF tag (not a JAM.* constant)
            assertEquals(
                    JAM.EUNSUPPORTED,
                    jam.mm(w, 0, Q2_K, k, a, 0, JAM.F32, k, c, 0, JAM.F32, m, m, n, k),
                    "unsupported weight dtype");
            assertEquals(
                    JAM.EUNSUPPORTED,
                    jam.mm(w, 0, JAM.F32, k, a, 0, JAM.F16, k, c, 0, JAM.F32, m, m, n, k),
                    "non-F32 activation");
            assertEquals(
                    JAM.EUNSUPPORTED,
                    jam.mm(w, 0, JAM.F32, k, a, 0, JAM.F32, k, c, 0, JAM.F16, m, m, n, k),
                    "non-F32 result");
            assertEquals(
                    JAM.EUNSUPPORTED,
                    jam.mm(w, 0, JAM.Q8_0, 48, a, 0, JAM.F32, 48, c, 0, JAM.F32, m, m, n, 48),
                    "k not a Q8_0-block multiple");
            assertEquals(
                    JAM.EINVAL,
                    jam.mm(w, 0, JAM.F32, k, a, 0, JAM.F32, k, c, 0, JAM.F32, m, m, 0, k),
                    "n <= 0 (guard skips bounds)");
            assertEquals(
                    JAM.EINVAL,
                    jam.mm(w, 0, JAM.F32, k - 1, a, 0, JAM.F32, k, c, 0, JAM.F32, m, m, n, k),
                    "ldw < k (guard skips bounds)");
        }
    }
    ...
    void nonZeroOffsetMatmul() { // sub-segment placement: address()+off arithmetic + bounds
        try (Arena ar = Arena.ofConfined()) {
            ...
            assertEquals(
                    JAM.OK,
                    jam.mm(w, wOff, JAM.F32, k, a, aOff, JAM.F32, k, c, cOff, JAM.F32, m, m, n, k));
            float[] out = new float[m * n];
    ...
    void concurrentCallsHitEbusy()
        ...
        try (Arena ar = Arena.ofShared()) { // shared: segments touched from worker threads
            ...
            for (int i = 0; i < m * k; i++)
                w.setAtIndex(JAVA_FLOAT, i, (float) Math.sin(i * 0.001));
            for (int i = 0; i < n * k; i++)
                a.setAtIndex(JAVA_FLOAT, i, (float) Math.cos(i * 0.001));
            MemorySegment[] cs = new MemorySegment[threads];
            for (int t = 0; t < threads; t++)
                cs[t] = ar.allocate((long) m * n * 4); // per-thread output, no aliasing
                        ...
                        pool.submit(
                                () -> {
                                    for (int rd = 0; rd < rounds; rd++) {
                                        ...
                                        (st == JAM.OK ? ok : st == JAM.EBUSY ? ebusy : other)
                                                .incrementAndGet();
                                    }
                                }));
            }
    ...
    @Test
    void configResolvesPropertyThenDefault() { // -Dprop, else env (jam.x.y -> JAM_X_Y), else
        // default
        System.setProperty("jam.unit.test.knob", "fromProp");
        try {
            assertEquals(
                    "fromProp",
                    NativeLoader.config("jam.unit.test.knob", "def"),
                    "system property wins");
            assertEquals(
                    "def",
                    NativeLoader.config("jam.unit.test.absent", "def"),
                    "absent -> default (env form jam.x.y -> JAM_X_Y)");
        } finally {
