...
- **Small sharp edges.** An unknown flag in last position is reported as unknown; the server answers 404 for a model name it does not serve and refuses `max_tokens: 0`; the always-reasoning refusal names the lever on every front end; a GBNF root that can only match the empty string is an error; a null embedding batch fails instead of returning nothing; the Narrate and Detect demos report a missing image in one line; LFM2.5 checkpoints whose template never writes a think span report the `NONE` thinking policy.
