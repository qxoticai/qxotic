...
- Scheduling (2026-08-30): the branch's fixed 4-chunks-per-participant claims cost the memory-bound gemv 3% against main's one static band per worker (Q4_K_M 16T tg32 44.2 vs 45.2-45.9; with the chunk count as a property: 1 -> 45.3 but straggler-fragile, 2 -> 45.3, 4 -> 44.3, 16 -> 42.6), while the band gemm needs every panel balanced individually (one band per participant cost it 10%); guided (shrinking) claims sat in between with more variance. The pool now has ONE primitive, `run(jobs, Job)`: jobs claimed one at a time, a job being whatever the caller wants balanced (a panel, a slice, a head-tile). `loop`/`forLoop` is sugar over it for many cheap uniform iterations: the range is presented as `2 x width` contiguous bands, one job each - the one scheduling constant, at one place, with the measurements beside it. `JAM.Parallel` has the same `run` + default `forLoop`. Q4_K_M tg32 44.7-45.1, 8B-A1B 42.4, prefill unchanged. `PerformanceCliff.NESTED_REGION` reports once when a loop is forced inline by nesting; the pool tests bound dispatch at 20 us per region.
- Thread census and the serial scan (2026-08-30): with `ps -T` running-thread counts and `/proc/PID/stat` CPU deltas, every configuration used exactly the caller plus `jinfer-0..N-2`, zero jam threads: decode 16T = 16.0 cores busy, 4T = 4.0, 4T prefill 3.9.
  16T native prefill was only 12.4 cores busy.
  Region accounting (temporary instrumentation) put 17% of the pp2048 wall outside any region, ~170 us per region; a stack sampler of the submitter during those gaps blamed `Convolutions.shortConvScan` (77% of samples: the LFM2 short-conv scan ran single-threaded over seqLen x dim) and `Ops.addInPlace` residual adds.
