...
**AI sovereignty for the JVM.** A complete, open stack for running LLMs locally in Java —
...
- **Model strings.** Wherever a model is expected, a local path or hub reference
  (`hf.co/user/repo:file`, `modelscope.cn/user/repo:file`) works. Downloads are resumable,
  checksum-verified, and cached — warm runs never touch the network.
...
- **Memory-first.** Tensors and activations live in `MemoryView` (jota), shared by jota, jam and
  jinfer — no copies between layers.
