...
public class HuggingFaceLlamaBaseLoader
    ...
    static Map<String, TensorEntry> loadTensorEntries(Path modelPath) throws IOException {
        ...
        return tensors.stream().collect(Collectors.toMap(ti -> ti.name()), ti -> ti);
    }
    ...
    //
    //    static FloatBuffer[] loadArrayOfFloatBuffers(int size, IntFunction<HFTensorEntry>
    // getTensorEntry) {
    ...
    // size, IntFunction<HFTensorEntry> getTensorEntry) {
    ...
    // IntFunction<HFTensorEntry> getTensorEntry) {
    //        ByteBuffer[] byteBuffers = loadArrayOfByteBuffersRearrangeHuggingFace(config, size,
    // getTensorEntry);
    ...
    //    }
    //
    //    static FloatTensor[] loadArrayOfFloats(int size, IntFunction<HFTensorEntry>
    // getTensorEntry) {
    ...
    public Llama.Weights loadWeights(Llama.Configuration config, SpanLoader spanLoader) {
        try (var timer = Timer.log("Load weights")) {
            ...
            FloatMatrixView tokenEmbeddings =
                    tensorAsMatrix(
                            huggingFace.getTensor("model.embed_tokens.weight"), spanLoader, true);
            ...
            FloatMatrixView classifierWeights =
                    huggingFace.containsTensor("lm_head.weight")
                            ? tensorAsMatrix(
                                    huggingFace.getTensor("lm_head.weight"), spanLoader, true)
                            : tokenEmbeddings;
            ...
            return new Llama.Weights(
                    ...
                    loadSpanArray(
                            true,
                            config.numberOfLayers,
                            i -> huggingFace.getTensor("blk." + i + ".attn_q_norm.weight"),
                            spanLoader),
                    loadSpanArray(
                            true,
                            config.numberOfLayers,
                            i -> huggingFace.getTensor("blk." + i + ".attn_k_norm.weight"),
                            spanLoader),
                    ...
                    loadMatrixArray(
                            config.numberOfLayers,
                            i -> huggingFace.getTensor("model.layers." + i + ".mlp.up_proj.weight"),
                            spanLoader),
    ...
    public static FloatMatrixView tensorAsMatrix(
            HFTensorEntry tensorInfo, SpanLoader spanLoader, boolean precomputeRowSpans) {
    ...
    public static FloatSpan[] loadSpanArray(
            int size, IntFunction<HFTensorEntry> getHFTensorEntry, SpanLoader spanLoader) {
    ...
    public static FloatMatrixView[] loadMatrixArray(
        ...
        // size).mapToObj(getHFTensorEntry).map(spanLoader).toArray(FloatSpan[]::new);
    ...
    public static FloatSpan[] loadSpanArray(
            boolean isOptional,
            int size,
            IntFunction<HFTensorEntry> getHFTensorEntry,
