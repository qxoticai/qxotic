...
public class Util {
    ...
    private static final ClassValue<GGMLType> ggmlType = new ClassValue<>() {
        @Override
    ...
    private static final DirectAccessOps<? extends FloatSpan> F32SpanDirectAccess = new DirectAccessOps<F32Span>() {
        @Override
    ...
    private static final DirectAccessOps<? extends FloatSpan> BF16DirectAccess = new DirectAccessOps<BF16Span>() {
        @Override
        public float getElementAt(BF16Span span, long index) {
            // return BFloat16.bfloat16ToFloat(span.memorySegment.getAtIndex(ValueLayout.JAVA_SHORT, index));
            return BFloat16.bfloat16ToFloat(readShort(span.memorySegment, index * BFloat16.BYTES));
        }

        @Override
        public void setElementAt(BF16Span span, long index, float value) {
            // span.memorySegment.setAtIndex(ValueLayout.JAVA_SHORT, index, BFloat16.floatToBFloat16(value));
            writeShort(span.memorySegment, index * BFloat16.BYTES, BFloat16.floatToBFloat16(value));
        }
    };

    private static final DirectAccessOps<? extends FloatSpan> F32BBSpanDirectAccess = new DirectAccessOps<F32BBSpan>() {
        @Override
    ...
    private static final DirectAccessOps<? extends FloatSpan> ArraySpanDirectAccess = new DirectAccessOps<ArraySpan>() {
        @Override
    ...
    private static final DirectAccessOps<? extends FloatSpan> Q8_0DirectAccess = new DirectAccessOps<Q8_0Span>() {
        @Override
        public float getElementAt(Q8_0Span span, long index) {
            ...
            byte quant = readByte(span.memorySegment, blockOffset + Float16.BYTES + withinBlockIndex);
    ...
    private static final DirectAccessOps<? extends FloatSpan> Q8_0BBDirectAccess = new DirectAccessOps<Q8_0BBSpan>() {
        @Override
        public float getElementAt(Q8_0BBSpan span, long longIndex) {
            ...
            byte quant = span.byteBuffer.get(blockOffset + Float16.BYTES + withinBlockIndex);
    ...
    private static final DirectAccessOps<? extends FloatSpan> Q4_0DirectAccess = new DirectAccessOps<Q4_0Span>() {
        @Override
        public float getElementAt(Q4_0Span span, long index) {
            ...
            if (modIndex < GGMLType.Q4_0.getElementsPerBlock() / 2) {
                quant = (byte) (readByte(span.memorySegment, blockOffset + Float16.BYTES + modIndex) & 0x0F);
            } else {
                quant = (byte) ((readByte(span.memorySegment, blockOffset + Float16.BYTES + modIndex - GGMLType.Q4_0.getElementsPerBlock() / 2) >>> 4) & 0x0F);
            }
            quant -= 8;
            return quant * scale;
        }
    ...
    private static final DirectAccessOps<? extends FloatSpan> Q4_1DirectAccess = new DirectAccessOps<Q4_1Span>() {
        @Override
        public float getElementAt(Q4_1Span span, long index) {
            ...
            float scale = Float.float16ToFloat(readShort(span.memorySegment, blockOffset));
            float offset = Float.float16ToFloat(readShort(span.memorySegment, blockOffset + Float16.BYTES));
            byte quant;
            long modIndex = index % GGMLType.Q4_1.getElementsPerBlock();
            if (modIndex < GGMLType.Q4_1.getElementsPerBlock() / 2) {
                quant = (byte) (readByte(span.memorySegment, blockOffset + Float16.BYTES + Float16.BYTES + modIndex) & 0x0F);
            } else {
                quant = (byte) ((readByte(span.memorySegment, blockOffset + Float16.BYTES + Float16.BYTES + modIndex - GGMLType.Q4_1.getElementsPerBlock() / 2) >>> 4) & 0x0F);
            }
    ...
    private static final DirectAccessOps<? extends FloatSpan> Q4_0BBDirectAccess = new DirectAccessOps<Q4_0BBSpan>() {
        @Override
        public float getElementAt(Q4_0BBSpan span, long longIndex) {
            ...
            if (modIndex < GGMLType.Q4_0.getElementsPerBlock() / 2) {
                quant = (byte) (span.byteBuffer.get(blockOffset + Float16.BYTES + modIndex) & 0x0F);
            } else {
                quant = (byte) ((span.byteBuffer.get(blockOffset + Float16.BYTES + modIndex - GGMLType.Q4_0.getElementsPerBlock() / 2) >>> 4) & 0x0F);
            }
            quant -= 8;
            return quant * scale;
        }
    ...
    private static final ClassValue<DirectAccessOps<? extends FloatSpan>> DIRECT_ACCESS = new ClassValue<>() {
        @Override
