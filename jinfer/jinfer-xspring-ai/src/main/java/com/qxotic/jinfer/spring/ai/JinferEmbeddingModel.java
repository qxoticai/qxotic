...
public final class JinferEmbeddingModel implements EmbeddingModel, AutoCloseable {
    ...
    private static Embedding toEmbedding(MemoryView<?> view, int dim, int truncate, int index) {
        ...
        float[] out =
                v.memory()
                        .base()
                        .asSlice(v.byteOffset(), (long) Math.min(dim, truncate) * Float.BYTES)
                        .toArray(ValueLayout.JAVA_FLOAT);
        return new Embedding(out, index);
    }
