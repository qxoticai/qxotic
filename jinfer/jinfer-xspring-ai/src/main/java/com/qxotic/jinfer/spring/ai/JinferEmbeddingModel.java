...
public final class JinferEmbeddingModel implements EmbeddingModel, AutoCloseable {
   ...
   * whose weights YOU loaded can fork - the weights' lifetime is your arena's, so a fork can never
   * dangle. A model that loaded its own weights refuses: it frees them at {@link #close()}, and a
   * fork would outlive them.
   */
   ...
   * arena holding weights and state deterministically; later calls fail with IllegalStateException.
   */
  @Override
  public void close() {
    lock.lock();
  ...
  public List<float[]> embed(
    ...
    for (List<Document> batch : strategy.batch(documents)) {
      List<String> texts = batch.stream().map(d -> loaded.documentPrefix() + text(d)).toList();
  ...
  private static String text(Document document) {
    if (!document.isText()) {
      throw new IllegalArgumentException(
          "document " + document.getId() + " carries media, not text: an embedder embeds text");
    }
  ...
  private EmbeddingResponse doCall(EmbeddingRequest request) {
    ...
    if (options != null && options.getModel() != null && !options.getModel().equals(modelName)) {
      throw new IllegalArgumentException(
          "per-request model is not supported: this model IS '"
              + modelName
              + "' (one loaded GGUF per instance)");
    }
    int outputDimension = loaded.resolveDimension(options == null ? null : options.getDimensions());
    ...
    try {
      total =
          loaded.embedAll(
              ...
              contextLength,
              inputs,
              v -> out.add(new Embedding(loaded.copyEmbedding(v, outputDimension), out.size())));
    } finally {
      lock.unlock();
    }
  ...
  public static final class Builder {
    ...
    /**
     * The model as ONE string: a local GGUF path, a hub ref ({@code hf.co/owner/repo:Q4_K_M}) or a
     * pasted browser URL - resolved by {@link #build()} with the rest of the load, so a remote ref
     * downloads there (see the package doc) and the chain never blocks.
     */
    ...
    /**
     * A model you loaded yourself ({@code Models.loadEmbedder(path, arena)}) - the weight-sharing
     * seam: several instances (and their {@link #fork() forks}) over ONE loaded copy are parallel
     * pipelines for the price of one load.
     *
     * <p>You own its weights arena: {@link JinferEmbeddingModel#close()} frees only this instance's
     * state, so close your arena after every instance built on it, never before. Getting the order
     * wrong sequentially is caught fail-fast (a safety canary throws IllegalStateException at the
     * next request); freeing the arena DURING a request is a data race and can still crash the VM.
     */
