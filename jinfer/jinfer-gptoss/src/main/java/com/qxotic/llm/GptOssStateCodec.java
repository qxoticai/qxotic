 ...
 * uniform kvDim. Full-attention layers store per-position K/V rows (shared base); sliding-window
 * layers (W=128) checkpoint a FIXED-SIZE snapshot: the live window rows {@code [max(0,to-W), to)},
 * read from their ring slots ({@code pos & (W-1)}), valid only at {@code position == to} — which is
 * exactly why blocks match completely or not at all. RoPE (YaRN/NeoX) is baked into K at absolute
 * positions, so restore writes each row back at its true slot. Attention sinks are learned weights,
 * not state; MoE routing is per-token and stateless.
 */
