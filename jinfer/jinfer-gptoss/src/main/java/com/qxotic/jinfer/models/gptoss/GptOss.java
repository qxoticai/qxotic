// gpt-oss against the com.qxotic.jinfer.models model API: a faithful port of the production jinfer
// GptOss
// forward
// (jinfer-models). A MoE transformer with alternating sliding-window/full attention, per-head
// attention
...
// implements only LanguageModel. Mirrors the Lfm2 port's structure: a LayerWeights[]
// record-of-records
...
// +
// projection biases, YaRN RoPE (float[] cos/sin), SWA ring cache on even layers, clampedSwiglu
// experts,
...
public final class GptOss
        ...
        return Parallel.onDecodePool(
                () -> {
                    ...
                    weights.outputWeight()
                            .matmul(s.xb, s.logits, configuration.vocabularySize(), dim);
                    return s.logits;
                });
    }
    ...
    static void addBias(
            FloatTensor out, long outOffset, F32FloatTensor bias, long biasOffset, int size) {
        out.addInPlace(
                outOffset,
                bias,
                biasOffset,
                size); // SIMD via F32FloatTensor.addInPlace (bit-identical to the scalar add)
    }
    ...
    void forward(State state, int[] tokens, int tokenOffset, int startPos, int seqLen) {
        ...
        // Lazy last-layer split: write every row's K/V to the cache but DEFER the attention + MoE
        // FFN tail.
        // state.residual is left as the last-layer INPUT; a query finishes exactly the rows it asks
        // for via
        // tailAt() in logits(). Prefill skips the last layer's (expensive MoE) attention+FFN for
        // un-queried rows.
     ...
     * ({@code scale = 1/sqrt(headSize)}), output projection + bias, residual add. Commits the
     * chunk's K/V to the (ring on SWA, linear on full) cache.
     */
    ...
    // --- MoE FFN (all layers; gpt-oss top-k-by-raw-logit, softmax-over-selected, clampedSwiglu,
    // biases) ---

    /**
     * Pre-norm (post_attention_norm) → router (+bias) → top-k by RAW logit → softmax over the
     * selected (×expertWeightsScale) → per-expert SEPARATE gate/up (+bias) → clampedSwiglu → down
     * (+bias), routed via the shared CSR {@link Moe#dispatch} and added to the residual.
     */
    private void moeFeedForward(State state, int l, FloatTensor resid, int seqLen) {
        ...
        int numExperts = config.expertCount(),
                topK = Math.min(config.expertUsedCount(), numExperts);
        ...
        Parallel.forRows(
                seqLen,
                s -> rmsnorm(state.xb, (long) s * dim, resid, (long) s * dim, postW, dim, eps));
        ...
        for (int s = 0; s < seqLen; s++) {
            ...
            for (int k = 0; k < topK; k++) {
                float ev = (float) Math.exp(state.moeRowTopP[base + k] - maxW);
                state.moeRowTopP[base + k] = ev;
                sum += ev;
            }
            float inv = sum == 0f ? 0f : 1f / sum;
            for (int k = 0; k < topK; k++)
                state.moeRowTopP[base + k] = state.moeRowTopP[base + k] * inv * scaleW;
        }
        ...
        Moe.dispatch(
                ...
                (e, n, gather, out) -> {
                    long gateUpOffset = (long) e * expertFF * dim,
                            downOffset = (long) e * dim * expertFF;
                    moe.gateExps()
                            .gemm(gather, dim, state.hb, expertFF, n, expertFF, dim, gateUpOffset);
                    moe.upExps()
                            .gemm(gather, dim, state.hb2, expertFF, n, expertFF, dim, gateUpOffset);
                    Parallel.forRows(
                            n,
                            j -> {
                                addBias(
                                ...
                                Activations.clampedSwigluMultiply(
                                        state.hb, j * expertFF, state.hb2, j * expertFF, expertFF);
                            });
                    moe.downExps().gemm(state.hb, expertFF, out, dim, n, dim, expertFF, downOffset);
                    Parallel.forRows(
                            n,
                            j -> addBias(out, (long) j * dim, moe.downBias(), (long) e * dim, dim));
                });

        Parallel.forRows(
                seqLen, s -> resid.addInPlace((long) s * dim, state.moeOutB, (long) s * dim, dim));
    }

    // === Lazy last-layer split (writeKv + tailAt) ===

    /**
     * Last-layer K/V half: pre-norm all rows, biased K/V projection + NeoX RoPE(K), commit to the
     * cache (ring on SWA, linear on full). No Q, no attention, no O, no MoE - state.residual left
     * untouched.
     */
     ...
     * input from state.residual[i] and attending cache[attStart..pos] inclusive (its own K/V
     * already committed by writeKv; bK/bV = null, sink-aware, SWA-windowed), then the MoE FFN on
     * state.th. state.residual is never written.
     */
    private void tailAt(State state, int i) {
        ...
        // attention: biased Q + NeoX RoPE + sink/SWA single-query flash over the cache + biased O
        // -> tscratch
        rmsnorm(
                state.tscratch,
                0,
    ...
    public record Configuration(
        ...
        /**
         * SWA layers keep a window-sized ring (capped at the requested context); full layers index
         * the whole context.
         */
        public int kvCachePositions(int layer, int cap) {
            return swaMask[layer] ? Math.min(cap, slidingWindow) : cap;
        }

        public int kvCacheIndex(int layer, int position) {
            return swaMask[layer] ? (position & (slidingWindow - 1)) : position;
        }

        public int attentionStart(int layer, int position) {
            return swaMask[layer] ? Math.max(0, position - slidingWindow + 1) : 0;
        }
    }
    ...
    public record AttentionWeights(
            ...
            FloatTensor wv,
            FloatTensor wo,
            F32FloatTensor qBias,
            F32FloatTensor kBias,
            F32FloatTensor vBias,
            F32FloatTensor oBias,
            F32FloatTensor sinks) {}

    public record MoeFfnWeights(
            FloatTensor router,
            F32FloatTensor routerBias,
            FloatTensor gateExps,
            F32FloatTensor gateBias,
            FloatTensor upExps,
            F32FloatTensor upBias,
            FloatTensor downExps,
            F32FloatTensor downBias) {}
    ...
    public static GptOss loadModel(
        ...
        int ropeOrigCtx =
                gguf.getValueOrDefault(
                        int.class, arch + ".rope.scaling.original_context_length", contextLength);
