...
// within a run.   java ... com.qxotic.llm.GptOssTurnTemplateOracle [model.gguf]
...
public final class GptOssTurnTemplateOracle {
    ...
    public static void main(String[] args) throws Exception {
        ...
        OracleScenario o = new OracleScenario(model, tk -> new GptOssTurnTemplate(tk, today), Map.of());

        o.compare("single user", false, List.of(Message.user("What is the capital of France?")));
        o.compare("single user + gen prompt", true, List.of(Message.user("What is the capital of France?")));
        o.compare("system (developer block) + user", true,
                List.of(Message.system("You are a concise assistant."),
                        Message.user("Give me a haiku about rivers.")));
        o.compare("multi-turn (assistant -> final channel)", true,
        ...
        int start = o.special("<|start|>"), msg = o.special("<|message|>"),
            chan = o.special("<|channel|>"), end = o.special("<|end|>");
        boolean inert = o.count(ids, start) == 1 && o.count(ids, msg) == 1
                && o.count(ids, chan) == 0 && o.count(ids, end) == 1
                && ids.get(0) == start && ids.get(ids.size() - 1) == end
                && o.tokenizer.decode(ids.subList(ids.indexOf(msg) + 1, ids.size() - 1)).equals(hostile.text());
