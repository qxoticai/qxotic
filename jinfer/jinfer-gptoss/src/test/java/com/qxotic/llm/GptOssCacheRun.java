// PromptCache validation on gpt-oss-20b (alternating SWA/full attention + sinks + all-MoE):
// CachedSession + GptOssTurnTemplate + GptOssKvCodec. Proves (1) cold resume restores the whole
// conversation, (2) cached and uncached greedy continuations are token-identical - including with
// a history MANY times the 128-token sliding window, so the SWA ring checkpoints wrap ~10x
...
public final class GptOssCacheRun {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("turn 1: " + tail(decode(a, 200)));
        a.ingest(template.encodeTurn(Message.user("And the smallest? Answer briefly.")));
        System.out.println("turn 2: " + tail(decode(a, 200)));
        ...
        // ================= long conversation (history >> 128: rings wrap ~10x) =================
        ...
        CachedSession<GptOss.State> b = CachedSession.resume(model, cache, model.newState(8192, 512), new long[0]);
        b.ingest(template.conversationStart());
        b.ingest(template.encodeTurn(Message.user(story.toString())));
        System.out.println("long turn 1: " + tail(decode(b, 200)));
        long[] longHist = b.fingerprints();
        check(longHist.length > 1280, "long history wraps the 128-token window many times (" + longHist.length + " > 1280)");
        ...
        CachedSession<GptOss.State> d = CachedSession.resume(model, cache, model.newState(8192, 512), mutated);
        ...
        System.out.println("GptOssCacheRun: all checks passed");
    }
    ...
    static double[] validate(String name, long[] history, Message probe) {
        ...
        CachedSession<GptOss.State> cached = CachedSession.resume(model, cache, model.newState(8192, 512), history);
        ...
        plain.ingest(turn);
        String plainReply = decode(plain, 200);

        check(cachedReply.equals(plainReply), name + ": cached and uncached greedy replies identical");
        System.out.println(name + " reply: " + tail(cachedReply));
        return new double[]{resumeMs, replayMs, cachedReply.isEmpty() ? 0 : lastReplyTokens / decodeSec};
    }

    static int lastReplyTokens;

    /** Open the assistant turn, greedy-decode (each step a single-token block; the model emits its
     *  own harmony channel tokens), close the open message. */
    static String decode(CachedSession<GptOss.State> s, int maxTokens) {
    ...
    /** Harmony replies start with the analysis channel; show just the final stretch for logs. */
    static String tail(String reply) {
