// Diff the new com.qxotic.llm.GptOss port against the production com.qxotic.jinfer GptOss on the same
// tokens: prefill both, then several decode steps, comparing next-token argmax and max per-logit diff.
//   java ... com.qxotic.llm.GptOssCompare <model.gguf> [prompt]
...
public final class GptOssCompare {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        String prompt = args.length > 1 ? args[1] : "The capital of France is";

        GptOss nw = GptOss.loadModel(Path.of(path), 4096);
        var tk = nw.tokenizer();
        ...
        com.qxotic.jinfer.ModelLegacy old = LegacyModelLoader.loadModel(Path.of(path), 4096);
        var os = old.createNewState();
        old.ingest(os, ids, 0, 0, ids.length);
        FloatTensor ol = old.computeLogits(os);

        GptOss.State ns = nw.newState(4096, Math.max(16, ids.length));
        ...
        // decode steps: feed the SAME token (old's argmax) to both, compare each step's logits.
        int tok = oTok;
