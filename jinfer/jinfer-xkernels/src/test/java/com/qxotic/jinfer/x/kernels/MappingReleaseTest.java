...
/**
 * The weights-lifetime law: {@link ModelLoader#loadTensors} maps tensor data into the caller's
 * arena, so with an automatic arena the mapping lives while any tensor view is reachable and is
 * UNMAPPED by GC once the graph is dropped - loading models can never leak mappings for the life of
 * the process. Verified against the ground truth ({@code /proc/self/maps}), so it runs on Linux
 * only.
 */
class MappingReleaseTest {
    ...
    void droppedTensorGraphUnmaps() throws Exception {
        ...
        try {
            ...
            Map<String, MemoryView<MemorySegment>> entries = load(file);
    ...
    private static Map<String, MemoryView<MemorySegment>> load(Path file) throws IOException {
