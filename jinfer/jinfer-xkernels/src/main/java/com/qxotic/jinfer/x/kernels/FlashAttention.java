...
public final class FlashAttention {
    ...
    /**
     * A K/V source: F32 or F16, extracted once at kernel entry (the old {@code instanceof
     * F16FloatTensor} dispatch, now a dtype check + a boolean).
     */
    record Src(Raw raw, boolean f16) {}

    private static Src src(MemoryView<MemorySegment> view, String name) {
        Views.requireContiguous(view, name);
        DataType dt = view.dataType();
        if (dt != DataType.FP32 && dt != DataType.FP16) {
            throw new IllegalArgumentException(
                    name + ": expected FP32 or FP16 but was " + dt.name());
        }
        MemorySegment segment = view.memory().base();
        return new Src(
                new Raw(
                        Segments.vectorSegment(segment),
                        Segments.vectorBase(segment) + view.byteOffset()),
                dt == DataType.FP16);
    }

    /** Scalar element read of a source (the old virtual {@code getFloat}). */
    @AlwaysInline(
            "hot scalar accessor: must inline into kernels (profiled out-of-line on CE native)")
    private static float srcFloat(Src s, long elemOff) {
        return s.f16()
                ? readFloat16(s.raw().vseg(), s.raw().vbase() + elemOff * F16_BYTES)
                : readFloat(s.raw().vseg(), s.raw().vbase() + elemOff * Float.BYTES);
    }
    ...
    static final class Buffers {
        ...
        MemorySegment kDec, vDec; // F16-cache block decode scratch, grown to Bc*headSize
        Raw kDecRaw, vDecRaw;

        Raw kDec(int capacity) {
            if (kDec == null || kDec.byteSize() < (long) capacity * Float.BYTES) {
                kDec = Arena.ofAuto().allocate((long) capacity * Float.BYTES, 64);
                kDecRaw = new Raw(Segments.vectorSegment(kDec), Segments.vectorBase(kDec));
            }
            return kDecRaw;
        }

        Raw vDec(int capacity) {
            if (vDec == null || vDec.byteSize() < (long) capacity * Float.BYTES) {
                vDec = Arena.ofAuto().allocate((long) capacity * Float.BYTES, 64);
                vDecRaw = new Raw(Segments.vectorSegment(vDec), Segments.vectorBase(vDec));
            }
            return vDecRaw;
        }
    }
        ...
        for (int j = 0; j < count; j++) {
            ...
            for (int d = bound; d < headSize; d++) {
                writeFloat(
                        ...
                        readFloat16(
                                src.raw().vseg(), src.raw().vbase() + (long) (so + d) * F16_BYTES));
            }
        }
    }
    ...
    /** {@code out[off, off+n) = value}, the old F32 {@code fillInPlace}. */
    private static void fillF32(Raw out, long off, int n, float value) {
        if (Segments.USE_VECTOR_API) {
            FloatVector fill = FloatVector.broadcast(Segments.F_SPECIES, value);
            int upperBound = Segments.F_SPECIES.loopBound(n);
            int i = 0;
            for (; i < upperBound; i += Segments.F_SPECIES.length()) {
                fill.intoMemorySegment(
                        out.vseg(), out.vbase() + (off + i) * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
            }
            for (; i < n; i++) {
                writeFloat(out.vseg(), out.vbase() + (off + i) * Float.BYTES, value);
            }
            return;
        }
        for (int i = 0; i < n; i++) {
    ...
    static void normalize(Raw out, int outOffset, int headSize, float scale) {
        if (Segments.USE_VECTOR_API) {
            ...
            int upperBound = Segments.F_SPECIES.loopBound(headSize);
            for (int i = 0; i < upperBound; i += Segments.F_SPECIES.length()) {
                long byteOffset = out.vbase() + (long) (outOffset + i) * Float.BYTES;
                FloatVector.fromMemorySegment(
                                Segments.F_SPECIES, out.vseg(), byteOffset, ByteOrder.LITTLE_ENDIAN)
                        .mul(scaleVector)
                        .intoMemorySegment(out.vseg(), byteOffset, ByteOrder.LITTLE_ENDIAN);
            }
            for (int i = upperBound; i < headSize; i++) {
                long byteOffset = out.vbase() + (long) (outOffset + i) * Float.BYTES;
                writeFloat(out.vseg(), byteOffset, readFloat(out.vseg(), byteOffset) * scale);
            }
            return;
        }
    ...
    static void accumulate(
        ...
        if (Segments.USE_VECTOR_API) {
            ...
            int upperBound = Segments.F_SPECIES.loopBound(headSize);
            if (!value.f16()) {
                Raw valueF32 = value.raw();
                for (int d = 0; d < upperBound; d += Segments.F_SPECIES.length()) {
                    long byteOffset = out.vbase() + (long) (outOffset + d) * Float.BYTES;
                    FloatVector acc =
                            FloatVector.fromMemorySegment(
                                    Segments.F_SPECIES,
                                    out.vseg(),
            ...
            for (int d = upperBound; d < headSize; d++) {
                ...
                writeFloat(
                        out.vseg(),
                        byteOffset,
                        readFloat(out.vseg(), byteOffset)
                                + srcFloat(value, valueOffset + d) * scale);
            }
            return;
        }
        for (int d = 0; d < headSize; d++) {
            ...
            writeFloat(
                    out.vseg(),
                    byteOffset,
                    readFloat(out.vseg(), byteOffset) + srcFloat(value, valueOffset + d) * scale);
        }
    }
    ...
    private static void pvTileF16(
        ...
        for (int d = 0; d < bound; d += len) {
            ...
            o0.intoMemorySegment(
                    out.vseg(),
                    out.vbase() + (long) (ob0 + d) * Float.BYTES,
                    ByteOrder.LITTLE_ENDIAN);
            o1.intoMemorySegment(
                    out.vseg(),
                    out.vbase() + (long) (ob1 + d) * Float.BYTES,
                    ByteOrder.LITTLE_ENDIAN);
            o2.intoMemorySegment(
                    out.vseg(),
                    out.vbase() + (long) (ob2 + d) * Float.BYTES,
                    ByteOrder.LITTLE_ENDIAN);
            o3.intoMemorySegment(
                    out.vseg(),
                    out.vbase() + (long) (ob3 + d) * Float.BYTES,
                    ByteOrder.LITTLE_ENDIAN);
        }
    }
    ...
    private static void qkRowF32(
        ...
        for (int k = 0; k < nKeys; k++) {
            ...
            for (int d = bound; d < headSize; d++) {
                s +=
                        readFloat(q.vseg(), q.vbase() + (long) (qOffset + d) * Float.BYTES)
                                * readFloat(
                                        key.vseg(), key.vbase() + (long) (ko + d) * Float.BYTES);
            }
    ...
    private static void qkTileWideF32(
        ...
        for (int k = 0; k < nKeys; k++) {
            int ko = kvOff[runStart + k];
            FloatVector a0 = FloatVector.zero(Segments.F_SPECIES),
                    a1 = FloatVector.zero(Segments.F_SPECIES),
                    a2 = FloatVector.zero(Segments.F_SPECIES),
                    a3 = FloatVector.zero(Segments.F_SPECIES);
    ...
    static void qkTile(
        ...
        if (len == 16 && !key.f16()) {
            if (headSize == 64) {
                qkTile64F32(
                        q, qBase, qStride, key.raw(), kvOff, runStart, nKeys, scale, S, sRow0,
                        BcRows);
                return;
            }
            if ((headSize & 63) == 0) {
                qkTileWideF32(
                        q, qBase, qStride, key.raw(), kvOff, runStart, nKeys, headSize, scale, S,
                        sRow0, BcRows);
                return;
            }
        }
        ...
        for (int k = 0; k < nKeys; k++) {
            ...
            if (f16) {
                for (int d = 0; d < bound; d += len) {
                    FloatVector kv = loadF16(kRaw, ko + d);
                ...
                for (int d = 0; d < bound; d += len) {
                    FloatVector kv = loadF32(kRaw, ko + d);
                    a0 = loadF32(q, qb0 + d).fma(kv, a0);
                    a1 = loadF32(q, qb1 + d).fma(kv, a1);
                    a2 = loadF32(q, qb2 + d).fma(kv, a2);
                    a3 = loadF32(q, qb3 + d).fma(kv, a3);
            ...
            for (int d = bound; d < headSize; d++) {
                float kvf = srcFloat(key, ko + d);
                s0 += readFloat(q.vseg(), q.vbase() + (long) (qb0 + d) * Float.BYTES) * kvf;
                s1 += readFloat(q.vseg(), q.vbase() + (long) (qb1 + d) * Float.BYTES) * kvf;
                s2 += readFloat(q.vseg(), q.vbase() + (long) (qb2 + d) * Float.BYTES) * kvf;
                s3 += readFloat(q.vseg(), q.vbase() + (long) (qb3 + d) * Float.BYTES) * kvf;
            }
            int col = runStart + k;
    ...
    static void pvTile(
        ...
        for (int d = bound; d < headSize; d++) {
            float r0 = readFloat(out.vseg(), out.vbase() + (long) (ob0 + d) * Float.BYTES),
                    r1 = readFloat(out.vseg(), out.vbase() + (long) (ob1 + d) * Float.BYTES),
                    r2 = readFloat(out.vseg(), out.vbase() + (long) (ob2 + d) * Float.BYTES),
                    r3 = readFloat(out.vseg(), out.vbase() + (long) (ob3 + d) * Float.BYTES);
            for (int k = 0; k < nKeys; k++) {
                int col = runStart + k;
                float vf = srcFloat(value, kvOff[col] + d);
                r0 += P[p0 + col] * vf;
                r1 += P[p1 + col] * vf;
                r2 += P[p2 + col] * vf;
                r3 += P[p3 + col] * vf;
            }
            writeFloat(out.vseg(), out.vbase() + (long) (ob0 + d) * Float.BYTES, r0);
            writeFloat(out.vseg(), out.vbase() + (long) (ob1 + d) * Float.BYTES, r1);
            writeFloat(out.vseg(), out.vbase() + (long) (ob2 + d) * Float.BYTES, r2);
            writeFloat(out.vseg(), out.vbase() + (long) (ob3 + d) * Float.BYTES, r3);
        }
    }

    // ---- score dots: the decode/rolling paths' q·k (old virtual FloatTensor.dot) -------------

    /** F32·F32 single-accumulator vector dot (old {@code F32FloatTensor.vectorDot}). */
    private static float vectorDotF32(Raw a, long aOff, Raw b, long bOff, int size) {
        FloatVector val = FloatVector.zero(Segments.F_SPECIES);
        int upperBound = Segments.F_SPECIES.loopBound(size);
        for (int i = 0; i < upperBound; i += Segments.F_SPECIES.length()) {
            var av =
                    FloatVector.fromMemorySegment(
                            Segments.F_SPECIES,
                            a.vseg(),
                            a.vbase() + (aOff + i) * Float.BYTES,
                            ByteOrder.LITTLE_ENDIAN);
            var bv =
                    FloatVector.fromMemorySegment(
                            Segments.F_SPECIES,
                            b.vseg(),
                            b.vbase() + (bOff + i) * Float.BYTES,
                            ByteOrder.LITTLE_ENDIAN);
            val = av.fma(bv, val);
        }
        ...
        for (int i = upperBound; i < size; i++) {
            result +=
                    readFloat(a.vseg(), a.vbase() + (aOff + i) * Float.BYTES)
                            * readFloat(b.vseg(), b.vbase() + (bOff + i) * Float.BYTES);
        }
        return result;
    }

    /** F16·F32 single-accumulator vector dot (old {@code F16FloatTensor.vectorDotF32}). */
    private static float vectorDotF16F32(Raw f16, long f16Off, Raw f32, long f32Off, int size) {
        assert Segments.S_SPECIES_HALF.length() == Segments.F_SPECIES.length();
        FloatVector val = FloatVector.zero(Segments.F_SPECIES);
        int upperBound = Segments.F_SPECIES.loopBound(size);
        for (int i = 0; i < upperBound; i += Segments.F_SPECIES.length()) {
        ...
        for (int i = upperBound; i < size; i++) {
            result +=
                    readFloat16(f16.vseg(), f16.vbase() + (f16Off + i) * F16_BYTES)
                            * readFloat(f32.vseg(), f32.vbase() + (f32Off + i) * Float.BYTES);
        }
        return result;
    }

    /** {@code q[qOff, +n) . k[kOff, +n)} for an F32 query against an F32-or-F16 key source. */
    private static float dotQK(Raw q, long qOff, Src k, long kOff, int n) {
        if (Segments.USE_VECTOR_API) {
            return k.f16()
                    ? vectorDotF16F32(k.raw(), kOff, q, qOff, n)
                    : vectorDotF32(q, qOff, k.raw(), kOff, n);
        }
        float sum = 0;
        for (int i = 0; i < n; i++) {
            sum +=
                    readFloat(q.vseg(), q.vbase() + (qOff + i) * Float.BYTES)
                            * srcFloat(k, kOff + i);
        }
        return sum;
    }
    ...
    public static void causalPrefill(
            ...
            MemoryView<MemorySegment> cK,
            MemoryView<MemorySegment> cV,
            int nHeads,
            int startPos,
            int seqLen,
            int headSize,
        ...
        Parallel.parallelFor(
                ...
                h -> {
                        ...
                        int off =
                                cur
                                        ...
                                        : ((ringMask != 0 ? (t & ringMask) : t) * kvDim
                                                + kvHeadOffset);
                        float score = dotQK(q, qOff, cur ? bK : cK, off, headSize) * scale;
                    ...
                    if (sinks == null) {
                        ...
                        float sink = readFloat(sinks.vseg(), sinks.vbase() + h * Float.BYTES);
    ...
    public static final class DecodeScratch {
        ...
        private Raw oRaw() {
            return new Raw(Segments.vectorSegment(o), Segments.vectorBase(o));
        }
    }
    ...
    public static void flashDecode(
        ...
        Parallel.parallelFor(
                ...
                h -> {
                    ...
                    fillF32(outRaw, qOff, headSize, 0f);
                    double gL = 0.0;
                    for (int p = 0; p < nParts; p++) {
                        ...
                        accumulate(outRaw, qOff, pOSrc, task * headSize, headSize, w);
                        gL += pL[task] * w;
                    }
                    if (sinksRaw == null) {
                        normalize(outRaw, qOff, headSize, gL == 0.0 ? 0f : (float) (1.0 / gL));
                    } else {
                        float sink = readFloat(sinksRaw.vseg(), sinksRaw.vbase() + h * Float.BYTES);
                        ...
                        normalize(
                                ...
                                qOff,
                                headSize,
                                denom == 0.0 ? 0f : (float) (factor / denom));
                    }
                });
    }
