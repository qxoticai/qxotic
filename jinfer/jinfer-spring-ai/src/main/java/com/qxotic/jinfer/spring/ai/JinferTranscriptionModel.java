// Spring AI TranscriptionModel backed by jinfer: in-process CPU speech recognition over a local
// GGUF, no server. Names no port - either you pass a path and architecture dispatch finds one, or
// you pass a model you loaded yourself.
...
/**
 * Spring AI {@code TranscriptionModel} backed by jinfer: in-process CPU speech recognition over a
 * local GGUF. {@link #call} returns the transcript; the typed {@code transcribe} doors return the
 * full {@link Transcription}, token spans and confidences included.
 *
    ...
    public AudioTranscriptionResponse call(AudioTranscriptionPrompt prompt) {
        ...
        Resource audio =
                Objects.requireNonNull(
                        prompt.getInstructions(), "prompt audio resource must not be null");
        // A silently ignored request parameter is a lie; this instance decodes what it decodes.
        AudioTranscriptionOptions options = prompt.getOptions();
        if (options != null && options.getModel() != null && !options.getModel().isBlank())
            throw new UnsupportedOperationException(
                    "model '"
                            + options.getModel()
                            + "' is not supported: this instance is bound to the loaded GGUF");
        byte[] bytes;
        try {
            bytes = audio.getContentAsByteArray();
        } catch (IOException e) {
            throw new UncheckedIOException("failed to read " + audio.getDescription(), e);
        }
        return new AudioTranscriptionResponse(new AudioTranscription(transcribe(bytes).text()));
    }
