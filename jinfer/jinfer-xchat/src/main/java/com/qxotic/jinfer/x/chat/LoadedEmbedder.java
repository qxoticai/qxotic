...
public record LoadedEmbedder<S extends RuntimeState>(
  ...
  public int resolveDimension(Integer requested) {
    ...
    if (!supportsCustomDimensions())
      throw new IllegalArgumentException(name + " has a fixed embedding dimension of " + dimension);
  ...
  /**
   * Copies one reused native embedding row to the heap. Native-width output stays bit-identical; a
   * shorter Matryoshka prefix is L2-normalized as prescribed by the model.
   */
  public float[] copyEmbedding(MemoryView<?> view, int outputDimension) {
    if (outputDimension < minimumDimension || outputDimension > dimension)
      throw new IllegalArgumentException(
          "outputDimension " + outputDimension + " out of " + minimumDimension + ".." + dimension);
    ...
    if (source.shape().size() != dimension)
      throw new IllegalArgumentException(
          "embedding: expected " + dimension + " values but received " + source.shape().size());
   ...
   * ONE forward pass per group, so RAG ingestion of hundreds of chunks costs a handful of prefills.
   * {@code sink} receives one pooled vector per text, in order; it may be a REUSED per-state buffer
   * - copy it out before returning. A text longer than {@code contextLength} throws {@link
   * IllegalArgumentException}. Returns the exact total token count. Serialize calls per state (the
   * state is one pipeline); {@code state} must come from {@link #model()}.
   */
  public int embedAll(
      RuntimeState state, int contextLength, List<String> texts, Consumer<MemoryView<?>> sink) {
    ...
    for (int i = 0; i < texts.size(); i++) {
      ...
      System.arraycopy(
          sequenceSuffix, 0, seq, sequencePrefix.length + ids.length, sequenceSuffix.length);
