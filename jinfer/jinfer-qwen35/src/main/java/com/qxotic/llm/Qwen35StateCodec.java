 ...
 *  plus the delta-net S matrix — that only exists at the state's current position, which is exactly why
 *  blocks match completely or not at all. MoE routing is per-row and stateless. The S matrix is a heap
 *  array, copied via a heap segment view. */
public final class Qwen35StateCodec extends AbstractStateCodec<Qwen35.State> {
    ...
    public Qwen35StateCodec(Qwen35.Configuration config) {
        super(config.numberOfLayers, l -> config.isFullAttention[l], l -> config.kvDim(),
              s -> s.keyCache, s -> s.valueCache,
              l -> config.isFullAttention[l] ? 0L : (convFloats(config) + ssmFloats(config)) * 4L);
        this.config = config;
        this.convFloats = convFloats(config);
    }

    private static int convFloats(Qwen35.Configuration c) { return (c.ssmConvKernel - 1) * c.convChannels(); }
    private static int ssmFloats(Qwen35.Configuration c) { return c.headVDim() * c.headVDim() * c.ssmTimeStepRank; }
