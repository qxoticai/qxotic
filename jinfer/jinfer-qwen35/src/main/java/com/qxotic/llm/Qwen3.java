// Qwen3 dense as an EMBEDDING model (Qwen3-Embedding) against the com.qxotic.jinfer API. Qwen3 dense is
// the standard Llama transformer (GQA + SwiGLU + RMSNorm pre-norm, no attention bias) with two twists:
...
// The embedding is the LAST token's hidden state after the final RMSNorm, L2-normalized (pooling_type=LAST,
// matching llama.cpp --pooling last). No LM head / vocab projection. Single batched causal prefill, no decode.
...
public final class Qwen3 implements EmbeddingModel<Qwen3.Configuration, Qwen3.Weights, Qwen3.State> {
    ...
    /** GQA attention: Q/K/V projections, per-head QK RMS-norm, NeoX RoPE, causal flash attention, output
     *  projection back into {@code state.xb}. */
    private void attention(State state, int layer, int startPos, int seqLen) {
        ...
        Parallel.forRows(seqLen, s -> {
            for (int h = 0; h < fHeads; h++) {
                ...
                rmsnorm(state.attnQ, off, state.attnQ, off, qNorm, fHeadSz, eps);   // per-head QK-norm before RoPE
    ...
    public record Weights(FloatTensor tokenEmbeddingTable, F32FloatTensor outputNorm,
    ...
    public static final class State implements RuntimeState {
        ...
        State(Configuration config, int contextCapacity, int batchCapacity) {
            ...
            int dim = config.embeddingLength, queryDim = config.queryDim(), kvDim = config.kvDim(), hidden = config.hiddenDim;
    ...
    static Qwen3 loadModel(FileChannel fileChannel, GGUF gguf, int contextLength, boolean loadWeightsFlag) throws IOException {
        ...
        Pair<float[], float[]> rope = RoPE.precomputeFreqsCis(config.contextLength, ropeDim, config.ropeTheta);
