...
public final class Qwen35Provider implements ModelProvider {
    ...
    public LoadedModel<?> load(FileChannel fileChannel, GGUF gguf, int contextLength)
        ...
        return Qwen35.loadModel(fileChannel, gguf, contextLength, true).loaded();
    }

    @Override
    public LoadedEmbedder<?> loadEmbedder(FileChannel fileChannel, GGUF gguf, int contextLength)
            throws IOException {
        if (!"qwen3".equals(gguf.getString("general.architecture"))) {
            return ModelProvider.super.loadEmbedder(fileChannel, gguf, contextLength);
        }
        Qwen3 m = Qwen3.loadModel(fileChannel, gguf, contextLength, true);
