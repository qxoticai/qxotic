...
/** Draft-and-verify loop for BailingMoe3's embedded single-layer NextN head. */
final class BailingMoe3Speculative {

    private BailingMoe3Speculative() {}

    static final class Scratch {
        final MemoryView<MemorySegment> verificationLogits;
        final BailingMoe3CheckpointCodec codec;
        final MemorySegment checkpoint;
        final int depth;

        Scratch(BailingMoe3 model, BailingMoe3.State state, int depth) {
            ...
            codec = new BailingMoe3CheckpointCodec(model.configuration());
    ...
    static SpeculationResult generate(
        ...
        if (!model.speculationReady())
            throw new IllegalStateException("BailingMoe3 model has no embedded NextN layer");
    ...
    private static SpeculationResult walk(
        ...
        long deadline =
                timeoutNanos > 0 && timeoutNanos <= Long.MAX_VALUE - started
                        ...
                        : Long.MAX_VALUE;
        int vocab = model.configuration().vocabularySize();
        BailingMoe3CheckpointCodec codec = scratch.codec;
