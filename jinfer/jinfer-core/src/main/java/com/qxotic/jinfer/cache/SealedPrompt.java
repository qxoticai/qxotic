 ...
 * build time and restored in milliseconds at serve time — the "prompt compiler" artifact for CPU
 * deployments (native image + sidecar file). Tree-less and hash-less on purpose: one known prompt
 * needs no keys or matching, just a fingerprint memcmp. A mismatch (different prompt, different
 * model) is always a clean fall-through to plain prefill, never a wrong restore.
 *
 ...
 * fingerprints are the only pages touched before a restore. The model seed (see {@link
 * PromptCache#modelSeed}) also covers the codec blob layout: any layout change ships as a
...
public final class SealedPrompt {
    ...
    public static <S extends RuntimeState> void compile(
        ...
        if (state.position() != n) {
            throw new IllegalStateException(
                    "state at " + state.position() + ", prompt is " + n + " positions");
        }
        ...
        try (FileChannel ch =
            ...
            MemorySegment map =
                    ch.map(FileChannel.MapMode.READ_WRITE, 0, kvOffset + kvBytes, arena);
            ByteBuffer header =
                    map.asSlice(0, fpOffset).asByteBuffer().order(ByteOrder.LITTLE_ENDIAN);
            header.putInt(MAGIC)
                    .putInt(FORMAT_VERSION)
                    .put(modelSeed, 0, 32)
                    .putInt(nameUtf8.length)
                    .put(nameUtf8)
                    .putInt(n);
            MemorySegment fp = map.asSlice(fpOffset, (long) n * Long.BYTES);
            for (int i = 0; i < n; i++)
                fp.setAtIndex(
                        java.lang.foreign.ValueLayout.JAVA_LONG_UNALIGNED, i, fingerprints[i]);
            MemorySegment kv = map.asSlice(kvOffset, kvBytes);
            codec.saveRows(state, 0, n, kv);
            codec.saveCheckpoint(
                    state, n, kv.asSlice(codec.rowBytes(n))); // sealed span: always checkpointed
            map.force();
        }
    }
     ...
     * modelSeed}; throws a descriptive error when it does not. The returned instance holds the
     * mapping for the JVM's lifetime (global arena) — sealed prompts are process-lifetime
     * artifacts.
     */
    public static SealedPrompt open(Path file, byte[] modelSeed) throws IOException {
        try (FileChannel ch = FileChannel.open(file, StandardOpenOption.READ)) {
            ...
            ByteBuffer header =
                    map.asSlice(0, Math.min(map.byteSize(), 4096))
                            .asByteBuffer()
            ...
            if (version != FORMAT_VERSION) {
                throw new IllegalStateException(
                        file
                                + " has sealed-prompt format v"
                                + version
                                + ", this build reads v"
                                + FORMAT_VERSION
                                + "; recompile the prompt");
            }
            ...
            if (!java.util.Arrays.equals(seed, 0, 32, modelSeed, 0, 32)) {
                throw new IllegalStateException(
                        "sealed prompt "
                                ...
                                + hex(modelSeed)
                                + "); the cache is model-specific - recompile it or load the"
                                + " matching GGUF");
            }
            int n = header.getInt();
            long fpOffset = align(4 + 4 + 32 + 4 + nameUtf8.length + 4);
            long kvOffset = align(fpOffset + (long) n * Long.BYTES);
            return new SealedPrompt(
                    file,
                    name,
    ...
    /**
     * Restores the sealed prompt into {@code state} iff it is a prefix of {@code requestFp} (plain
     * comparison, no hashing) and returns the positions resumed; returns 0 on any mismatch — the
     * caller prefills normally, a miss never degrades correctness.
     */
    public <S extends RuntimeState> int tryRestore(S state, StateCodec<S> codec, long[] requestFp) {
        int n = positions();
        if (requestFp.length < n) return 0;
        for (int i = 0; i < n; i++) {
            if (fingerprints.getAtIndex(java.lang.foreign.ValueLayout.JAVA_LONG_UNALIGNED, i)
                    != requestFp[i]) return 0;
        }
