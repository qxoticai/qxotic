 ...
 * predicate, the per-layer {@code kvDim}, and key/value accessors; the block sizes are accumulated
 * once from those. A concrete codec supplies that wiring plus, for hybrid models, the genuinely
 * model-specific {@link #checkpoint} (recurrent/windowed state). Pure-attention models leave the
 * checkpoint at its no-op default ({@code checkpointBytes == 0}, every block a resume point). Save
 * and restore share one walk (a direction flag), so the two blob layouts can't drift apart.
 */
public abstract class AbstractStateCodec<S extends RuntimeState> implements StateCodec<S> {
    ...
    /**
     * @param checkpointLayerBytes checkpoint bytes contributed by layer {@code l} (0 for a row
     *     layer).
     */
    ...
    @Override
    public final long rowBytes(int positions) {
        return positions * bytesPerPosition;
    }

    @Override
    public final long checkpointBytes() {
        return checkpointBytes;
    }

    @Override
    public final void saveRows(S s, int from, int to, MemorySegment dst) {
        rows(s, from, to, dst, true);
    }

    @Override
    public final void restoreRows(S s, int from, int to, MemorySegment src) {
        rows(s, from, to, src, false);
    }

    @Override
    public final void saveCheckpoint(S s, int to, MemorySegment dst) {
        checkpoint(s, to, dst, true);
    }

    @Override
    public final void restoreCheckpoint(S s, int to, MemorySegment src) {
        checkpoint(s, to, src, false);
    }

    /**
     * The shared rows walk (one direction per {@code out}): row layer l → K rows {@code [from,to)}
     * then V rows, native F16, layer-major.
     */
    ...
    /**
     * Serialize ({@code out=true}) or restore the model's recurrent/windowed checkpoint as of
     * position {@code to}. Default no-op — pure-attention models ({@code checkpointBytes == 0})
     * never reach it.
     */
