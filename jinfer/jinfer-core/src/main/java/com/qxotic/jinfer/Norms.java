...
public final class Norms {
    ...
    public static void rmsnorm(FloatTensor out, FloatTensor x, F32FloatTensor weight, int size, float eps) {
    ...
    public static float sumOfSquares(FloatTensor x, long xOffset, int size) {
        if (x instanceof F32FloatTensor xF32 && FloatTensor.USE_VECTOR_API) {
            ...
            float ss = acc.reduceLanes(VectorOperators.ADD);
            for (; i < size; i++) { float xi = x.getFloat(xOffset + i); ss += xi * xi; }
            return ss;
        }
        float ss = 0f;
        for (int i = 0; i < size; i++) { float xi = x.getFloat(xOffset + i); ss += xi * xi; }
        return ss;
    }
    ...
    public static void scaleByWeight(FloatTensor out, long outOffset, FloatTensor x, long xOffset, F32FloatTensor weight, int size, float scale) {
        if (out instanceof F32FloatTensor outF32 && x instanceof F32FloatTensor xF32 && FloatTensor.USE_VECTOR_API) {
            ...
            for (; i < upperBound; i += species.length()) {
                var xv = FloatVector.fromMemorySegment(species, xF32.vseg, xF32.vbase + (long) (xOffset + i) * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
                var wv = FloatVector.fromMemorySegment(species, weight.vseg, weight.vbase + (long) i * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
                wv.mul(sv).mul(xv).intoMemorySegment(outF32.vseg, outF32.vbase + (long) (outOffset + i) * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
            }
            for (; i < size; i++) out.setFloat(outOffset + i, weight.getFloat(i) * scale * x.getFloat(xOffset + i));
            return;
        }
        for (int i = 0; i < size; i++) out.setFloat(outOffset + i, weight.getFloat(i) * scale * x.getFloat(xOffset + i));
    }
