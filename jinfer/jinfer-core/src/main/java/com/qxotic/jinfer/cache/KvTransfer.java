...
public final class KvTransfer {
     ...
     *  {@code pos & (w-1)} (the span wraps at most once, so at most two contiguous runs), padded
     *  to {@code w} rows in the blob. {@code elemBytes} is the ring's native element width (2 for
     *  F16). Returns the bytes covered — always {@code w * rowElems * elemBytes}, pad included —
     *  so save and restore advance identically. */
    public static long window(FloatTensor ring, int to, int w, long rowElems, int elemBytes,
                              MemorySegment blob, long byteOff, boolean out) {
