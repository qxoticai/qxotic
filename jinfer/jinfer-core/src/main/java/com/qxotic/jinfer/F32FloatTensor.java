...
public final class F32FloatTensor extends SegmentFloatTensor {
    ...
    public float getFloat(long index) {
        // through GLOBAL_SEGMENT (readFloat) so the access inlines on native image (see
        // FloatTensor)
    ...
    public FloatVector getFloatVector(VectorSpecies<Float> species, long index) {
        ...
        return FloatVector.fromMemorySegment(
                species, vseg, vbase + (long) index * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
    }
    ...
    public FloatTensor clampInPlace(long thisOffset, int size, float lo, float hi) {
        if (USE_VECTOR_API) {
            FloatVector vlo = FloatVector.broadcast(F_SPECIES, lo),
                    vhi = FloatVector.broadcast(F_SPECIES, hi);
            int upperBound = F_SPECIES.loopBound(size);
            int i = 0;
            for (; i < upperBound; i += F_SPECIES.length()) {
    ...
    public FloatTensor saxpyInPlace(
        ...
        if (that instanceof F16FloatTensor f16 && USE_VECTOR_API) {
            ...
            for (; i < upperBound; i += F_SPECIES.length()) {
                ...
                FloatVector thatVector =
                        bits32.and(0x8000)
                                ...
                                .reinterpretAsFloats();
                FloatVector thisVector =
                        FloatVector.fromMemorySegment(
                                F_SPECIES,
                                vseg,
                                vbase + (long) (thisOffset + i) * Float.BYTES,
                                ByteOrder.LITTLE_ENDIAN);
                va.fma(thatVector, thisVector)
                        .intoMemorySegment(
                                vseg,
                                vbase + (long) (thisOffset + i) * Float.BYTES,
                                ByteOrder.LITTLE_ENDIAN);
            ...
            for (; i < size; i++) {
                setFloat(
                        thisOffset + i,
                        a * f16.getFloat(thatOffset + i) + getFloat(thisOffset + i));
            }
            return this;
        }
    ...
    public void copyTo(long thisOffset, FloatTensor that, long thatOffset, int size) {
        if (that instanceof F32FloatTensor f32) {
            MemorySegment.copy(
                    memorySegment,
                    (long) thisOffset * Float.BYTES,
                    f32.memorySegment,
                    (long) thatOffset * Float.BYTES,
    ...
    FloatTensor siluMultiplyInPlace(long thisOffset, FloatTensor that, long thatOffset, int size) {
        if (that instanceof F32FloatTensor f32 && USE_VECTOR_API) {
            ...
            // tanh:
            // only mul/add/div (no exp, no integer bit-ops), so it vectorizes on GraalVM/jvmci too
            // (where the
            // lanewise EXP intrinsic is absent and the scalar fallback was ~24% of prefill). ~1e-5
            // abs error.
            int upperBound = F_SPECIES.loopBound(size);
            int i = 0;
            for (; i < upperBound; i += F_SPECIES.length()) {
                ...
                var u =
                        FloatVector.fromMemorySegment(
                                ...
                                f32.vbase + (long) (thatOffset + i) * Float.BYTES,
                                ByteOrder.LITTLE_ENDIAN);
                // silu(g)*u with siluVec/tanhVec INLINED by hand: the FloatVector temporaries never
                // cross a
                // method boundary, so they scalar-replace into SIMD registers on any JIT — not only
                // those that
                // inline the helpers (a weaker inliner otherwise boxes the escaping return value).
                // Identical
                // math to siluVec(g).mul(u); the helpers stay for GELU / the scalar tail. Keep in
                // sync with tanhVec.
                FloatVector y =
                        g.mul(0.5f).max(-TANH_CUTOFF).min(TANH_CUTOFF); // tanh input = g/2, clamped
            ...
            for (; i < size; i++) {
                ...
                setFloat(
                        thisOffset + i,
                        (float) (g / (1.0 + Math.exp(-g)) * f32.getFloat(thatOffset + i)));
            }
            return this;
        }
    ...
    /**
     * Vectorized SiLU g*(0.5+0.5*tanh(g/2)). tanh(y) via njuffa's minimax rational approximation
     * (the "cutoff" variant): y is clamped to +/-CUTOFF, where tanh has saturated to ~1 (so no
     * output clamp is needed), then tanh(y) = y + y*num(y^2)/den(y^2). Only mul/add/div/fma ->
     * vectorizes on GraalVM/jvmci (unlike a lanewise EXP, which Graal does not intrinsify). Source:
     * njuffa, StackOverflow "fast tanhf". Precision: |error| <= ~1.9e-5 for tanh over all float32;
     * <= 1.1e-4 abs / 4.5e-3 rel for this SiLU over g in [-40,40] (worst near g~11.5; near-exact
     * for |g|<2). Well under Q8_0's ~3.9e-3 quantization noise.
     */
    @AlwaysInline(
    ...
    // njuffa minimax-rational tanh coefficients (the "cutoff" variant). One source of truth, shared
    // by
    ...
    /**
     * Vectorized tanh(x) via njuffa's minimax rational (the "cutoff" variant): x clamped to
     * +/-CUTOFF (tanh saturated to ~1 there, so no output clamp), tanh = x + x*num(x^2)/den(x^2).
     * Only mul/add/div/fma, so it runs fast on GraalVM/jvmci (which does NOT intrinsify lanewise
     * TANH/EXP). Source: njuffa, StackOverflow "fast tanhf". |error| <= ~1.9e-5 over all float32.
     * Shared by SiLU and Gemma's GELU.
     */
    @AlwaysInline(
            "hot Vector API helper: escaping FloatVector boxes per call (see hotspot_compiler)")
    ...
    FloatTensor reluSqrInPlace(long thisOffset, int size) {
        if (USE_VECTOR_API) {
            // x = max(0,x)^2, fully vectorized (max + mul only, no scalar setFloat). Nemotron
            // FFN/expert act.
            int upperBound = F_SPECIES.loopBound(size);
            int i = 0;
            for (; i < upperBound; i += F_SPECIES.length()) {
    ...
    private static float vectorDot(
            F32FloatTensor thiz, long thisOffset, F32FloatTensor that, long thatOffset, int size) {
        ...
        for (int i = 0; i < upperBound; i += F_SPECIES.length()) {
            var a =
                    FloatVector.fromMemorySegment(
                            F_SPECIES,
                            thiz.vseg,
                            thiz.vbase + (long) (thisOffset + i) * Float.BYTES,
                            ByteOrder.LITTLE_ENDIAN);
            var b =
                    FloatVector.fromMemorySegment(
                            F_SPECIES,
