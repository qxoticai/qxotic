...
public final class RuntimeFlags {
    ...
    // and
    // reused across QT query rows, ~1.7x faster than the per-position rolling path at
    // multi-thousand-
    // token depth (and it flattens the prefill-vs-context curve). On by default; disable with
    // -Djinfer.flashAttention=false. NOTE: needs the f16-decode inline hints (see Makefile
    // INLINE_HINTS)
    // — without them the qkTile/pvTile vectors box and flash falls behind rolling.
    static final boolean FLASH_ATTENTION =
            !"false".equals(System.getProperty("jinfer.flashAttention"));
    ...
    // (Llama.rollingAttnCached64) decodes 8 f16 KV vectors per key in ONE method; Graal keeps that
    // in
    // zmm registers, but HotSpot C2's vector box-elimination budget is smaller and it BOXES the f16
    // decode (~78x more boxed-vector ops measured), collapsing decode -50% at long context. When
    // this
    ...
    static final int DECODE_BLOCK_PARALLEL_MIN_RANGE =
            Integer.getInteger("jinfer.decodeBlockParallelMinRange", 1024);
