 ...
 * are SPARSE — multi-token blocks always carry one, single-token decode runs one every {@code
 * jinfer.checkpointStride} (64) positions — so a decode block costs its rows only and a resume
 * lands at the deepest checkpointed block, the caller re-ingesting the short tail. The cache is a
 * pure optimization — every miss degrades to recompute, never to a wrong answer.
 *
 * <p>{@link #resume} matches and restores once per request and returns a {@link Cursor}; the cursor
 * commits each subsequently ingested span in O(span) — one digest of the span against the tip key,
 * no re-walk — which is what makes single-token commits during decode natural. Large blocks and
 * single-token blocks are the same mechanism at different spans.
 *
...
public final class PromptCache<S extends RuntimeState> {
     ...
     * Detaches — commits become no-ops — if the budget rejects a block or the tip is evicted; reads
    ...
    public final class Cursor {
        ...
        /**
         * Commit the span {@code spanFp[off..off+len)} just ingested, as a group end
         * (checkpointed).
         */
        ...
        /**
         * Commit the span; {@code groupEnd} marks the last chunk of the caller's logical ingest (a
         * turn, a whole prompt) - the resume points that matter.
         */
        public void commit(long[] spanFp, int off, int len, S state, boolean groupEnd) {
            ...
            boolean checkpoint =
                    ckptBytes == 0
                            || (len > 1
                                    ? (groupEnd || to - tip.ckptTo >= multiTokenGap)
     ...
     */
    public PromptCache(
            StateCodec<S> codec, CacheStore store, long budgetBytes, byte[] modelSeed, int stride) {
     ...
     * seed change and auto-invalidates persisted blocks (miss → recompute, no migration). One cache
     * instance/file per model is the deployment shape; see {@link #modelSeed}.
     */
     ...
     * and for naming a persisted cache file: file length + SHA-256 of the first and last MiB of the
     ...
     * into {@code state}, and returns the session's write cursor sitting at the resume point — the
     * deepest CHECKPOINTED block of the match (== the match itself for models without recurrent
     * state); the caller re-ingests everything past {@link Cursor#position()}. 0 on a cold start.
     * Rows of matched blocks beyond the resume point are not restored — the re-ingest recomputes
     * them.
     */
        ...
        if (tip != sentinel) {
            ...
            if (codec.checkpointBytes() > 0) {
                codec.restoreCheckpoint(
                        state, tip.to, tip.mem.asSlice(codec.rowBytes(tip.to - tip.from)));
            }
            hits++;
     ...
     * remaining chain contiguous. {@code keep} (the committing cursor's tip) is never evicted: a
    ...
    /**
     * Serializes every block into {@code out} — a read-only, shareable artifact holding any number
     * of prompts, shared prefixes stored once (content addressing). Open it with {@link #open}.
     */
    public void freeze(java.nio.file.Path out) throws java.io.IOException {
        ...
        try (java.nio.channels.FileChannel ch =
            ...
            ByteBuffer h =
                    map.asSlice(0, HEADER_BYTES).asByteBuffer().order(ByteOrder.LITTLE_ENDIAN);
            h.putInt(MAGIC_FROZEN)
    ...
    /**
     * Maps a frozen cache file lazily (header + index read; KV bytes untouched until a restore) and
     * validates it belongs to the model identified by {@code modelSeed} — throws a descriptive
     * error when it does not. The returned cache is FROZEN: {@link #resume} serves every stored
     * prompt; cursor commits are silent no-ops, so live conversation turns are not cached on it (a
     * RAM overlay tier is future work). The mapping lives for the process (global arena).
     */
    public static <S extends RuntimeState> PromptCache<S> open(
            java.nio.file.Path file, StateCodec<S> codec, byte[] modelSeed)
        ...
        if (!java.util.Arrays.equals(stored, seed32(modelSeed))) {
            throw new IllegalStateException(
                    "frozen cache "
                            + file
                            + " (model seed "
                            + java.util.HexFormat.of().formatHex(stored, 0, 8)
        ...
        CacheStore readOnly =
                new CacheStore() {
                    ...
                    public MemorySegment allocate(long bytes) {
                        throw new UnsupportedOperationException("frozen cache");
                    }
        ...
        PromptCache<S> cache =
                new PromptCache<>(codec, readOnly, 0, modelSeed, true, 1); // frozen: never commits
        ByteBuffer idx =
        ...
        for (int i = 0; i < count; i++) {
            ...
            PromptCache<S>.Block parent =
                    parentKey.equals(cache.sentinel.key)
                            ? cache.sentinel
                            : cache.blocks.get(parentKey);
            if (parent == null) {
                throw new IllegalStateException(
                        file + " index is corrupt: block " + i + " has an unknown parent");
            }
            boolean checkpointed = codec.checkpointBytes() == 0 || len > codec.rowBytes(to - from);
            PromptCache<S>.Block b =
                    cache
                    .new Block(
                            key,
                            parent,
                            from,
                            to,
                            checkpointed ? to : parent.ckptTo,
                            map.asSlice(offset, len));
