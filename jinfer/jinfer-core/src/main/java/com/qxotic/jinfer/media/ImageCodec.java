...
//   default: ffmpeg under GraalVM native-image (where javax.imageio is impractical), imageio on a normal JVM.
...
public final class ImageCodec {
    ...
    /** Decode an image file into a raw {@link Media.Image} (RGB, [0,1], HWC) via the selected backend. */
    public static Media.Image load(Path path) throws IOException {
        return decoder().load(path);
    }

    /** Decode encoded image bytes into a raw {@link Media.Image} (RGB, [0,1], HWC) via the selected backend. */
     ...
     *  Class.forName site) keeps native-image from constant-folding it, so the ImageIO backend and
     *  java.desktop stay out of native images. Falls back to ffmpeg if the backend can't load. */
    private static ImageDecoder loadReflectively(String className) {
        try {
            ...
            System.err.println("image decoder '" + className + "' unavailable (" + e + "); falling back to ffmpeg");
