...
// logits(state) + Sampler + Batch.step. Owns the whole generation contract - Params/Listener/StopSpec/
// GenerationResult plus the tokenizer-based stop/stream/reasoning machinery (formerly shared with the
...
public final class Generator {
     ...
     *  {@code maxTokens} is the completion budget in generated tokens; negative means "as much as the
     *  context allows". {@code timeoutNanos} is a wall-clock decode deadline as a DURATION (converted
     *  to an absolute deadline at {@link #generate} entry); 0 means no deadline. {@code inlineThink}
     *  keeps think spans inline in the text instead of routing them to the reasoning channel. */
    ...
    /** Streaming callbacks; fields are nullable. A non-null {@code onContent} switches to incremental
     *  text decoding: deltas are UTF-8 safe and never contain a configured text stop. {@code onReasoning}
     *  (only honored with {@code onContent}) receives think-span text. {@code onToken} sees every sampled
     *  token - specials and the stop token included - before any text for it is emitted. */
     ...
     * {@code text} has think spans routed out (unless inlineThink) and text stops applied;
     * {@code reasoning} carries the think-span text for non-streaming passes. {@code toolCalls} is
     ...
     *  {@code <think>}/{@code </think>} markers when reasoning is disabled. */
    ...
    public static <S extends RuntimeState> GenerationResult generate(
        ...
        require(promptPositions <= contextLength,
        ...
        String reasoning = listener.onContent() == null && !params.inlineThink()
                ? reasoningText(tokenizer, responseTokens) : null;
    ...
    /**
     * Caps the think span: once {@code budget} tokens have been sampled inside {@code <think>}, the close
     * marker is forced so the remaining completion budget always goes to content (thinking models otherwise
    ...
    /** Routes each generated token to the content, reasoning, or tool-call channel. Think markers flip the
     ...
     *  string; once a stop string appears the text before it is emitted and the consumer goes silent. */
    ...
    static final class Utf8TokenDecoder {
        ...
        /** Emits any accumulated incomplete bytes as-is, resetting the pending buffer. Called at channel
         *  transitions (e.g. think->content) to prevent stale bytes from polluting the next channel. */
        void flushPending() {
            if (pending.size() > 0) {
