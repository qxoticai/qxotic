 ...
 * close as best-effort on non-closeable arenas ({@link BaseState}, {@code CacheStore}), so the
 * degrade is a latency-of-free change, not a leak. When GraalVM lifts the restriction, the build
 * re-enables real shared arenas via {@code -Djinfer.sharedArenas=true} (see the native profile) -
