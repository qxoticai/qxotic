...
public final class BlockTree<S extends RuntimeState> {
     ...
     */
    public BlockTree(StateCodec<S> codec, CacheStore store, long budgetBytes, byte[] modelSeed) {
        this.codec = codec;
        this.store = store;
    ...
    /**
     * A fast, stable model identity for {@link #BlockTree(StateCodec, CacheStore, long, byte[])}
