// ffmpeg-based audio decoder: decodes any container/codec (mp3, wav, flac, ogg, m4a, ...) to raw PCM,
...
public final class FfmpegAudioDecoder implements AudioDecoder {
    ...
    private static byte[] runFfmpeg(List<String> cmd, byte[] stdin) throws IOException {
        ...
        if (code != 0) {
            throw new IOException("ffmpeg exited " + code + ": " + err.toString(StandardCharsets.UTF_8).strip());
        }
        return out;
    }
}
