...
/** Sparse-checkpoint gate: multi-token blocks carry checkpoints, single-token decode runs one
 *  every stride; a resume lands at the deepest checkpoint and re-ingesting the tail reproduces
 *  the never-cached state exactly. The fake's recurrent state is order-dependent (like a conv
 *  history), so any checkpoint-currency mistake changes the final value. */
public final class SparseCheckpointTest {
    ...
    static final class FakeState implements RuntimeState {
        ...
        @Override public int contextCapacity() { return rows.length; }
        @Override public int batchCapacity() { return rows.length; }
    ...
    static final class FakeCodec implements StateCodec<FakeState> {
        ...
        @Override public void saveRows(FakeState s, int from, int to, MemorySegment dst) {
            for (int p = from; p < to; p++) dst.setAtIndex(ValueLayout.JAVA_LONG, p - from, s.rows[p]);
        }
        @Override public void restoreRows(FakeState s, int from, int to, MemorySegment src) {
            for (int p = from; p < to; p++) s.rows[p] = src.getAtIndex(ValueLayout.JAVA_LONG, p - from);
        }
        @Override public void saveCheckpoint(FakeState s, int to, MemorySegment dst) {
            dst.set(ValueLayout.JAVA_LONG, 0, s.recurrent);
        }
        @Override public void restoreCheckpoint(FakeState s, int to, MemorySegment src) {
    ...
    public static void main(String[] args) {
        ...
        check(cache.stats().contains("checkpoints=2"), "sparse: 2 checkpoints across 31 blocks (" + cache.stats() + ")");
        long expectBytes = (10 * 8 + 8) + (29 * 8) + (8 + 8);   // turn+ckpt, 29 bare singles, 1 ckpt single
        check(store.usedBytes() == expectBytes, "single-token blocks cost rows only (" + store.usedBytes() + " vs " + expectBytes + ")");
        ...
        check(cur2.position() == 26 && resumed.position() == 26, "resume lands at the last checkpoint (26, got " + cur2.position() + ")");
        ...
        check(Arrays.equals(resumed.rows, reference.rows) && resumed.recurrent == reference.recurrent,
        ...
        check(cache.resume(mutated, 40, diverged).position() == 26, "divergent tail resumes at the checkpoint");
        ...
        check(cache.resume(fp, 40, again).position() == 40, "re-committed tail becomes the new resume point");
        check(again.recurrent == reference.recurrent && Arrays.equals(again.rows, reference.rows),
    ...
    static void check(boolean ok, String what) {
        ...
        else { failures++; System.out.println("FAIL: " + what); }
    }
}
