 ...
 * (sized &gt; L3 so it streams from DRAM) run through {@code vectorGemv512} repeatedly. Compare the
 * GB/s to decode's effective bandwidth (~101 GB/s) and llama.cpp's (~121) to tell whether the gap
 * is the kernel itself or the per-token overhead of ~360 small gemvs + barriers.
 *
...
public final class GemvBench {
    ...
    private static void main(String[] args) {
        ...
        for (int it = 0; it < 60; it++)
            MatMul.instance().mm(w, 0, dim1, x, 0, dim1, out, 0, dim0, dim0, 1, dim1);
        ...
        System.out.printf(
                "gemv %dx%d  %.1f GB/s  (%.3f ms/gemv, %d iters, weights %.0f MB)%n",
