...
public final class SessionPoolTest {
    ...
    static final class FakeState implements RuntimeState {
        ...
        @Override public int contextCapacity() { return 64; }
    ...
    public static void main(String[] args) {
        ...
        CachedSession<FakeState> a = CachedSession.resume(model, cache, model.newState(0, 0), new long[0]);
        ...
        check(pool.acquire(diverged, diverged.length) == null, "mid-stream divergence is never reused");

        // Identical conversation (no delta): not a STRICT prefix -> no reuse (logits would be stale).
        ...
        CachedSession<FakeState> b = CachedSession.resume(model, cache, model.newState(0, 0), new long[0]);
        ...
        check(pool.acquire(tooLong, tooLong.length) == null, "request past contextCapacity is not pooled onto the state");

        // LRU eviction: capacity 2; releasing C evicts the least-recent (B).
        CachedSession<FakeState> c = CachedSession.resume(model, cache, model.newState(0, 0), new long[0]);
        ...
        check(pool.acquire(new long[]{1, 2, 9}, 3) == null, "evicted least-recent session (B) is gone");
        check(pool.acquire(followUp, followUp.length) == a, "most-recent sessions survive eviction");
