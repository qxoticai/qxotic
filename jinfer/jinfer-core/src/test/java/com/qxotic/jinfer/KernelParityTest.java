 ...
 * are synthetic blocks (random payload bytes, scale fields rewritten to finite values), so no model
 * file is needed and a run takes seconds. Exits non-zero on any mismatch.
 *
 * <p>Run via {@code make test}. The vector kernels under test are selected by the same dispatch as
 * inference (USE_VECTOR_API, species width, native lib if {@code -Dllama.nativeGemmLib} or {@code
 * -Dllama.staticGemm} is set), so the harness covers whatever configuration it runs under.
 */
public final class KernelParityTest {
    ...
    private static void putScaleF16(MemorySegment seg, long offset, Random rng) {
        seg.set(
                ...
                offset,
                (short) (0x3800 | (rng.nextInt() & 0x3FF)));
    }
            ...
            switch (type) {
                ...
                case MXFP4 ->
                        seg.set(
                                ...
                                off,
                                (byte) (120 + rng.nextInt(10))); // e8m0 ~ 2^-4..2^1
                case NVFP4 -> {
                    for (int s = 0; s < 4; s++) // 4 UE4M3 scales ~0.5..0.9
    ...
    private static void check(
            String what, double actual, double expected, double sumAbs, double relTol) {
        checks++;
        double tol = relTol * sumAbs + 1e-3;
        if (!(Math.abs(actual - expected) <= tol)) {
            failures++;
            System.err.printf(
                    "FAIL %s: got %.8g expected %.8g (tol %.3g)%n", what, actual, expected, tol);
        }
    }
    ...
    static void testDot(GGMLType type) {
        ...
        for (int[] c : cases) {
            ...
            check(
                    type + ".dot(off=" + off + ",size=" + size + ")",
                    w.dot(off, x, 0, size),
                    ref[0],
                    ref[1]);
            // and through scalarDot (the non-vector fallback) for the same span
            check(
                    type + ".scalarDot(off=" + off + ",size=" + size + ")",
                    FloatTensor.scalarDot(w, off, x, 0, size),
                    ref[0],
                    ref[1]);
        }
    }
        ...
        for (int seqLen : new int[] {7, 13, 16}) {
            ...
            for (int s = 0; s < seqLen; s++) {
                for (int row = 0; row < dim0; row += 13) {
                    ...
                    check(
                            type + ".gemm[seq=" + seqLen + "," + s + "," + row + "]",
                            out.getFloat(s * dim0 + row),
                            ref[0],
                            ref[1],
                            5e-3);
                }
