 ...
 * — which only exists at the state's current position, exactly why blocks match completely or not
 * at all. MOE layers route per token and carry no cross-token state. The S matrix is a heap array
 * copied via a heap segment view.
 *
 * <p>Size note: the fixed checkpoint dominates — 23 SSM layers × (2 MiB S + 72 KiB ring) is ~47.6
 * MiB per block regardless of span, vs ~6 KiB/position of attention K/V. Turn-aligned blocks
 * amortize it; single-token decode blocks are expensive, so harnesses keep decode budgets small and
 * cache budgets large (checkpoint compression is a noted follow-up).
 */
public final class NemotronHStateCodec extends AbstractStateCodec<NemotronH.State> {
    ...
    public NemotronHStateCodec(NemotronH.Configuration config) {
        super(
                ...
                s -> s.valueCache,
                l ->
                        config.layerTypes()[l] == NemotronH.LayerType.SSM
    ...
    private static int convFloats(NemotronH.Configuration c) {
        return (c.ssmConvKernel() - 1) * c.ssmConvChannels();
    }
