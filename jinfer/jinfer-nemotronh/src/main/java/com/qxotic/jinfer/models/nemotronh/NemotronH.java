// Nemotron-H ("nemotron_h" / "nemotron_h_moe", Nemotron Cascade 2) against the com.qxotic.jinfer
// model
// API: a faithful port of the production jinfer Nemotron forward. A hybrid where each block is
// exactly
// ONE of {Mamba2-SSM mixer, full-attention, MoE-FFN}, behind a single pre-norm. Architecture
// specifics:
...
// all FFN/expert activations are squared-ReLU (no gate projection); a shared expert is always
// added.
//
// Batched prefill + single-token decode: the per-token projections (SSM in/conv/out, attention
// Q/K/V/O,
// MoE router/experts) batch into GEMMs over the chunk, while only the Mamba2 recurrence stays
// sequential
// (one parallelFor over heads, rows sequential inside, conv-ring + SSM state carried in State).
// Decode
...
public final class NemotronH
    ...
    public void forward(State s, Batch batch) {
        ...
        switch (batch.input()) {
            case Batch.Input.Tokens t -> {
                // decode (n==1): run on the physical-core spin pool so the SSM scan/conv
                // parallelize over
                // the decode cores; prefill (n>1) stays on the SMT-wide common pool.
                if (n == 1)
                    Parallel.onDecodePool(
                            () -> {
                                forward(s, t.ids(), 0, from, n);
                                return null;
                            });
            ...
            case Batch.Input.Sequences seq ->
                    throw new UnsupportedOperationException(
                            "Nemotron-H is generative: packed sequences (batched embedding) not"
                                    + " supported");
            case Batch.Input.Embeddings e ->
                    throw new UnsupportedOperationException(
                            "Nemotron-H is text-only: embedding input is not supported");
        }
    ...
    /**
     * Batched forward over the chunk: embed all rows, then per block a batched pre-norm feeds
     * whichever mixer this layer is. seqLen==1 takes the single-token cores (decode); seqLen>1 the
     * batched cores (prefill) — projections become GEMMs, only the Mamba2 recurrence stays
     * sequential (per head).
     */
    void forward(State s, int[] ids, int tokenOffset, int startPos, int seqLen) {
        ...
        for (int l = 0; l < c.numberOfLayers; l++) {
            ...
            switch (c.layerTypes[l]) {
                case SSM -> {
                    if (seqLen == 1) ssmForward(s, l);
                    else ssmForwardBatch(s, l, seqLen);
                }
                case ATTENTION -> attention(s, l, startPos, seqLen);
                case MOE -> {
                    if (seqLen == 1) moeForward(s, l);
                    else moeForwardBatch(s, l, seqLen);
                }
            }
    ...
    private void ssmForward(State state, int l) {
        ...
        int dInner = c.ssmInnerSize,
                dState = c.ssmStateSize,
                nHead = c.ssmTimeStepRank,
                nGroup = c.ssmGroupCount,
                dConv = c.ssmConvKernel;
        ...
        for (int h = 0; h < nHead; h++)
            dt[h] = softplus(state.ssmInProj.getFloat(dtOff + h) + w.dtBias().getFloat(h));
        ...
        // 3. selective scan: parallel over heads (independent state); h_new = h*exp(dt*A) +
        // B*(x*dt); y = C.h + D*x; gate SiLU(z)
        ...
        Parallel.parallelFor(
                ...
                h -> {
                    ...
                    for (int ii = 0; ii < headDim; ii++) {
                        ...
                        int stBase = idx * dState,
                                bOff = dInner + g * dState,
                                cOff = dInner + qSize + g * dState;
                        float sum = 0f;
    ...
    /**
     * Full causal GQA attention — NO RoPE / q-k norm / biases: Q/K/V projections, K/V appended to
     * the cache at `position`, sink-free flash decode, output projection written back to state.xb.
     */
    ...
    /**
     * MoE-FFN: sigmoid router with additive selection bias (top-k by biased score, combine weight
     * is the UNBIASED sigmoid, optionally normalized then scaled); squared-ReLU experts (no gate)
     * plus an always-on shared expert. Reads state.xb (normed), writes the result back to state.xb.
     */
    private void moeForward(State state, int l) {
        Configuration c = configuration;
        MoeFfnWeights w = weights.layers[l].moe();
        int dim = c.embeddingLength,
                expertFF = c.expertFeedForwardLength,
                numExperts = c.expertCount;
        int topK = Math.min(c.expertUsedCount, numExperts);

        w.router().matmul(state.xb, state.moeRouter, numExperts, dim);
        for (int e = 0; e < numExperts; e++)
            state.moeRouter.setFloat(e, sigmoid(state.moeRouter.getFloat(e)));
        ...
        for (int k = 0; k < topK; k++) {
            ...
            float coeff = topP[k];
            for (int i = 0; i < dim; i++)
                state.moeAccum.setFloat(
                        i, state.moeAccum.getFloat(i) + state.moeExpertOut.getFloat(i) * coeff);
        }
        ...
        if (w.upShexp() != null) { // shared expert (always active), squared-ReLU, coefficient 1
            ...
            for (int i = 0; i < dim; i++)
                state.moeAccum.setFloat(
                        i, state.moeAccum.getFloat(i) + state.moeExpertOut.getFloat(i));
        }
    ...
    /**
     * Top-k experts by (sigmoid router + probsB bias), keeping the UNBIASED router value as the
     * combine weight. Non-destructive "not already taken" scan, so the router tensor is left
     * intact.
     */
    private static void selectTopK(
            FloatTensor router,
            int base,
            F32FloatTensor probsB,
            int numExperts,
            int topK,
            int[] topE,
            float[] topP) {
        for (int k = 0; k < topK; k++) {
            ...
            for (int e = 0; e < numExperts; e++) {
                float score =
                        router.getFloat(base + e) + (probsB == null ? 0f : probsB.getFloat(e));
                boolean taken = false;
                for (int j = 0; j < k; j++)
                    if (topE[j] == e) {
                        taken = true;
    ...
    // === Batched mixers (prefill): projections become GEMMs over the chunk; only the Mamba2
    // recurrence
    //     stays sequential (one parallelFor over heads, rows sequential inside). Token-exact vs
    // decode. ===

    /**
     * Batched Mamba2 SSM: in-proj GEMM, batched causal conv1d (parallel over channels, reads the
     * ring for negative positions), selective scan parallel-over-heads + sequential-rows (state
     * carries), per-group gated RMSNorm, out-proj GEMM. Mirrors {@link #ssmForward} row-for-row.
     */
    private void ssmForwardBatch(State state, int l, int seqLen) {
        ...
        int headDim = dInner / nHead,
                convCh = c.ssmConvChannels(),
                qSize = nGroup * dState,
                inProjSize = c.ssmInProjSize();
        float eps = c.rmsNormEps;
        ...
        Parallel.forRows(
                ...
                s -> {
                    int p = s * fInProj;
                    for (int i = 0; i < fDInner; i++)
                        z[s * fDInner + i] = state.ssmInProj.getFloat(p + i);
                    for (int i = 0; i < fConvCh; i++)
                        xbc[s * fConvCh + i] = state.ssmInProj.getFloat(p + fDInner + i);
                    for (int h = 0; h < fNHead; h++)
                        dt[s * fNHead + h] =
                                softplus(state.ssmInProj.getFloat(p + dtOff + h) + dtB.getFloat(h));
                });

        // 2. batched depthwise causal conv1d + SiLU; row s reads taps s-(dConv-1)+k from chunk or
        // conv ring.
        FloatTensor convState = state.ssmConvState[l];
                ...
                ch -> {
                    ...
                    for (int s = 0; s < fSeq; s++) {
                        ...
                        for (int k = 0; k < fK; k++) {
                            ...
                            float in =
                                    pos < 0
                                            ? convState.getFloat((pos + fHist) * fConvCh + ch)
                                            : xbc[pos * fConvCh + ch];
        ...
        Parallel.parallelFor(
                0,
                convCh,
                ch -> { // roll the conv ring: keep the last dConv-1 xBC rows
                ...
                h -> {
                    ...
                    for (int s = 0; s < fSeq; s++) {
                        ...
                        for (int ii = 0; ii < fHeadDim; ii++) {
                            ...
                            int stBase = idx * fDState,
                                    bOff = cBase + fDInner + g * fDState,
                                    cOff = cBase + fDInner + fQSize + g * fDState;
                            float sum = 0f;
                            ...
                            y[base + idx] =
                                    (sum + convOut[cBase + idx] * D_h) * silu(z[base + idx]);
                        }
                    }
                });
                    ...
                    for (int g = 0; g < fNGroup; g++) {
                        ...
                        for (int i = 0; i < fGroupDim; i++)
                            state.ssmTmp.setFloat(
                                    off + i, y[off + i] * inv * normW.getFloat(g * fGroupDim + i));
                    }
                });
     ...
     * GEMMs via the shared {@link Moe#dispatch}, plus the batched shared expert. Mirrors {@link
     * #moeForward}.
     */
    private void moeForwardBatch(State state, int l, int seqLen) {
        ...
        state.xb.copyTo(
                ...
                state.moeInputB,
                0,
                seqLen * dim); // snapshot (CSR scrambles rows; router/shared read it)
        w.router()
                .gemm(state.moeInputB, dim, state.moeRouterB, numExperts, seqLen, numExperts, dim);

        int[] counts = state.moeExpertCounts;
        F32FloatTensor probsB = w.expProbsB();
        int fTopK = topK, fNumExperts = numExperts;
        boolean fNorm = c.expertWeightsNorm;
        float fScale = c.expertWeightsScale;
        Parallel.forRows(
                seqLen,
                s -> { // per-row routing is independent (selectTopK is non-destructive)
                    int rb = s * fNumExperts;
                    for (int e = 0; e < fNumExperts; e++)
                        state.moeRouterB.setFloat(
                                rb + e, sigmoid(state.moeRouterB.getFloat(rb + e)));
                    ...
                    selectTopK(
                            ...
                            fTopK,
                            te,
                            tp); // top-k by sigmoid+bias; combine = unbiased
                    float weightSum = 0f;
                    if (fNorm) {
    ...
    public record SsmWeights(
            FloatTensor inProj,
            F32FloatTensor conv1d,
            F32FloatTensor conv1dBias,
            F32FloatTensor a,
            F32FloatTensor d,
            F32FloatTensor dtBias,
            F32FloatTensor norm,
            FloatTensor outProj) {}

    public record MoeFfnWeights(
            FloatTensor router,
            F32FloatTensor expProbsB,
            FloatTensor upExps,
            FloatTensor downExps,
            FloatTensor upShexp,
            FloatTensor downShexp) {}
    ...
    public static final class State extends com.qxotic.jinfer.BaseState {
        ...
        final FloatTensor moeRouter,
                moeHidden,
                moeExpertOut,
                moeAccum,
                moeSharedHidden; // single-token decode scratch
    ...
    public static NemotronH loadModel(Path ggufPath, Arena arena) throws IOException {
    ...
    public static NemotronH loadModel(FileChannel fileChannel, GGUF gguf, Arena arena)
            throws IOException {
    ...
    public static NemotronH loadModel(
        ...
        int headSize =
                gguf.getValueOrDefault(
                        ...
                        embeddingLength / Math.max(1, numberOfHeads));
        float rmsNormEps =
                gguf.getValueOrDefault(
        ...
        float expertWeightsScale =
                gguf.getValueOrDefault(float.class, arch + ".expert_weights_scale", 1.0f);
                ...
                case SSM ->
                        ssm =
                                new SsmWeights(
                                        ...
                                        ModelLoader.loadQuantized(
                                                tensors.get(p + "ssm_out.weight")));
                case MOE ->
                        moe =
                                new MoeFfnWeights(
                                        ModelLoader.loadQuantized(
                                                tensors.get(p + "ffn_gate_inp.weight")),
                                        f32OrNull(tensors, p + "exp_probs_b.bias"),
                                        ModelLoader.loadQuantized(
                                                tensors.get(p + "ffn_up_exps.weight")),
                                        ModelLoader.loadQuantized(
                                                tensors.get(p + "ffn_down_exps.weight")),
