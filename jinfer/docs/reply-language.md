...
Status: DESIGN validated against all nine tool-capable families below; brick 1 (token-identity terminals, `Grammar.Term`, `GrammarTokenTermTest`) and brick 2 (the region-tagged walk: `ReplyLanguage` with `Selection`/`Walk`, `ReplyLanguageTest`) LANDED and adversarially hardened; SEVEN of nine families MIGRATED to the walk (gpt-oss, SmolLM3, Granite, Qwen3.5, NemotronH, LFM2, Mistral - the last four parser-only, keeping their E2E-green legacy pins for forcing; gpt-oss/SmolLM3/Granite/Mistral also force through schema-bound selections, and Mistral gained forcing it NEVER had).
