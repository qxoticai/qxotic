...
public final class ScoringBench {
    ...
    static double[] httpLeg(String base, String query, List<TextSegment> docs) throws Exception {
        ...
        body.append(JsonCodec.stringify(query)).append(",\"documents\":[");
        for (int i = 0; i < docs.size(); i++) {
            if (i > 0) body.append(',');
            body.append(JsonCodec.stringify(docs.get(i).text()));
        }
        body.append("]}");
        HttpRequest req =
        ...
        Arrays.sort(ms);
        Map<String, Object> parsed = (Map<String, Object>) JsonCodec.parse(last);
