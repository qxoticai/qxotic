...
class GrammarCostProbe {
    ...
    void probe() throws Exception {
        Path model = ModelFixture.LFM25_8B_Q8.require();
        GGUF g;
        ...
        F32FloatTensor logits = F32FloatTensor.allocate(Arena.ofAuto(), vocab);
        ...
        for (int i = 0; i < doc.length(); i++) {
            for (int t = 0; t < vocab; t++) logits.setFloat(t, 0f);
        ...
        int open = com.qxotic.jinfer.llm.SpecialTokens.require(tok, "<think>");
        walk.feed(open);
        t0 = System.nanoTime();
        int reps = 2000;
