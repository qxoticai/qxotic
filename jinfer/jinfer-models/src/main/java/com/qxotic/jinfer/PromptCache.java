 ...
 *   regular traffic keeps stride pairs ({@code jinfer.promptCacheStride}) plus a dense tail
 *   ({@code jinfer.promptCacheDenseTail}). Unlike checkpoints, bx rows split cleanly.</li>
 * </ul>
...
final class PromptCache {
     ...
     * keep the hist trailing rows before each multiple of {@code jinfer.promptCacheStride}
     * plus the last {@code jinfer.promptCacheDenseTail} positions of the stream. Null when
