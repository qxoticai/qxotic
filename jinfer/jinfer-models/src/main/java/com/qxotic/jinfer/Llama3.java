...
final class Llama3 implements ModelLegacy {
    ...
    static final class State implements InferenceState {
        final int capacity;
        final FloatTensor x, xb, k, v, logits, hb, hb2;
        ...
        State(Configuration config) {
            ...
            // F16 KV cache (half the footprint of F32). Decode reads the cache through the vectorized
            // q.dot / FlashAttention.accumulate path where the f16->f32 decode is nearly free, so the
            // halved memory traffic dominates as context grows: measured neutral at depth 0 and +4%/+8%
            // at 4k/16k on the 1B (and more on larger models), with no short-context regression.
