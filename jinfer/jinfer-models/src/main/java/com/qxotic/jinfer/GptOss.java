...
final class GptOss implements Model {
    ...
    private void attention(State state, int l, int startPos, int seqLen) {
        ...
        if (seqLen == 1) {
            ...
            FlashAttention.flashDecode((F32FloatTensor) state.q, (F32FloatTensor) state.xbK,
                    keyCache, valueCache, bK, bV, heads, position, attStart, headSize, kvDim, kvMul,
                    attScale, isSWA ? config.slidingWindow - 1 : 0, w.attnSinks[l], state.decodeScratch);
        } else {
    ...
    static final class State implements InferenceState {
        ...
        final int capacity;
        final FloatTensor x, xb, xb2, xbK, q, logits;
        ...
        State(Configuration config) {
            ...
            for (int l = 0; l < config.numberOfLayers; l++) {
                ...
                keyCache[l] = F16FloatTensor.allocate(positions * kvDim);
                valueCache[l] = F16FloatTensor.allocate(positions * kvDim);
