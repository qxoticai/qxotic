...
//   java ... com.qxotic.llm.Lfm2ChatRun [model.gguf]
...
public final class Lfm2ChatRun {
    ...
    public static void main(String[] args) throws Exception {
        ...
        batches.addAll(template.encodeTurn(Message.system("You are a concise assistant. Answer briefly.")));
        ...
        String reply2 = generate(model, s, template,
                template.encodeTurn(Message.user("Now: what is 17 * 23? Reply with just the number.")),
                tk, stops, 200);
        System.out.println("=== arithmetic ===\n" + reply2 + "\n");

        // Turn 3: reference the earlier conversation to prove the KV history is live.
        String reply3 = generate(model, s, template,
                template.encodeTurn(Message.user("What was the topic of the haiku you wrote earlier? One word.")),
                tk, stops, 200);
