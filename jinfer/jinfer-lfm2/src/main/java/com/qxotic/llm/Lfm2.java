// LFM2.5 (Liquid Foundation Model 2.5) against the com.qxotic.llm model API: a port of the production
// jinfer LFM2.5 forward (the hybrid in Llama.java). Each layer is EITHER GQA attention (kv-heads > 0)
// OR a gated short-convolution mixer (kv-heads == 0); the FFN is EITHER dense SwiGLU OR top-k MoE.
// Text-only (no media encoders, no MTP heads) so this implements only LanguageModel. Mirrors the Gemma4
// port's API decomposition (embed / layer / attention / feedForward / commitKv); the deltas vs Gemma4:
// no embedding scale, SiLU-GLU (not GeLU), attention scale = 1/sqrt(headSize), no V-norm, no SWA / no
// shared-KV / no per-layer-embeddings, and a rolling shortConvState alongside the KV cache (forked too).
...
public final class Lfm2 implements LanguageModel<Lfm2.Configuration, Lfm2.Weights, Lfm2.State> {
    ...
    public FloatTensor logits(State s, int output) {
        ...
        return Parallel.onDecodePool(() -> {
            rmsnorm(s.xb, 0, s.residual, (long) row * dim, weights.finalNorm, dim, configuration.rmsNormEps);
    ...
    private void shortConvMixer(State state, int l, int seqLen) {
        ...
        F32FloatTensor preNorm = weights.layers[l].attnNorm();   // conv layers use attn_norm as the mixer pre-norm
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb, (long) s * dim, state.residual, (long) s * dim, preNorm, dim, eps));
    ...
    /**
     * Causal short-convolution as a dConv-tap FIR over bx = B∘x rows (scalar; ported from the production
     * {@code Llama.shortConvScan}). For each channel: {@code out[s] = C_gate[s] * (Σ_{k<hist} state[k]·kernel[k]
     * + bx[s]·kernel[hist])}, where {@code state} holds the previous {@code hist=dConv-1} bx values; bx is
     * materialized in place over the B block of shortConvTmp and the newest bx rolls into shortConvState.
     */
    private void shortConvScan(State state, int l, int seqLen) {
        int dim = configuration.embeddingLength;
        int dConv = configuration.shortConvLCache, hist = dConv - 1;
        F32FloatTensor kernel = weights.layers[l].shortConv().kernel();   // per channel: dConv taps at c*dConv + k
        ...
        for (int s = 0; s < seqLen; s++) {
            ...
            for (int c = 0; c < dim; c++) {
                ...
                int kBase = c * dConv;
                float sum = 0f;
                for (int k = 0; k < hist; k++) sum += convState.getFloat((long) k * dim + c) * kernel.getFloat(kBase + k);
                sum += bx * kernel.getFloat(kBase + dConv - 1);
                out.setFloat(outOff + c, cg * sum);
                for (int k = 0; k < hist - 1; k++) convState.setFloat((long) k * dim + c, convState.getFloat((long) (k + 1) * dim + c));
     ...
     *  {@code scale = 1/sqrt(headSize)}, output projection, optional post-norm, added to the residual. */
    private void attention(State state, int l, int startPos, int seqLen) {
        ...
        headNormRope(state.query, queryDim, config.numberOfHeads, headSize, halfHead, attn.qNorm(), startPos, seqLen, real, imag);
        ...
        headNormRope(bK, kvDim, nKvHeads, headSize, halfHead, attn.kNorm(), startPos, seqLen, real, imag);
        ...
        if (seqLen > 1) {
            FlashAttention.slidingWindowPrefill(state.query, state.xbK,
                    ...
                    config.numberOfHeads, startPos, seqLen, headSize, kvDim, queryDim, kvDim, kvMul,
                    scale, 0, 0, null);
        } else {
            FlashAttention.flashDecode((F32FloatTensor) state.query, (F32FloatTensor) state.xbK,
                    state.keyCache[l], state.valueCache[l], bK, bV,
                    config.numberOfHeads, startPos, 0, headSize, kvDim, kvMul,
                    scale, 0, null, state.decodeScratch);
        }

        attn.wo().gemm(state.xbK, queryDim, state.xb2, dim, seqLen, dim, queryDim);
        F32FloatTensor postAttW = weights.layers[l].postAttnNorm();
        if (postAttW != null) Parallel.forRows(seqLen, s -> rmsnorm(state.xb2, (long) s * dim, state.xb2, (long) s * dim, postAttW, dim, eps));
    ...
    private void headNormRope(FloatTensor t, int rowStride, int nHeads, int headSize, int halfHead,
        ...
        Parallel.forRows(seqLen, s -> {
            ...
            RoPE.applyNeox(t, (long) s * rowStride, nHeads, headSize, halfHead, ropePos, real, imag);
        });
    }
    ...
    private void feedForward(State state, int l, int seqLen) {
        Configuration config = configuration;
        if (weights.layers[l].moe() != null) { moeFeedForward(state, l, seqLen); return; }
        int dim = config.embeddingLength, hiddenDim = config.feedForwardLength[l];
        float eps = config.rmsNormEps;
        DenseFfnWeights ffn = weights.layers[l].dense();
        F32FloatTensor ffnNormW = weights.layers[l].ffnNorm(), postFfwW = weights.layers[l].postFfnNorm();
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb, (long) s * dim, state.residual, (long) s * dim, ffnNormW, dim, eps));
        ...
        if (postFfwW != null) Parallel.forRows(seqLen, s -> rmsnorm(state.xb, (long) s * dim, state.xb, (long) s * dim, postFfwW, dim, eps));
    ...
    private void moeFeedForward(State state, int l, int seqLen) {
        ...
        F32FloatTensor ffnNormW = weights.layers[l].ffnNorm(), postFfnNorm = weights.layers[l].postFfnNorm();
        ...
        for (int s = 0; s < seqLen; s++) {
            ...
            if (moe.expProbsBias() != null) {
                for (int i = 0; i < nExperts; i++) state.moeRouterB.setFloat(ro + i, state.moeRouterB.getFloat(ro + i) + moe.expProbsBias().getFloat(i));
            }
            if (config.expertGatingFunc == 2) state.moeRouterB.mapInPlace(ro, nExperts, v -> (float) (1.0 / (1.0 + Math.exp(-v))));
            ...
            for (int ki = 0; ki < topK; ki++) {
                ...
                for (int ei = 0; ei < nExperts; ei++) {
                    ...
                    if (v > bestVal) { bestVal = v; best = ei; }
                }
        ...
        Moe.dispatch(r, dim, state.xb, state.moeGather, state.moeDownB, state.moeOutB, null,
                (e, n, gather, out) -> {
                    moe.gateExps().gemm(gather, dim, state.hb, expertFF, n, expertFF, dim, (long) e * expertFF * dim);
                    moe.upExps().gemm(gather, dim, state.hb2, expertFF, n, expertFF, dim, (long) e * expertFF * dim);
                    Parallel.forRows(n, j -> Activations.siluMultiply(state.hb, j * expertFF, state.hb2, j * expertFF, expertFF));
                    moe.downExps().gemm(state.hb, expertFF, out, dim, n, dim, expertFF, (long) e * dim * expertFF);
                });

        Parallel.forRows(seqLen, s -> {
            if (postFfnNorm != null) rmsnorm(state.moeOutB, (long) s * dim, state.moeOutB, (long) s * dim, postFfnNorm, dim, eps);
    ...
    public record Configuration(int embeddingLength, int[] feedForwardLength, int numberOfLayers,
                                int numberOfHeads, int[] numberOfKeyValueHeadsPerLayer, int vocabularySize,
                                int contextLength, float rmsNormEps, float ropeTheta, int headSizeFull,
                                float logitSoftcapping, int shortConvLCache, int expertCount, int expertUsedCount,
                                int expertFeedForwardLength, int leadingDenseBlockCount, int expertGatingFunc)
            implements Config {
        public int headSize() { return headSizeFull; }
        public int queryDim() { return numberOfHeads * headSizeFull; }
        public int kvDim(int layer) { return numberOfKeyValueHeadsPerLayer[layer] * headSizeFull; }
        public boolean isRecurrentLayer(int layer) { return numberOfKeyValueHeadsPerLayer[layer] == 0; }
    ...
    public record Weights(FloatTensor tokenEmbeddings, LayerWeights[] layers, F32FloatTensor finalNorm,
                          F32FloatTensor ropeReal, F32FloatTensor ropeImag, FloatTensor wcls) {}
    ...
    public static final class State implements RuntimeState {
        ...
        // MoE scratch (chunk-wide CSR routing); allocated only when the model has experts, else null.
    ...
    public static Lfm2 loadModel(FileChannel fileChannel, GGUF gguf, int contextLength, boolean loadWeightsFlag) throws IOException {
        ...
        float logitSoftcapping = gguf.getValueOrDefault(float.class, arch + ".final_logit_softcapping", 0f);
        ...
        int leadingDenseBlockCount = gguf.getValueOrDefault(int.class, arch + ".leading_dense_block_count", numberOfLayers);
        ...
        // Per-layer kv-head count: 0 marks a recurrent (short-conv) layer (no attn_k tensor); attention
        ...
        Configuration config = new Configuration(embeddingLength, feedForwardLength, numberOfLayers, numberOfHeads,
                kvHeads, tokenizer.vocabularySize(), contextLength, rmsNormEps, ropeTheta, headSizeFull,
                logitSoftcapping, shortConvLCache, expertCount, expertUsedCount, expertFeedForwardLength,
                leadingDenseBlockCount, expertGatingFunc);
    ...
    static Weights loadWeights(Map<String, GGMLTensorEntry> tensors, Configuration config) {
        int n = config.numberOfLayers;
        Pair<float[], float[]> rope = RoPE.precomputeFreqsCis(config.contextLength(), config.headSizeFull, config.ropeTheta);
        ...
        FloatTensor wcls = tensors.containsKey("output.weight")
                ? ModelLoader.loadQuantized(tensors.get("output.weight")) : tokenEmbeddings;   // tied embeddings
        // LFM2.5 names the final norm token_embd_norm (no separate output_norm); embeddings are tied.
        F32FloatTensor finalNorm = ModelLoader.toF32Tensor(tensors.containsKey("output_norm.weight")
                ? tensors.get("output_norm.weight") : tensors.get("token_embd_norm.weight"));

        LayerWeights[] layers = new LayerWeights[n];
        for (int i = 0; i < n; i++) {
            String p = "blk." + i + ".";
            F32FloatTensor attnNorm = ModelLoader.toF32Tensor(tensors.get(p + "attn_norm.weight"));
            F32FloatTensor postAttnNorm = ModelLoader.f32OrNull(tensors, p + "post_attention_norm.weight");
            ...
            if (config.isRecurrentLayer(i)) {
                shortConv = new ShortConvWeights(
                        ModelLoader.toF32Tensor(tensors.get(p + "shortconv.conv.weight")),
                        ModelLoader.loadQuantized(tensors.get(p + "shortconv.in_proj.weight")),
                        ModelLoader.loadQuantized(tensors.get(p + "shortconv.out_proj.weight")));
            } else {
            ...
            layers[i] = new LayerWeights(attnNorm, postAttnNorm, ffnNorm, postFfnNorm, attention, shortConv, dense, moe);
        }
