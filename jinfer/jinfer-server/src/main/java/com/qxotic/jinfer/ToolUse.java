...
final class ToolUse {
    ...
    /** tool_choice "required"/named function: seed the assistant turn with {@code <|tool_call_start|>}
     *  (plus "[name" for a named choice) so the model can only complete a tool call instead of merely
     *  being asked to make one. The open paren is deliberately NOT seeded: a bare trailing "(" lands
    ...
    static GenerationResult parse(Model model, GenerationResult result, Map<String, Object> request) {
