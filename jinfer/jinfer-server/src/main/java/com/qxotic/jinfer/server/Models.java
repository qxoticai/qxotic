...
// (each bundles itself via loaded()). The single place that knows the full arch -> port mapping for
// the
...
public final class Models {
    ...
    public static LoadedModel<?> load(FileChannel fileChannel, GGUF gguf, int ctx)
        ...
        return switch (arch) {
            case "gemma4" -> Gemma4.loadModel(fileChannel, gguf, ctx, true).loaded();
            case "gpt-oss" -> GptOss.loadModel(fileChannel, gguf, ctx, true).loaded();
            case "qwen35", "qwen35moe" -> Qwen35.loadModel(fileChannel, gguf, ctx, true).loaded();
            case "nemotron_h", "nemotron_h_moe" ->
                    NemotronH.loadModel(fileChannel, gguf, ctx).loaded();
            case "llama", "minicpm", "mistral3", "smollm3" ->
                    Llama.loadModel(fileChannel, gguf, ctx, true).loaded(); // same-graph variants
            case "granite" -> Granite.loadModel(fileChannel, gguf, ctx, true).loaded();
            default -> {
                if (arch.startsWith("lfm"))
                    yield Lfm2.loadModel(fileChannel, gguf, ctx, true).loaded();
