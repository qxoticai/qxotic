...
final class Http {
    ...
    static boolean preamble(HttpExchange exchange, ServerConfig.Access access) throws IOException {
        ...
        if (authorized(exchange, access.bearerToken())) return false;
    ...
    private static boolean cors(HttpExchange exchange, ServerConfig.Access access) {
        ...
        if (origin != null) {
            boolean wildcard = access.allowedOrigins().contains("*");
            if (!wildcard && !access.allowedOrigins().contains(origin)) return false;
            headers.set("Access-Control-Allow-Origin", wildcard ? "*" : origin);
            if (!wildcard) headers.set("Vary", "Origin");
        }
