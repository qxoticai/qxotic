...
final class Validation {
    ...
    static void validateChatRequest(Map<String, Object> request) {
        ...
        for (Object message : messages) {
            ...
            LLMOptions.require(List.of("system", "user", "assistant", "tool").contains(role),
        ...
        if (fmt instanceof Map<?, ?> m) {
            ...
            LLMOptions.require("json_object".equals(type) || "text".equals(type),
            ...
            if ("json_object".equals(type)) {
                ...
                LLMOptions.require(hasJsonHint,
        ...
        if (toolChoice instanceof String s) {
            LLMOptions.require(List.of("auto", "none", "required").contains(s), "tool_choice must be auto, none, required, or a function choice object");
        } else if (toolChoice instanceof Map<?, ?> map) {
            LLMOptions.require("function".equals(map.get("type")), "Only function tool_choice objects are supported");
            Object function = map.get("function");
            LLMOptions.require(function instanceof Map<?, ?> fn && fn.get("name") instanceof String, "tool_choice.function.name is required");
    ...
    static void validateTool(Object value) {
        ...
        LLMOptions.require("function".equals(Values.stringValue(tool.get("type"), "function")), "Only function tools are supported");
        Map<String, Object> function = Values.asObject(tool.get("function"), "tool.function");
        LLMOptions.require(function.get("name") instanceof String name && !name.isBlank(), "tool.function.name is required");
    }
    ...
    static void validateGenerationParams(Map<String, Object> request, LLMOptions options) {
        LLMOptions.require(request.get("model") instanceof String name && !name.isBlank(), "model is required");
        if (request.get("model") instanceof String name) {
            String served = options.modelPath().getFileName().toString();
            LLMOptions.require(name.equalsIgnoreCase(served), "Unknown model: %s (this server serves %s)", name, served);
        }
        ...
        LLMOptions.require(Float.isFinite(temperature) && 0 <= temperature && temperature <= 2, "Invalid argument: temperature must be within [0, 2]");
        float topp = Values.floatValue(request.get("top_p"), options.topp());
        LLMOptions.require(Float.isFinite(topp) && 0 <= topp && topp <= 1, "Invalid argument: top_p must be within [0, 1]");
        LLMOptions.require(0 <= Values.intValue(request.getOrDefault("max_tokens", request.get("max_completion_tokens")), options.maxTokens()), "Invalid argument: max_tokens must be non-negative");
        LLMOptions.require(-1 <= Values.intValue(request.get("reasoning_max_tokens"), -1), "Invalid argument: reasoning_max_tokens must be -1 (uncapped) or non-negative");
        Values.longValue(request.get("seed"), 0); // type check only
        LLMOptions.require(!request.containsKey("logprobs") && !request.containsKey("top_logprobs"),
                "logprobs is not supported");
        LLMOptions.require(!request.containsKey("logit_bias"), "logit_bias is not supported");
        LLMOptions.require(!request.containsKey("frequency_penalty") && !request.containsKey("presence_penalty"),
