 ...
 * embedder the generic loader resolves ({@link Models#loadEmbedder} - no per-architecture code):
 * many variable-length sequences packed into segmented forwards over one KV context, each
 * sequence's pooled vector streamed out. The workload is legacy {@code EmbedBench}'s verbatim
 * (ragged lengths by multiplicative hash, greedy content-independent filler tokens), the timing
 * loop is {@code JinferBench}'s (adaptive warmup until throughput settles, then timed reps).
 *
 * <pre>
 *   java ... EmbedBench -m model.gguf [-s 256] [--minlen 8] [--maxlen 64] [-b 512] [-r 5] [-w 3]
 * </pre>
 */
public final class EmbedBench {
    ...
    private static void usage(PrintStream out) {
        out.println(
                """
                EmbedBench - ragged/packed batched-embedding throughput

                usage: EmbedBench -m <embedder.gguf> [options]
