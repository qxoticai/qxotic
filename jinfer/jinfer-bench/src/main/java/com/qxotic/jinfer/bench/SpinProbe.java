        ...
        Parallel.onDecodePool(
                () -> {
                    for (int w = 0; w < 5000; w++)
                        Parallel.parallelFor(0, RuntimeFlags.DECODE_THREADS, i -> {}); // warm
                    long t0 = System.nanoTime();
                    for (int i = 0; i < iters; i++)
                        Parallel.parallelFor(0, RuntimeFlags.DECODE_THREADS, j -> {});
                    long ns = System.nanoTime() - t0;
                    System.err.printf(
                            "empty parallelFor (%d participants): %.3f us/region  over %d iters%n",
