...
/**
 * Throughput benchmark for the ragged/packed batched-embedding path ({@link EmbeddingModel#embed}):
 * many variable-length sequences packed into a single segmented forward over one KV context, each
 * sequence's pooled vector streamed out. Reports tokens/s and sequences/s, warmed adaptively like
 * {@link JinferBench} (the JVM must reach JIT/GC steady state before timing). Greedy,
 * content-independent filler tokens.
 *
 * <pre>
 * embed-bench -m Qwen3-Embedding-*.gguf [-s 256] [--minlen 8] [--maxlen 64] [-b 512] [-r 5] [-w 3]
 * </pre>
 */
public final class EmbedBench {

    private static volatile double
            sink; // blackhole so the pooled vectors aren't dead-code-eliminated
    ...
    public static void main(String[] args) throws Exception {
        ...
        Qwen3.State state =
                model.newState(ctx, batchCap); // embed() resets it each call, so reuse it
    ...
    private static double runOnce(
            Qwen3 model, Qwen3.State state, Batch.Input.Sequences seqs, int total, int nSeq) {
        int[] got = {0};
        ...
        if (got[0] != nSeq)
            throw new IllegalStateException("expected " + nSeq + " embeddings, got " + got[0]);
        return tps;
    }
