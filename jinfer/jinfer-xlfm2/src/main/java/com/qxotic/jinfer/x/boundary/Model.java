 ...
 * output — a {@link LanguageModel} adds a vocab-logits head, an {@link EmbeddingModel} a pooled
 * head. Weight-bearing (the role HuggingFace calls {@code XxxModel}); weights are captured so
 * {@link #ingest} never threads them, and exposed so a model can be cheaply cloned over shared
 * weights ({@code new Impl(config(), weights())}). State is caller-owned, forkable, and many run at
 * once.
 *
 ...
 * read raw addresses via {@code Segments.GLOBAL_SEGMENT} - the JDK's close handshake cannot save a
 * raw read, so a violation is a crash, not an exception), and the weights arena must outlive every
 * model sharing those weights. Every public entry point that runs kernels ({@link #ingest}, a head
 * projection) is a default wrapper: it claims the state (fail-fast single-pipeline contract, see
 * {@link BaseState#enter()}) and its trailing reachability fence pins the model across the call -
 * implementations override the unfenced seam ({@link #forward}, ...) and callers never think about
 * it.
 */
