// Diff the new com.qxotic.llm.Granite port against the production com.qxotic.jinfer Llama3 (granite) on
// the same tokens: prefill both, compare next-token argmax + max per-logit diff, then 8 decode steps.
//   java ... com.qxotic.llm.GraniteCompare <model.gguf> [prompt]
...
public final class GraniteCompare {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        String prompt = args.length > 1 ? args[1] : "The capital of France is";

        Granite nw = Granite.loadModel(Path.of(path), 4096);
        var cfg = nw.config();
        var tk = nw.tokenizer();
        List<Integer> pt = new ArrayList<>();
        if (cfg.addBos()) pt.add(cfg.bosTokenId());
        ...
        Granite.State ns = nw.newState(4096, Math.max(16, ids.length));
