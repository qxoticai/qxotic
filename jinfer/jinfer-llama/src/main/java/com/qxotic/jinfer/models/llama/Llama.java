// The standard Llama transformer (RoPE GQA attention + SwiGLU FFN + RMSNorm) against the
...
public final class Llama implements LanguageModel<Llama.Configuration, Llama.Weights, Llama.State> {
    ...
    public FloatTensor logits(State s, int output) {
        ...
        return Parallel.onDecodePool(
                () -> {
                    tailAt(s, row); // finish the deferred last-layer tail for this row -> s.th (s.x
                    // stays read-only)
                    ...
                    if (ls != 1.0f)
                        s.logits.mapInPlace(0, configuration.vocabularySize, v -> v / ls);
                    return s.logits;
                });
    }
    ...
    void forward(State state, int[] tokens, int tokenOffset, int startPos, int seqLen) {
        ...
        // for the tail here; the saving is the last layer's attention+FFN skipped for every
        // un-queried row.
     ...
     * cache. No Q, no attention, no O, no FFN - and state.x is left untouched (the last-layer input
     * residual).
     */
    private void writeKv(State state, int l, int startPos, int seqLen) {
        ...
        commitKv(
                state, l, startPos,
                seqLen); // the tail reads every row (incl. its own) from the F16 cache
    }

    /**
     * Lazy tail: finish the last layer for retained chunk-row {@code i} into state.th, reading its
     * input from state.x[i] and attending cache[0..pos] inclusive - a single causal query aimed at
     * row i (its own K/V is already in the F16 cache from writeKv, read like any other position).
     * state.x is never written.
     */
    private void tailAt(State state, int i) {
        ...
        rmsnorm(
                ...
                w.attnNorm[L],
                dim,
                eps); // pre-norm reads s.x[i] directly (read-only)
        w.wq[L].gemm(
                state.tscratch,
                dim,
                state.attnQ,
        ...
        // Single causal query over cache[0..pos] INCLUSIVE (bK/bV = null): row i's own K/V is
        // already in the
        // F16 cache from writeKv, read like every other position - no separate current-token
        // buffer.
     ...
     * attn-temp), K/V appended to the contiguous cache, then causal flash attention (or scalar
     * single-token decode), output projection written back to {@code state.xb}.
     */
    private void attention(State state, int layer, int startPos, int seqLen) {
        ...
        Parallel.forRows(
                ...
                s -> {
                    ...
                    if (aScale != 1.0f)
                        state.attnQ.mapInPlace((long) s * fQDim, fQDim, v -> v * aScale);
                });

        FloatTensor keyCache = state.keyCache[layer], valueCache = state.valueCache[layer];
        float attScale = config.attentionScale();
        // Full causal attention over cache[0..startPos) + this chunk's F32 K/V (state.k/v);
        // window=0.
        if (seqLen == 1) {
            FlashAttention.flashDecode(
                    (F32FloatTensor) state.attnQ,
        ...
        commitKv(
                state, layer, startPos,
                seqLen); // commit this chunk's K/V to the cache for later positions
    ...
    public static Llama loadModel(
        ...
        boolean addBos =
                gguf.getValueOrDefault(boolean.class, "tokenizer.ggml.add_bos_token", true);
