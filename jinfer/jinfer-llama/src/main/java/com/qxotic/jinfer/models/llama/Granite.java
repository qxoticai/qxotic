// Granite 4.1 (dense "granite" architecture) against the com.qxotic.jinfer.models model API: a
// focused port
// of the
// production jinfer Llama3's granite path. A standard RoPE GQA + SwiGLU + RMSNorm transformer with
// Granite's four scalars baked in: embedding_scale (x token embeddings), residual_scale (x each
// sublayer
// output before the residual add), logit_scale (logits DIVIDED by it), and a custom attention.scale
// that
// REPLACES 1/sqrt(headSize). Interleaved RoPE (GGUF "llama" pair convention), plain freq base (no
// YaRN,
...
public final class Granite
     ...
     * interleaved RoPE, K/V appended to the cache, causal flash attention (or scalar single-token
     * decode), output projection back to {@code state.xb}. (No attention-temperature tuning —
     * that's mistral3.)
     */
    private void attention(State state, int layer, int startPos, int seqLen) {
        ...
        for (int s = 0;
                s < seqLen;
                s++) { // commit this chunk's K/V to the cache for later positions
    ...
    public static Granite loadModel(Path ggufPath, int contextLength, Arena arena)
            throws IOException {
