...
// embedder): aspect-preserving smart-resize, bilinear HWC->CHW resample, and patch im2col. Pure
// geometry -
...
final class VisionPreprocess {
    ...
    /**
     * Default ON. Aspect-preserving like llama.cpp (fewer patches -> faster). Off
     * (-Dvis.squareResize) = fixed square, for gemma4.java-reference parity.
     */
    ...
    static int budget(int deflt) {
        return IMAGE_TOKEN_BUDGET > 0 ? IMAGE_TOKEN_BUDGET : deflt;
    }
    ...
    /**
     * Bilinear (half-pixel centers) resample of an HWC image to a [3, th, tw] CHW plane, values in
     * [0,1].
     */
    static float[] toCHW(Media.Image im, int tw, int th) {
        ...
        for (int y = 0; y < th; y++)
            for (int x = 0; x < tw; x++) {
                ...
                int y0 = Math.max(0, Math.min(H - 1, (int) Math.floor(fy))),
                        y1 = Math.min(H - 1, y0 + 1);
                int x0 = Math.max(0, Math.min(W - 1, (int) Math.floor(fx))),
                        x1 = Math.min(W - 1, x0 + 1);
                float wy = Math.max(0f, fy - y0), wx = Math.max(0f, fx - x0);
                for (int c = 0; c < 3; c++) {
                    int cc = Math.min(c, C - 1);
                    float a = v[(y0 * W + x0) * C + cc],
                            b = v[(y0 * W + x1) * C + cc],
                            d0 = v[(y1 * W + x0) * C + cc],
                            d1 = v[(y1 * W + x1) * C + cc];
                    out[c * plane + y * tw + x] =
                    ...
                    for (int c = 0; c < 3; c++)
                        for (int ky = 0; ky < ps; ky++)
                            for (int kx = 0; kx < ps; kx++) {
                                float pix =
                                        chw[c * plane + (gy * ps + ky) * tw + (gx * ps + kx)] * 2f
                                                - 1f;
