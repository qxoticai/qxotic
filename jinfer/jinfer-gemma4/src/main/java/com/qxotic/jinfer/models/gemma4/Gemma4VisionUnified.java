...
// transformer at all - no attention, no FFN, no per-block norms. The 3x3 token merge is baked
// directly
// into a bigger conv patch: the base patch (16) is multiplied by n_merge (3) to a 48x48 conv, so
// each
// token already spans a 48x48 region (48*48*3 = 6912 input features) and n_merge becomes 1.
//
// Graph (all patches batched):
//   im2col 48x48            -> [n, 6912]      (channel-outer [c, ky, kx] ordering, pixels scaled to
// [-1,1])
...
//
// The three patch norms are PyTorch LayerNorm (mean-subtracting, weight+bias, eps 1e-5); only the
// final
// pre-projection norm is RMSNorm (eps = clip.vision.attention.layer_norm_epsilon, ~1e-6). Resize is
// the
// same aspect-preserving smart_resize as gemma4v (BILINEAR, factor 48), so a 640x480 image yields
// the
...
public final class Gemma4VisionUnified implements Embedder<Media.Image> {
    ...
    Gemma4VisionUnified(
        ...
        this.patchVec = 3 * patchSize * patchSize;
        this.rmsEps = rmsEps;
        this.patchEmbd = patchEmbd;
        this.patchBias = patchBias;
    ...
    private static void layerNorm(
            FloatTensor x, int n, int dim, F32FloatTensor w, F32FloatTensor b) {
        Parallel.forRows(
                n,
                t -> {
                    long off = (long) t * dim;
                    ...
                    for (int d = 0; d < dim; d++)
                        x.setFloat(
                                off + d,
                                (x.getFloat(off + d) - mean) * inv * w.getFloat(d) + b.getFloat(d));
                });
    }
    ...
    private static void addBias(FloatTensor x, int n, int dim, FloatTensor bias) {
        Parallel.forRows(
                ...
                t -> {
                    long off = (long) t * dim;
                    for (int d = 0; d < dim; d++)
                        x.setFloat(off + d, x.getFloat(off + d) + bias.getFloat(d));
                });
    }
    ...
    public static Gemma4VisionUnified loadModel(Path mmprojPath) throws IOException {
        try (FileChannel fc = FileChannel.open(mmprojPath, StandardOpenOption.READ)) {
            ...
            int patchSize =
                    basePatch * Math.max(1, merge); // unified bakes the merge into the conv patch
            int visionDim = gguf.getValueOrDefault(int.class, "clip.vision.embedding_length", 3840);
            int modelDim =
                    gguf.getValueOrDefault(int.class, "clip.vision.projection_dim", visionDim);
            float rmsEps =
            ...
            return new Gemma4VisionUnified(
                    ...
                    posEmbd,
                    mmProj,
                    ModelLoader.f32OrNull(t, "v.patch_norm.1.weight"),
                    ModelLoader.f32OrNull(t, "v.patch_norm.1.bias"),
                    ModelLoader.f32OrNull(t, "v.patch_norm.2.weight"),
                    ModelLoader.f32OrNull(t, "v.patch_norm.2.bias"),
                    ModelLoader.f32OrNull(t, "v.patch_norm.3.weight"),
                    ModelLoader.f32OrNull(t, "v.patch_norm.3.bias"));
        }
    }
}
