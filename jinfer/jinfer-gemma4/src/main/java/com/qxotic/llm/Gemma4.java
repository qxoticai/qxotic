...
// (A4B) FFN paths are both ported. gemma-4 is a multimodal, MTP-capable architecture, so this implements
// MultiModal and MultiToken (instanceof = the arch supports it). The current text-only GGUFs load neither
...
public final class Gemma4 implements LanguageModel<Gemma4.Configuration, Gemma4.Weights, Gemma4.State>,
     ...
     *  residual in {@code state.residual}; finalize the output head for that row. (LAST → output 0 is the
     *  last row's residual; ALL → output indexes the rows in order.) */
    @Override
    public FloatTensor logits(State s, int output) {
        ...
        // The vocab projection (vocab × dim) is the heaviest single op in decode and memory-bandwidth
        ...
        return Parallel.onDecodePool(() -> {
            rmsnorm(s.xb, 0, s.residual, row * dim, weights.rmsFinalWeights, dim, configuration.rmsNormEps());
            weights.classifierWeights.matmul(s.xb, s.logits, configuration.vocabularySize(), dim);
            Activations.softcap(s.logits, 0, configuration.vocabularySize(), configuration.logitSoftcapping());
            return s.logits;
        });
    }
    ...
    public record Configuration(
        ...
        /** KV ring slots a state with this {@code contextCapacity} needs for {@code layer}: the capacity
         *  for full layers, capped at the window for SWA layers (the ring never holds more than the
         *  window, and a sub-window capacity never wraps the {@code & (slidingWindow-1)} index). */
    ...
    public static final class Weights {
        ...
        final FloatTensor[] queryWeights, keyWeights, valueWeights, outputWeights;        // valueWeights[l] null => V = K
        ...
        Weights(FloatTensor tokenEmbeddings, F32FloatTensor[] rmsAttentionWeights, FloatTensor[] queryWeights, FloatTensor[] keyWeights,
                ...
                F32FloatTensor[] rmsPostAttentionWeights, F32FloatTensor[] rmsFFNWeights, FloatTensor[] ffnGate, FloatTensor[] ffnDown,
                FloatTensor[] ffnUp, F32FloatTensor[] rmsPostFFNWeights, F32FloatTensor rmsFinalWeights, float[] layerOutputScales,
                F32FloatTensor ropeRealFull, F32FloatTensor ropeImagFull, F32FloatTensor ropeRealSWA,
    ...
    public static final class State extends com.qxotic.jinfer.BaseState {
        // Batched scratch (batchCapacity rows): the residual stream and projections hold one row per
        ...
        // MoE scratch (chunk-wide CSR routing + expert/shared buffers); allocated only for MoE (A4B), else null.
        ...
        State(Configuration config, int contextCapacity, int batchCapacity) {
            ...
            this.perLayerInputs = plDim > 0 ? FloatTensor.allocateF32(c * plDim * config.numberOfLayers()) : null;
     ...
     * for every row in {@code state.residual}; {@link #logits} finalizes the retained row(s).
     */
     ...
     *  each layer's slice, and fold in the per-layer token embedding. No-op when the model has no PLE. */
    private void buildPerLayerInputs(State state, int[] tokens, int tokenOffset, int seqLen) {
        ...
        weights.perLayerModelProjection.gemm(state.residual, dim, state.perLayerInputs, plTotal, seqLen, plTotal, dim);
        Parallel.forRows(seqLen, s -> {   // per-layer RMS-norm + token-embedding add, parallel over rows
            int base = s * plTotal;
            for (int i = 0; i < plTotal; i++) state.perLayerInputs.setFloat(base + i, state.perLayerInputs.getFloat(base + i) * projScale);
            for (int l = 0; l < numLayers; l++) {
                rmsnorm(state.perLayerInputs, base + l * plDim, state.perLayerInputs, base + l * plDim,
                        weights.rmsPerLayerProjectionWeights, plDim, config.rmsNormEps());
            }
            long tokEmbOffset = (long) tokens[tokenOffset + s] * plTotal;
            for (int i = 0; i < plTotal; i++) {
                float tokEmb = weights.perLayerTokenEmbeddings.getFloat(tokEmbOffset + i) * sqrtPlDim;
    ...
    private void attention(State state, int l, int startPos, int seqLen, boolean nonCausal) {
        ...
        int nKvHeads = config.numberOfKeyValueHeadsPerLayer()[l], kvMul = config.numberOfHeads() / nKvHeads;
        ...
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb, s * dim, state.residual, s * dim, attNormW, dim, eps));

        // Q projection, per-head Q-norm + RoPE
        weights.queryWeights[l].gemm(state.xb, dim, state.query, queryDim, seqLen, queryDim, dim);
        headNormRope(state.query, queryDim, config.numberOfHeads(), headSize, halfHead, weights.queryNormWeights[l], startPos, seqLen, real, imag);

        // K/V projection into the per-layer LINEAR batch buffer (own-KV layers): K-norm + RoPE, V
        // no-weight norm. NOT written to the ring yet — committed at chunk end so prior reads stay intact.
        if (config.hasKv(l)) {
            ...
            headNormRope(bKl, kvDim, nKvHeads, headSize, halfHead, weights.keyNormWeights[l], startPos, seqLen, real, imag);
            Parallel.forRows(seqLen, s -> {
                for (int h = 0; h < nKvHeads; h++) {
                    rmsnormNoWeight(bVl, s * kvDim + h * headSize, bVl, s * kvDim + h * headSize, headSize, eps);
                }
            });
        }
        ...
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb2, s * dim, state.xb2, s * dim, postAttW, dim, eps));
        state.residual.addInPlace(0, state.xb2, 0, seqLen * dim);
    }

    /** Per-head RMS-norm then NeoX RoPE over each row of {@code t}: {@code nHeads} heads of
     *  {@code headSize}, rows {@code rowStride} apart, rotated at absolute position {@code startPos + row}.
     *  Shared by the Q and K projections. */
    private void headNormRope(FloatTensor t, int rowStride, int nHeads, int headSize, int halfHead,
                              F32FloatTensor normW, int startPos, int seqLen, F32FloatTensor real, F32FloatTensor imag) {
        ...
        Parallel.forRows(seqLen, s -> {
            ...
            RoPE.applyNeox(t, s * rowStride, nHeads, headSize, halfHead, startPos + s, real, imag);
        });
    }

    /** Position-wise FFN over the chunk, added to the residual: dense GeGLU, or the shared-MLP + top-k
     *  expert MoE (A4B) when the layer routes. */
    private void feedForward(State state, int l, int seqLen) {
        ...
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb, s * dim, state.residual, s * dim, ffnNormW, dim, eps));
        ...
        Parallel.forRows(seqLen, s -> Activations.geluMultiply(state.hb, s * hiddenDim, state.hb2, s * hiddenDim, hiddenDim));
        weights.ffnDown[l].gemm(state.hb, hiddenDim, state.xb, dim, seqLen, dim, hiddenDim);
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb, s * dim, state.xb, s * dim, postFfwW, dim, eps));
        state.residual.addInPlace(0, state.xb, 0, seqLen * dim);
    }

    /** Per-layer embedding (Gemma-3n PLE): GELU-gated projection of this layer's PLE slice, added to the
     *  residual. No-op when the model has no PLE. */
    private void mergePerLayerInput(State state, int l, int seqLen) {
        ...
        weights.perLayerInputGate[l].gemm(state.residual, dim, state.plGate, plDim, seqLen, plDim, dim);
        Parallel.forRows(seqLen, s -> Activations.geluMultiply(state.plGate, s * plDim, state.perLayerInputs, s * plTotal + l * plDim, plDim));
        weights.perLayerProjection[l].gemm(state.plGate, plDim, state.plProj, dim, seqLen, dim, plDim);
        Parallel.forRows(seqLen, s -> rmsnorm(state.plProj, s * dim, state.plProj, s * dim, plPostW, dim, eps));
     ...
     *  materialized (no {@code att} scratch). Reads the single in-chunk key from the batch buffer and
     *  prior keys from the ring cache. Mathematically identical to the two-pass softmax — and to the
     *  flash prefill path ({@link #flashAttention}), which already uses online softmax. */
    private void decodeAttention(State state, int layer, int position,
        ...
        FlashAttention.flashDecode((F32FloatTensor) state.query, (F32FloatTensor) state.xbK,
                state.keyCache[kvLayer], state.valueCache[kvLayer], state.batchK[kvLayer], state.batchV[kvLayer],
                configuration.numberOfHeads(), position, attStart, headSize, kvDim, kvMul,
                1.0f, isSWA ? window - 1 : 0, null, state.decodeScratch);
    }

    /** Shared dense MLP + top-k expert MoE FFN (the A4B variant), batched across the chunk. Shared MLP
     *  runs as batched GEMMs; the expert FFN groups rows by routed expert (CSR, via {@link Moe#dispatch})
     *  so each expert's weights are read once per chunk. Output is
    ...
    private void moeFeedForward(State state, int l, int dim, int hiddenDim, int seqLen) {
        ...
        float eps = config.rmsNormEps();
        int nExperts = config.expertCount(), topK = config.expertUsedCount(), expertFF = config.expertFeedForwardLength();
        ...
        F32FloatTensor ffnNormW = weights.rmsFFNWeights[l], postNorm1 = weights.rmsExpertPostNorm1[l];
        Parallel.forRows(seqLen, s -> rmsnorm(state.xb, s * dim, state.residual, s * dim, ffnNormW, dim, eps));
        weights.ffnGate[l].gemm(state.xb, dim, state.hb, hiddenDim, seqLen, hiddenDim, dim);
        weights.ffnUp[l].gemm(state.xb, dim, state.hb2, hiddenDim, seqLen, hiddenDim, dim);
        Parallel.forRows(seqLen, s -> Activations.geluMultiply(state.hb, s * hiddenDim, state.hb2, s * hiddenDim, hiddenDim));
        ...
        // Expert routing: both the expert input (pre_ffw_norm2) and the router input normalize the same
        // residual row by its RMS — compute the sum of squares once and apply the shared 1/rms scale to
        ...
        Parallel.forRows(seqLen, s -> {
            float rms = (float) (1.0 / Math.sqrt(sumOfSquares(state.residual, s * dim, dim) / dim + eps));
            scaleByWeight(state.moeInputB, s * dim, state.residual, s * dim, preNorm2, dim, rms);
            scaleByWeight(state.moeRouterScaled, s * dim, state.residual, s * dim, gateInpScale, dim, rms * invSqrtDim);
        });
        weights.expertRouter[l].gemm(state.moeRouterScaled, dim, state.moeRouterB, nExperts, seqLen, nExperts, dim);
        ...
        F32FloatTensor postNorm2 = weights.rmsExpertPostNorm2[l], postFfw = weights.rmsPostFFNWeights[l];
        Parallel.forRows(seqLen, s -> {
    ...
    public static Gemma4 loadModel(FileChannel fileChannel, GGUF gguf, int maxContextLength, boolean loadWeightsFlag) throws IOException {
        ...
        float ropeThetaFull = gguf.getValueOrDefault(float.class, "gemma4.rope.freq_base", 1000000f);
        ...
        Configuration config = new Configuration(embeddingLength, feedForwardLength, numberOfLayers, numberOfHeads,
                numberOfKeyValueHeadsPerLayer, tokenizer.vocabularySize(), maxContextLength, rmsNormEps, ropeThetaFull,
                ropeThetaSWA, headSizeFull, headSizeSWA, slidingWindow, logitSoftcapping, isSWA, ownKvLayers,
    ...
    static Weights loadWeights(Map<String, GGMLTensorEntry> tensors, Configuration config) {
        ...
        if (config.embeddingLengthPerLayer() > 0 && tensors.containsKey("per_layer_token_embd.weight")) {
            perLayerTokenEmbeddings = ModelLoader.loadQuantized(tensors.get("per_layer_token_embd.weight"));
            perLayerModelProjection = ModelLoader.loadQuantized(tensors.get("per_layer_model_proj.weight"));
            rmsPerLayerProjectionWeights = ModelLoader.toF32Tensor(tensors.get("per_layer_proj_norm.weight"));
            perLayerInputGate = ModelLoader.quantArray(n, i -> tensors.get("blk." + i + ".inp_gate.weight"));
            perLayerProjection = ModelLoader.quantArray(n, i -> tensors.get("blk." + i + ".proj.weight"));
