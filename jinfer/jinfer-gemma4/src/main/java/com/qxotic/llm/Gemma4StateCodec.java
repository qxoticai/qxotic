 ...
 *  layers reuse earlier rings and carry no state. Full-attention own layers store per-position K/V rows
 *  (shared base); sliding-window own layers checkpoint a FIXED-SIZE snapshot: the live window rows
 *  {@code [max(0,to-W), to)}, read from their ring slots ({@code pos & (W-1)}), valid only at
 *  {@code position == to} — which is exactly why blocks match completely or not at all. RoPE is baked
 *  into K at absolute positions, so restore writes each row back at its true slot; the ring span wraps at
 *  most once, so each direction is at most two contiguous copies (see {@link KvTransfer#window}). */
public final class Gemma4StateCodec extends AbstractStateCodec<Gemma4.State> {
    ...
    public Gemma4StateCodec(Gemma4.Configuration config) {
        super(config.ownKvLayers(), l -> !config.isSWA()[l], l -> config.kvDim(l),
