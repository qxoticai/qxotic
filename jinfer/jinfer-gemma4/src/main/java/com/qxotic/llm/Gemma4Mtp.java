...
/**
 * Gemma 4 MTP (multi-token prediction / self-speculative draft head), arch {@code gemma4-assistant}.
 * A 4-layer draft transformer that, at a decoded position, predicts the NEXT token from the backbone's
 * final hidden and the just-sampled token, then chains its own hidden to draft further.
 *
 * <p>Structure (from llama.cpp {@code src/models/gemma4-assistant.cpp}, verified against the sidecar
 * GGUF): {@code xh = concat(backbone.tokEmbd[next]*sqrt(1536), backboneHidden[1536])} → {@code
 * pre_projection[3072,256]} → 4 layers → {@code output_norm} → tied {@code token_embd[256,vocab]} for
 * draft logits; {@code post_projection[256,1536]} produces the next backbone-dim hidden to chain
 * depth&gt;1.
 *
 * <p>The draft layers project Q ONLY (no K/V weights, {@code shared_kv_layers=4}) and attend against the
 * BACKBONE's KV cache: SWA draft layers (0-2) read backbone layer {@code ownKvLayers-2}, the full draft
 * layer (3) reads {@code ownKvLayers-1} - exactly jinfer's {@link Gemma4.Configuration#kvSourceLayer}
 * mapping, and llama.cpp's assistant KV-share map. This shared-KV Q-only attention is the crux the
 * draft-forward (Stage 2) implements and the token-identity gate verifies.
 *
 * <p>This class is the load + capability surface (Stage 1): a shape-verified sidecar reader. The draft
 * forward and the speculative loop are the subsequent stages; {@link Gemma4} exposes MTP through the core
 * {@code MultiToken} seam once they land.
 */
public final class Gemma4Mtp {
    ...
    public record Config(
        ...
        int headSize(int layer) { return isSWA[layer] ? headSizeSWA : headSizeFull; }
        int queryDim(int layer) { return numberOfHeads * headSize(layer); }
    }

    /** Draft weights. {@code tokenEmbeddings} is tied: the draft input embedding AND the draft LM head. */
    public static final class Weights {
        ...
        Weights(FloatTensor tokenEmbeddings, FloatTensor preProjection, FloatTensor postProjection,
                F32FloatTensor outputNorm, F32FloatTensor[] attnNorm, F32FloatTensor[] attnQNorm,
                F32FloatTensor[] attnPostNorm, F32FloatTensor[] ffnNorm, F32FloatTensor[] ffnPostNorm,
                FloatTensor[] wq, FloatTensor[] wo, FloatTensor[] ffnGate, FloatTensor[] ffnUp,
                FloatTensor[] ffnDown, float[] layerOutputScales, float[] ropeFreqFactors) {
    ...
    /** Loads and shape-verifies the MTP sidecar. {@code backboneVocab} is the backbone's vocab, which the
     *  tied draft head must match (the draft predicts backbone tokens). */
    public static Gemma4Mtp loadSidecar(Path sidecar, int backboneVocab) throws IOException {
        try (FileChannel fc = FileChannel.open(sidecar, StandardOpenOption.READ)) {
            ...
            if (!arch.equals("gemma4-assistant")) {
                throw new IllegalArgumentException("not a gemma4-assistant MTP sidecar: arch=" + arch);
            }
    ...
    private static Config readConfig(GGUF gguf, int backboneVocab) {
        ...
        // isSWA per layer: full-attention layers carry the larger head (key_length); SWA carry key_length_swa.
        // The GGUF flags the pattern via the per-layer attn_q width, matched here to the Gemma convention
    ...
    private static Weights loadWeights(Map<String, GGMLTensorEntry> t, Config c) {
        ...
        FloatTensor preProjection = req(t, "nextn.pre_projection.weight", 2L * c.backboneDim() * dim);
        FloatTensor postProjection = req(t, "nextn.post_projection.weight", (long) dim * c.backboneDim());
        ...
        F32FloatTensor[] attnPostNorm = new F32FloatTensor[n], ffnNorm = new F32FloatTensor[n], ffnPostNorm = new F32FloatTensor[n];
        FloatTensor[] wq = new FloatTensor[n], wo = new FloatTensor[n];
        FloatTensor[] ffnGate = new FloatTensor[n], ffnUp = new FloatTensor[n], ffnDown = new FloatTensor[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (scale == null) throw new IllegalStateException("missing " + b + "layer_output_scale.weight");
            outScales[i] = ModelLoader.loadQuantized(scale).getFloat(0);
        }
        float[] ropeFreqFactors = ModelLoader.ropeFreqFactors(t);
        return new Weights(tokenEmbeddings, preProjection, postProjection, outputNorm, attnNorm, attnQNorm,
                attnPostNorm, ffnNorm, ffnPostNorm, wq, wo, ffnGate, ffnUp, ffnDown, outScales, ropeFreqFactors);
    }

    private static FloatTensor req(Map<String, GGMLTensorEntry> t, String name, long expectedElems) {
        ...
        if (got != expectedElems) {
            throw new IllegalStateException(name + " shape " + java.util.Arrays.toString(e.shape())
                    + " = " + got + " elems, expected " + expectedElems);
        }
        return ModelLoader.loadQuantized(e);
    }

    private static F32FloatTensor f32(Map<String, GGMLTensorEntry> t, String name, long expectedElems) {
