// The standard Mistral3 transformer (RoPE GQA attention + SwiGLU FFN + RMSNorm) against the com.qxotic.llm
...
//   - Mistral3 3.x: "llama3" RoPE frequency scaling (rope_freqs.weight).
//   - MiniCPM:   embedding_scale / residual_scale / logit_scale (default 1.0 → plain Mistral3).
...
public final class Mistral3 implements LanguageModel<Mistral3.Configuration, Mistral3.Weights, Mistral3.State> {

    private final Configuration configuration;
    private final LFMTokenizer tokenizer;
    private final Weights weights;

    Mistral3(Configuration configuration, LFMTokenizer tokenizer, Weights weights) {
    ...
    public void ingest(State s, com.qxotic.jinfer.Batch batch) {
        ...
        switch (batch.input()) {
            ...
            case com.qxotic.jinfer.Batch.Input.Embeddings e ->
                throw new UnsupportedOperationException("Mistral3 is text-only: embedding input is not supported");
        }
        s.lastChunkLen = n;
    ...
    public static Mistral3 loadModel(Path ggufPath, int contextLength) throws IOException {
    ...
    static Mistral3 loadModel(FileChannel fileChannel, GGUF gguf, int contextLength, boolean loadWeightsFlag) throws IOException {
        ...
        if (!loadWeightsFlag) return new Mistral3(config, tokenizer, null);
        ...
        return new Mistral3(config, tokenizer, loadWeights(tensors, config, rope));
    }

    /** RoPE flavor from GGUF metadata: YaRN (mistral3), "llama3" per-frequency scaling (rope_freqs.weight),
     *  or plain RoPE (Mistral3/MiniCPM). Returns the interleaved cos/sin tables applyInterleaved consumes. */
