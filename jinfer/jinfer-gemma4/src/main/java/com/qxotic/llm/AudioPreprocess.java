...
// power spectrum |X|^2 -> mel filterbank (Slaney/librosa scale, area-normalized) -> max(.,mel_floor)
...
// NOT yet ported. Verify this against llama.cpp's `log_mel_spectrogram.json` dump before relying on it.
...
public final class AudioPreprocess {
    ...
    public float[] logMel(float[] pcm, int[] outFrames) {
        ...
        for (int i = 0; i < nFrames; i++) {
            int off = i * HOP - N_FFT / 2 + HOP / 2;   // center the window on the hop position (reflect at edges)
    ...
    private static float[] buildHann() {
        ...
        for (int i = 0; i < WINDOW; i++) {
            w[pad + i] = (float) (0.5 * (1.0 - Math.cos(2.0 * Math.PI * i / WINDOW)));   // periodic: /length
        }
        return w;
    }
    ...
    private static float[] buildMelFilterbank(int nMel) {
        ...
        java.util.function.DoubleUnaryOperator hzToMel = f -> (f < minLogHz) ? f * linSlope
                : minLogMel + Math.log(f / minLogHz) / logStep;
        java.util.function.DoubleUnaryOperator melToHz = m -> (m < minLogMel) ? m / linSlope
    ...
    static float[] toMono16k(Media.Audio audio) {
        ...
        if (ch != 1) for (int i = 0; i < frames; i++) {
            float s = 0f; for (int c = 0; c < ch; c++) s += in[i * ch + c]; mono[i] = s / ch;
        }
        ...
        for (int i = 0; i < outp.length; i++) {
            double sp = i / ratio; int j = (int) sp; double fr = sp - j;
            float a = mono[Math.min(j, mono.length - 1)], b = mono[Math.min(j + 1, mono.length - 1)];
