...
// NO conformer, NO attention. The raw 16 kHz waveform is chunked into non-overlapping 640-sample frames
// (640 samples @ 16 kHz = 40 ms per token, hop = 640, last frame zero-padded), and each frame becomes one
...
// clip.audio metadata: num_mel_bins is repurposed as the 640-sample FRAME SIZE (not a mel-bin count),
// projection_dim = embedding_length of the text model (3840). Attention over the audio tokens is CAUSAL
// (mtmd_decode_use_non_causal is false for GEMMA4UA) - unlike the bidirectional image path. Markers are
...
public final class Gemma4Audio implements Embedder<Media.Audio> {
    ...
    Gemma4Audio(int frameSize, int modelDim, float eps, FloatTensor mmProj) {
        this.frameSize = frameSize; this.modelDim = modelDim; this.eps = eps; this.mmProj = mmProj;
    }
    ...
    public FloatTensor encode(Media.Audio audio) {
        ...
        // 1. frame the raw waveform: [nTok, frameSize], last frame zero-padded (no windowing/normalization).
        FloatTensor frames = FloatTensor.allocateF32(nTok * frameSize);
        Parallel.forRows(nTok, t -> {
        ...
        // 2. per-frame RMSNorm (no weight; embedding_pre_projection_norm), then the linear projection.
        Parallel.forRows(nTok, t -> Norms.rmsnormNoWeight(frames, (long) t * frameSize, frames, (long) t * frameSize, frameSize, eps));
    ...
    public int tokenCount(int nSamples) { return Math.max(1, (nSamples + frameSize - 1) / frameSize); }

    /** Downmix to mono and resample to 16 kHz (linear). llama.cpp resamples with a higher-order kernel, so
     *  for exact parity supply already-16 kHz-mono audio; this keeps arbitrary inputs usable. */
    ...
    public static Gemma4Audio loadModel(Path mmprojPath) throws IOException {
        try (FileChannel fc = FileChannel.open(mmprojPath, StandardOpenOption.READ)) {
            ...
            float eps = gguf.getValueOrDefault(float.class, "clip.audio.attention.layer_norm_epsilon", 1e-6f);
            FloatTensor mmProj = ModelLoader.loadQuantized(t.get("mm.a.input_projection.weight"));
            // The raw-waveform FRAME SIZE (640) is the projection's input dim. clip.audio.num_mel_bins reads
            // 128 (a stale/repurposed value); llama.cpp hardcodes 640 for gemma4ua, so derive it from the weight.
