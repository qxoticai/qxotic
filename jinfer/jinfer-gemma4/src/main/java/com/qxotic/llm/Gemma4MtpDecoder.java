 ...
 * {256 SWA, 512 full}, backbone 1 KV head at the same head size -> GQA with {@code kvMul=4} over the
 * backbone's single-KV-head cache; {@link FlashAttention#flashDecode} with null batch buffers reads
 * exactly the cached window {@code [attStart, position]}.
 *
 ...
 * Q-only attn, wo, attn_post_norm, +res, ffn gelu-par, ffn_post_norm, +res, *out_scale);
 * {@code output_norm}; draft logits = tied {@code token_embd @ cur} (no softcap); {@code post_proj @
 * cur -> 1536} chains depth&gt;1.
 */
public final class Gemma4MtpDecoder {
    ...
    public Gemma4MtpDecoder(Gemma4Mtp mtp, Gemma4 backbone) {
        ...
        Pair<float[], float[]> swa = RoPE.precomputeFreqsCis(ctx, cfg.headSizeSWA(), cfg.ropeThetaSWA());
        Pair<float[], float[]> full = w.ropeFreqFactors != null
                ? RoPE.precomputeFreqsCisFromFreqs(ctx, cfg.headSizeFull(), cfg.ropeThetaFull(), w.ropeFreqFactors)
    ...
    public FloatTensor draft(Gemma4.State backboneState, FloatTensor hidden, long hiddenOffset, int token, int position) {
        ...
        for (int l = 0; l < cfg.numberOfLayers(); l++) {
            ...
            // attn: norm -> Q -> per-head q_norm + rope -> Q-only attention on backbone rings -> wo -> post_norm -> +res
            ...
            int kvMul = cfg.numberOfHeads() / backbone.config().numberOfKeyValueHeadsPerLayer()[kvSrc];
            int window = backbone.config().slidingWindow();
            int attStart = swa ? Math.max(0, position - window + 1) : 0;
            // draftState has no own KV: null batch buffers -> attend only backbone cache [attStart, position]
            FlashAttention.flashDecode(q, attn, backboneState.keyCache[kvSrc], backboneState.valueCache[kvSrc],
                    null, null, cfg.numberOfHeads(), position, attStart, headSize, bkvDim, kvMul,
                    1.0f, swa ? window - 1 : 0, null, decodeScratch);
        ...
        w.tokenEmbeddings.matmul(cur, draftLogits, cfg.vocabularySize(), dim);   // tied head, no softcap
        w.postProjection.matmul(cur, hNext, bd, dim);
        return draftLogits;
    }

    /** The 1536-dim backbone-space hidden produced by the last {@link #draft} call, to chain depth&gt;1. */
    public FloatTensor chainedHidden() { return hNext; }
}
