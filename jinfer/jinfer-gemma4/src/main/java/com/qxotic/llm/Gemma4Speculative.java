 ...
 * is rolled back with {@link com.qxotic.jinfer.RuntimeState#resumeAt} (stale KV rows are
 * overwritten by the next append, sound under the causal mask).
 *
 * <p>Every emitted token is the backbone's own greedy argmax read from a verified row, so the
 * output is token-identical to plain greedy BY CONSTRUCTION; the draft only decides how many
 * backbone forwards it takes. The verify batch always starts with the exact next token (known from
 * the previous row's logits), so each iteration commits at least one correct token.
 *
