...
public final class Gemma4VisionRun {
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 0 && args[0].equals("parity")) {
            parity(args[1]);
            return;
        }
        if (args.length > 0 && args[0].equals("uenc")) {
        ...
        System.out.printf(
                "tokens=%d dim=%d  encode=%.1fms  mean=%.6f meanAbs=%.6f min=%.4f max=%.4f%n",
                n, dim, ms, sum / (n * dim), abs / (n * dim), min, max);
        System.out.printf(
                "row0[0..7]= %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f%n",
                rows.getFloat(0),
                rows.getFloat(1),
                rows.getFloat(2),
                rows.getFloat(3),
    ...
    /**
     * Vision-encoder throughput (pp): warmup + timed reps over one image. Usage: bench <mmproj>
     * [image] [reps]
     */
    static void bench(String[] args) throws Exception {
        ...
        int dim = enc.modelDim,
                nTok = (int) (r0.size() / dim),
                nPatch = nTok * enc.merge * enc.merge;
        ...
        mean /= reps;
        double sd = 0;
        for (double v : t) sd += (v - mean) * (v - mean);
        sd = Math.sqrt(sd / Math.max(1, reps - 1));
        System.out.printf(
                "vision-encoder bench: %dx%d img -> %d patches -> %d tokens (dim %d), %d reps"
                        + " (warm)%n",
                img.width(), img.height(), nPatch, nTok, dim, reps);
        System.out.printf(
                "  %.1f ± %.1f ms/image  |  %.1f img/s  |  %.0f patches/s  |  %.0f out-tok/s%n",
                mean, sd, 1000.0 / mean, nPatch * 1000.0 / mean, nTok * 1000.0 / mean);
    }

    /**
     * image -> Gemma4Vision -> embedding rows -> ingest between <start_of_image>/<end_of_image> ->
     * generate.
     */
    static void e2e(String textGguf, String mmproj, String imagePath, String prompt)
        ...
        int bos = sp.getOrDefault("<bos>", 2),
                ...
                eot = sp.getOrDefault("<turn|>", -1); // gemma4 turn markers (NOT <start_of_turn>)
        int soi = sp.getOrDefault("<|image>", -1),
                eoi = sp.getOrDefault("<image|>", -1); // gemma4 open/close image wrappers
        System.err.printf(
                "modalities=%s  special: bos=%d sot=%d eot=%d soi=%d eoi=%d%n",
        ...
        System.err.printf(
                "image %dx%d -> %d vision tokens (dim=%d)%n", img.width(), img.height(), nImg, dim);
    ...
    static void uenc(String mmproj, Media.Image img) throws Exception {
        ...
        System.out.printf(
                "gemma4uv tokens=%d dim=%d  encode=%.1fms  mean=%.6f meanAbs=%.6f min=%.4f"
                        + " max=%.4f%n",
                n, dim, ms, sum / (n * dim), abs / (n * dim), min, max);
        System.out.printf(
                "row0[0..7]= %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f%n",
    ...
    /**
     * Decode the same image with both backends (ffmpeg + javax.imageio) and report the max abs
     * pixel difference - proves the two decoders produce identical Media.Image values.
     */
    static void parity(String path) throws Exception {
        ...
        System.out.printf(
                "ffmpeg: %dx%d c%d (%d values) | imageio: %dx%d c%d (%d values)%n",
                a.width(),
                a.height(),
                a.channels(),
                a.values().length,
        ...
        System.out.printf(
                "max abs pixel diff = %.6f  (differing values: %d / %d)%n",
                maxAbs, ndiff, a.values().length);
    }
