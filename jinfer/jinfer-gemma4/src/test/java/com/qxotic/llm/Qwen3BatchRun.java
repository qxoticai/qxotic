// Qwen3 packed batched-embedding: isolation invariant (packed == individual) + throughput vs one-by-one.
...
public final class Qwen3BatchRun {
    public static void main(String[] args) throws Exception {
        ...
        if (bench > 0) { benchmark(model, dim, vocab, eos, bench); return; }

        String promptsFile = System.getProperty("prompts");
        if (promptsFile != null) {   // batched-embed each line, print vectors (space-separated) for llama.cpp cross-check
            ...
            int tot = 0; for (int[] q : qs) tot += q.length;
            var st = model.newState(tot, tot);
            var pk = (Batch.Input.Sequences) Batch.pack(qs).input();
            StringBuilder sb = new StringBuilder();
            model.embed(st, pk, e -> {
                for (int i = 0; i < dim; i++) { if (i > 0) sb.append(' '); sb.append(e.getFloat(i)); }
                sb.append('\n');
            });
            System.out.print(sb);
            return;
        ...
        String[] texts = {
                ...
                "def add(a, b):\n    return a + b",
                "Photosynthesis converts light energy into chemical energy stored in glucose within plant cells.",
                "yes",
        ...
        int total = 0; for (int[] q : seqs) total += q.length;
        ...
        // each sequence embedded as its OWN 1-sequence pack (segmented path, same GEMM N as individual)
        ...
        System.out.printf("=== isolation invariant: packed (bc=%d, total=%d) vs individual ===%n", bc, total);
        double worst = 1.0, worstSolo = 1.0;
        for (int i = 0; i < seqs.length; i++) {
            double c = cosine(indiv[i], batched.get(i)), cs1 = cosine(indiv[i], solo[i]);
            worst = Math.min(worst, c); worstSolo = Math.min(worstSolo, cs1);
            System.out.printf("  seq %-2d (%3d tok): packed=%.6f  solo(same-N)=%.6f  %s%n",
    ...
    static void benchmark(Qwen3 model, int dim, int vocab, int eos, int nSeq) {
        int len = Integer.getInteger("len", 32), warm = Integer.getInteger("warm", 8), reps = Integer.getInteger("reps", 20);
        ...
        Runnable batched = () -> { model.embed(bs, packed, e -> {}); };
        // one-by-one: reuse one right-sized state, reset per seq
        var os = model.newState(len, len);
        Runnable onebyone = () -> { for (int[] q : seqs) { os.reset(); model.ingest(os, Batch.prefill(q)); model.embedding(os); } };

        double msB = timed(batched, warm, reps), msO = timed(onebyone, warm, reps);
        System.out.printf("=== batched embedding throughput (%d seqs x %d tok = %d tokens, warm JVM) ===%n", nSeq, len, total);
        System.out.printf("  batched:    %.2f ms  ->  %.1f emb/s   %.0f tok/s%n", msB, nSeq * 1000.0 / msB, total * 1000.0 / msB);
        System.out.printf("  one-by-one: %.2f ms  ->  %.1f emb/s   %.0f tok/s%n", msO, nSeq * 1000.0 / msO, total * 1000.0 / msO);
    ...
    static float[] toArray(FloatTensor t, int dim) { float[] a = new float[dim]; for (int i = 0; i < dim; i++) a[i] = t.getFloat(i); return a; }
