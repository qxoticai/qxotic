// Diff the new com.qxotic.llm.Mistral3 port against the production com.qxotic.jinfer Mistral3 on the same
// tokens: prefill both, compare the next-token argmax and the max per-logit difference, then 8 decode steps.
//   java ... com.qxotic.llm.Mistral3Compare <model.gguf> [prompt]
...
public final class Mistral3Compare {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        String prompt = args.length > 1 ? args[1] : "The capital of France is";

        Mistral3 nw = Mistral3.loadModel(Path.of(path), 4096);
        var tk = nw.tokenizer();
        int bos = bos(tk);
        ...
        Mistral3.State ns = nw.newState(4096, Math.max(16, ids.length));
