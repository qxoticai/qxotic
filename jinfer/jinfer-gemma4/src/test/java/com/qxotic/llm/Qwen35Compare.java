// Diff the new com.qxotic.llm.Qwen35 port against the production com.qxotic.jinfer Qwen35 on the same
// tokens: prefill both, compare the next-token argmax + max per-logit diff, then 8 decode steps.
//   java ... com.qxotic.llm.Qwen35Compare <model.gguf> [prompt]
...
public final class Qwen35Compare {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        String prompt = args.length > 1 ? args[1] : "The capital of France is";

        Qwen35 nw = Qwen35.loadModel(Path.of(path), 4096);
        var tk = nw.tokenizer();
        List<Integer> pt = new ArrayList<>(tk.encode(prompt));   // Qwen3.5: no BOS
        ...
        Qwen35.State ns = nw.newState(4096, Math.max(16, ids.length));
