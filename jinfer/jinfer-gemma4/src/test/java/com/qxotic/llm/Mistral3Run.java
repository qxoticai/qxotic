// Runnable smoke check for the standard-Mistral3 port: load a Mistral3-family GGUF and greedily decode through
// the com.qxotic.llm model API.   java ... com.qxotic.llm.Mistral3Run <model.gguf> [prompt] [nTokens]
// CHAT=1 wraps the prompt in the Mistral3-3 header chat format.
...
public final class Mistral3Run {
    public static void main(String[] args) throws Exception {
        ...
        Mistral3 model = Mistral3.loadModel(Path.of(path), 4096);
        var c = model.config();
        ...
        if (System.getenv("CHAT") != null) {   // Mistral3-3: <|start_header_id|>user<|end_header_id|>\n\n{p}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n
        ...
        Mistral3.State s = model.newState(c.contextLength(), Math.max(16, ids.length));
