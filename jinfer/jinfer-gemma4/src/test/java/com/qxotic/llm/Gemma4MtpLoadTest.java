// Stage 1 gate for the Gemma 4 MTP sidecar loader: load the real gemma4-assistant GGUF and assert the
// grounded geometry + that all 49 tensors are present at the expected shapes (req() throws otherwise).
...
public final class Gemma4MtpLoadTest {
    ...
    public static void main(String[] args) throws Exception {
        Path sidecar = Path.of(args.length > 0 ? args[0]
        ...
        check(java.util.Arrays.equals(c.isSWA(), new boolean[]{true, true, true, false}), "isSWA [T,T,T,F]");
        ...
        // All 49 tensors loaded at grounded shapes (req() already threw on any mismatch); spot-check sizes.
        ...
        check(w.wq[0].size() == 256L * 1024 && w.wq[3].size() == 256L * 2048, "wq per-layer widths");
        check(w.attnQNorm[0].size() == 256 && w.attnQNorm[3].size() == 512, "q_norm per-layer head size");
        check(w.layerOutputScales.length == 4, "4 layer output scales");
        check(w.ropeFreqFactors != null && w.ropeFreqFactors.length == 256, "rope_freqs 256 factors (full layer)");

        if (failures > 0) {
            System.out.println(failures + " failure(s)");
            System.exit(1);
        }
        System.out.println("Gemma4MtpLoadTest: all checks passed (sidecar loads, geometry + 49 tensors verified)");
    }
}
