// Diff the new com.qxotic.llm.Lfm2 port against the production com.qxotic.jinfer Llama (LFM2.5) on the
// same tokens: prefill both, compare the next-token argmax and the max per-logit difference.
//   java ... com.qxotic.llm.LfmCompare <model.gguf> [prompt]
...
public final class LfmCompare {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        String prompt = args.length > 1 ? args[1] : "The capital of France is";

        Lfm2 nw = Lfm2.loadModel(Path.of(path), 4096);
        var tk = nw.tokenizer();
        ...
        Lfm2.State ns = nw.newState(4096, Math.max(16, ids.length));
