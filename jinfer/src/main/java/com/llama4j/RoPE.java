...
final class RoPE {
     ...
     *  {@code cr}/{@code ci} are a {@link #precomputeFreqsCis}-family table at stride {@code ropeHalf}. */
    static void applyInterleaved(FloatTensor q, int headOffset, int position, float[] cr, float[] ci, int ropeHalf) {
    ...
    /** YaRN-scaled RoPE tables (cos/sin) in the same interleaved per-pair layout as
     *  {@link #precomputeFreqsCis} — the attention mscale is baked into cos/sin. Mirrors ggml's
     *  {@code rope_yarn} (theta interpolated/extrapolated by the correction ramp; in-kernel mscale =
     *  1 + 0.1·ln(1/freq_scale) when ext_factor != 0), times {@code attnFactor} — llama.cpp's
     *  {@code cparams.yarn_attn_factor}, the extra magnitude factor it folds onto the kernel mscale.
     *  Pass 1.0 for the plain YaRN magnitude (gpt-oss); pass 1/(kernel mscale) to net a magnitude of
     *  1.0 (mistral3, whose yarn_log_multiplier cancels the kernel mscale). */
    public static Pair<float[], float[]> precomputeFreqsCisYarn(int contextLength, int headSize, double ropeTheta,
                                                                float ropeScalingFactor, int originalContextLength,
                                                                float betaFast, float betaSlow, float extFactor, float attnFactor) {
        assert headSize % 2 == 0;
        int halfHead = headSize / 2;
        float[] cr = new float[contextLength * halfHead];
        float[] ci = new float[contextLength * halfHead];
        ...
        float mscale = attnFactor * (extFactor != 0f ? (float) (1.0 + 0.1 * Math.log(1.0 / Math.max(1e-12, freqScale))) : 1f);
        int n = 0;
        for (int pos = 0; pos < contextLength; pos++) {
            for (int i = 0; i < headSize; i += 2) {
