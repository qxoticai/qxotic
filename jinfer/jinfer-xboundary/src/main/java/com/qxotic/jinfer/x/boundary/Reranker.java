 ...
 * convention - they never leak into a provider integration, which only sees numbers. Tensor-free by
 * construction: strings in, doubles out; everything {@link com.qxotic.jota.memory.MemoryView} stays
 * inside the recipe.
 *
 * <p>Two shapes exist. A {@link CrossEncoder} judges each pair in a causal forward and reads a
 * verdict (Qwen3-Reranker) - it implements three template methods and inherits the
 ...
 * reuse is structurally impossible for it: the forward is bidirectional (frame KV attends to the
 * previous candidate), and short-conv state is a flat trailing window, not addressable by position,
 * so a cursor rewind cannot restore it.
 */
public interface Reranker<S extends RuntimeState> {
    ...
    interface CrossEncoder<S extends RuntimeState> extends Reranker<S> {
        ...
        /**
         * The candidate, plus the scaffold that closes the judge turn. Retain ONLY the verdict row
         * ({@link Batch.Outputs#LAST}): the template reads one row per candidate, and retaining ALL
         * rows pays hidden-state memory for rows nobody reads.
         */
         ...
         * expectation over grade digits; only the port knows which. May read through the CLAIMING
         * heads ({@code model().logits(state)}): {@link BaseState#enter()} is reentrant, so the
         * head's claim nests inside {@link #scoreAll}'s.
         */
