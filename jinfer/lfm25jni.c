...
typedef struct {
    int kind; // 0 = f32 gemm, 1 = gemv, 2 = quantize activations, 3 = vnni gemm
...
static void compute_vnni_band(const gemm_task_t *task, int tile, int worker) {
    ...
    if (rp->cap_blocks < kblocks) {
        ...
        rp->dw = (float *) aligned_alloc(64, (size_t) (VNNI_BAND / 16) * kblocks * 16 * sizeof(float));
        rp->cw = (float *) aligned_alloc(64, (size_t) (VNNI_BAND / 16) * kblocks * 16 * sizeof(float));
