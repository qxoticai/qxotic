...
public final class CacheScenario<S extends RuntimeState> {
    ...
    /**
     * Per-model knobs. Reply-text strictness is NOT here - it is the model's {@link
     * Harness#deterministicDecode} property, declared once with the harness.
     */
    ...
    public void run(String runName) {
        ...
        if (cfg.longCase() != null) {
            ...
            longBench =
                    validate(
                            longCache,
                            "long (" + longHist.length + " pos)",
                            longHist,
                            b.state(),
                            Message.user(cfg.longCase().probe()));
        }
        ...
        System.out.printf(
                "%-38s %12s %14s %10s%n", "history", "resume (ms)", "replay (ms)", "speedup");
        System.out.printf(
                "%-38s %12.1f %14.0f %9.0fx%n",
                "short (" + shortHist.length + " pos)",
                shortBench.resumeMs(),
                shortBench.replayMs(),
                shortBench.replayMs() / shortBench.resumeMs());
        if (longBench != null) {
            System.out.printf(
                    "%-38s %12.1f %14.0f %9.0fx%n",
                    "long (" + longHist.length + " pos)",
                    longBench.resumeMs(),
                    longBench.replayMs(),
                    longBench.replayMs() / longBench.resumeMs());
        }
        System.out.printf(
                "decode: %.1f tok/s%n", (longBench != null ? longBench : shortBench).tokPerSec());
    ...
    private Bench validate(
        ...
        h.check(
                ...
                name
                        ...
                        + ")");
        h.check(
                h.statesEqual(cached.state(), liveState, history.length),
        ...
        if (h.deterministicDecode) {
            h.check(
                    cachedReply.text().equals(plainReply),
                    name + ": cached and uncached greedy replies identical");
        } else {
            System.out.println(
                    "      (cached == uncached reply text: "
                            + cachedReply.text().equals(plainReply)
                            + " - informational; MoE decode is not byte-deterministic under"
                            + " threading)");
        }
        ...
        return new Bench(
                resumeMs,
                replayMs,
                cachedReply.text().isEmpty() ? 0 : cachedReply.tokens() / decodeSec);
    }
