// Shared Mixture-of-Experts dispatch. The router/gating (softmax/sigmoid, top-k, normalization) is
// a
// model's identity and stays per-architecture; this owns only the architecture-independent
// plumbing:
...
public final class Moe {
    ...
    public static void dispatch(
            ...
            FloatTensor input,
            FloatTensor gather,
            FloatTensor expertOut,
            FloatTensor out,
            FloatTensor expertScale,
            ExpertKernel kernel) {
        int[] off = r.offsets;
        off[0] = 0;
        ...
        for (int e = 0; e < r.numExperts; e++) {
            ...
            if (n == 0) continue;
            Parallel.forRows(
                    n, j -> input.copyTo(r.rowByExpert[start + j] * dim, gather, j * dim, dim));
            kernel.apply(e, n, gather, expertOut);
            Parallel.forRows(
                    n,
                    j ->
                            out.saxpyInPlace(
                                    r.rowByExpert[start + j] * dim,
                                    expertOut,
                                    j * dim,
                                    dim,
