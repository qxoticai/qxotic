...
final class Estimators implements TokenCountEstimator {
    ...
    private int countParts(List<com.qxotic.jinfer.x.chat.Content> parts) {
        int sum = 0;
        for (com.qxotic.jinfer.x.chat.Content part : parts) {
            sum +=
                    switch (part) {
                        case com.qxotic.jinfer.x.chat.Content.Text t ->
                                estimateTokenCountInText(t.text());
                        case com.qxotic.jinfer.x.chat.Content.Media b -> {
                        ...
                        case com.qxotic.jinfer.x.chat.Content.ToolCall c ->
                                estimateTokenCountInText(c.name())
                                        + estimateTokenCountInText(Json.stringify(c.arguments()));
                        case com.qxotic.jinfer.x.chat.Content.ToolResult r ->
                                estimateTokenCountInText(r.text());
                        case com.qxotic.jinfer.x.chat.Content.Reasoning ignored ->
