...
public final class SimpleLlama {
    ...
    private static final class TimingProfiler {
        ...
        private String renderReport(String label, Map<String, Long> nanosByKey, Map<String, Long> countByKey) {
            long totalNanos = 0L;
            long gemmNanos = 0L;
            ...
            nanosByKey.entrySet().stream()
                    ...
                    .forEach(entry -> {
                        ...
                        details.append("  ")
                                ...
                                .append(count)
                                .append(", avg=")
                                .append(String.format(java.util.Locale.ROOT, "%.3f us", avgUs))
                                .append(")\n");
                    });
            ...
            details.append("  gemm.total: ")
                    .append(String.format(java.util.Locale.ROOT, "%.3f ms", gemmNanos / 1_000_000.0))
                    .append("\n")
                    .append("  rest.total: ")
                    .append(String.format(java.util.Locale.ROOT, "%.3f ms", restNanos / 1_000_000.0))
                    .append("\n");
    ...
    public static void main(String[] args) throws Exception {
        ...
        Environment env = new Environment(Device.PANAMA, DataType.FP32, Environment.current().runtimes(), ExecutionMode.EAGER);
    ...
    private static void runBenchmark(Options options, LoadedModel loaded) throws IOException {
        ...
        int threadCount = Integer.getInteger("ai.qxotic.llama.kernel.threads", Runtime.getRuntime().availableProcessors());
        ...
        for (int run = 0; run < totalRuns; run++) {
            ...
            int[] decodeTokens = generateDeterministicTokens(options.benchTg, vocabSize, runSeed ^ 0x9E3779B9);
            ...
            System.out.printf("%s %d/%d: pp%d %.2f tok/s, tg%d %.2f tok/s%n",
                    phase,
                    run + 1,
                    totalRuns,
                    options.benchPp,
        ...
        System.out.println();
        System.out.printf("| %-30s | %10s | %8s | %7s | %16s | %22s |%n",
                "model",
                "size",
                "backend",
                "threads",
                "test",
                "t/s");
        System.out.printf("| %-30s | %10.2f | %8s | %7d | %16s | %10.2f ± %-10.2f |%n",
                "llama all F32",
                modelBytes / (1024.0 * 1024.0 * 1024.0),
                "CPU",
                threadCount,
    ...
    private static void runInstruct(Options options, LoadedModel loaded) {
        ...
        LlamaModel model = loaded.model;
        Sampler sampler = new Sampler(loaded.config.vocabularySize(), options.temperature, options.topP, options.seed);
        ...
        if (options.systemPrompt != null && !options.systemPrompt.isBlank()) {
            append(promptTokens, format.encodeMessage(new Message(Llama3ChatFormat.SYSTEM, options.systemPrompt)));
        }
        append(promptTokens, format.encodeMessage(new Message(Llama3ChatFormat.USER, options.prompt)));
        ...
        int decodeTimingBucket = Math.max(0, Integer.getInteger("ai.qxotic.llama.decode.timing.bucket", 0));
        TimingProfiler.Snapshot bucketSnapshot = decodeTimingBucket > 0 ? model.timingSnapshot() : null;
        ...
        for (int i = 0; i < options.maxTokens; i++) {
            ...
            if (decodeTimingBucket > 0 && tokenCount % decodeTimingBucket == 0) {
                ...
                System.out.printf(
                        ...
                        bucketTps,
                        bucketTokens);
                String bucketReport = model.timingReportDelta(
        ...
        System.out.printf("context: %d/%d prompt: %.2f tokens/s (%d) generation: %.2f tokens/s (%d)%n",
                state.position, loaded.config.contextLength(),
                promptTps, promptArray.length,
                genTps, tokenCount);
    ...
    private static void runInteractive(Options options, LoadedModel loaded) {
        ...
        if (options.systemPrompt != null && !options.systemPrompt.isBlank()) {
            append(conversation, format.encodeMessage(new Message(Llama3ChatFormat.SYSTEM, options.systemPrompt)));
        }
        ...
        while (true) {
            ...
            long promptElapsedNs = System.nanoTime() - promptStartNs;
            double promptTps = newTokens.length > 0 ? newTokens.length / (promptElapsedNs / 1_000_000_000.0) : 0.0;
            String promptTimingReport = model.timingReport("prompt");
    ...
    private static final class LlamaState {
        ...
        LlamaState(LlamaConfig cfg, MemoryDomain<?> domain) {
            ...
            for (int i = 0; i < cfg.nLayers(); i++) {
                this.keyCache[i] =
                        MemoryView.of(
                                domain.memoryAllocator().allocateMemory(DataType.FP32, Shape.of(cfg.contextLength(), kvDim)),
                                DataType.FP32,
                                Layout.rowMajor(Shape.of(cfg.contextLength(), kvDim)));
                this.valueCache[i] =
                        MemoryView.of(
                                domain.memoryAllocator().allocateMemory(DataType.FP32, Shape.of(kvDim, cfg.contextLength())),
                                DataType.FP32,
    ...
    private static final class DecodeScratch {
        ...
        DecodeScratch(LlamaConfig cfg, MemoryDomain<?> domain) {
            int kvDim = cfg.nKvHeads() * cfg.headDim();
            int maxBatch = Math.max(1, Integer.getInteger("ai.qxotic.llama.prefill.batch", Integer.getInteger("ai.qxotic.llama.batch.size", 512)));
            ...
            this.xBuf = MemoryView.of(scratchMemory, offX, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.xTmpBuf = MemoryView.of(scratchMemory, offXTmp, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.xbBuf = MemoryView.of(scratchMemory, offXb, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.ffInBuf = MemoryView.of(scratchMemory, offFfIn, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.qPreBuf = MemoryView.of(scratchMemory, offQPre, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.kPreBuf = MemoryView.of(scratchMemory, offKPre, DataType.FP32, Layout.rowMajor(Shape.of(1, kvDim)));
            this.vPreBuf = MemoryView.of(scratchMemory, offVPre, DataType.FP32, Layout.rowMajor(Shape.of(1, kvDim)));
            this.qRotBuf = MemoryView.of(scratchMemory, offQRot, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.kRotBuf = MemoryView.of(scratchMemory, offKRot, DataType.FP32, Layout.rowMajor(Shape.of(1, kvDim)));
            this.attnScoresBuf = MemoryView.of(scratchMemory, offAttnScores, DataType.FP32, Layout.rowMajor(Shape.of(cfg.contextLength())));
            this.attentionOutBuf = MemoryView.of(scratchMemory, offAttnOut, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.nHeads() * cfg.headDim())));
            this.ffnGateBuf = MemoryView.of(scratchMemory, offFfnGate, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.ffnDim())));
            this.ffnUpBuf = MemoryView.of(scratchMemory, offFfnUp, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.ffnDim())));
            this.ffnHiddenBuf = MemoryView.of(scratchMemory, offFfnHidden, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.ffnDim())));
            this.ffnDownBuf = MemoryView.of(scratchMemory, offFfnDown, DataType.FP32, Layout.rowMajor(Shape.of(1, cfg.dim())));
            this.logitsBuf = MemoryView.of(scratchMemory, offLogits, DataType.FP32, Layout.rowMajor(Shape.of(cfg.vocabularySize())));
            // Batch buffers
            this.batchXBuf = MemoryView.of(scratchMemory, offBatchX, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
            this.batchXTmpBuf = MemoryView.of(scratchMemory, offBatchXTmp, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
            this.batchXbBuf = MemoryView.of(scratchMemory, offBatchXb, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
            this.batchQPreBuf = MemoryView.of(scratchMemory, offBatchQPre, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
            this.batchKPreBuf = MemoryView.of(scratchMemory, offBatchKPre, DataType.FP32, Layout.rowMajor(Shape.of(kvDim, maxBatch)));
            this.batchVPreBuf = MemoryView.of(scratchMemory, offBatchVPre, DataType.FP32, Layout.rowMajor(Shape.of(kvDim, maxBatch)));
            this.batchFFInBuf = MemoryView.of(scratchMemory, offBatchFFIn, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
            this.batchFfnGateBuf = MemoryView.of(scratchMemory, offBatchFfnGate, DataType.FP32, Layout.rowMajor(Shape.of(cfg.ffnDim(), maxBatch)));
            this.batchFfnUpBuf = MemoryView.of(scratchMemory, offBatchFfnUp, DataType.FP32, Layout.rowMajor(Shape.of(cfg.ffnDim(), maxBatch)));
            this.batchFfnHiddenBuf = MemoryView.of(scratchMemory, offBatchFfnHidden, DataType.FP32, Layout.rowMajor(Shape.of(cfg.ffnDim(), maxBatch)));
            this.batchFfnDownBuf = MemoryView.of(scratchMemory, offBatchFfnDown, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
            this.batchAttnOutBuf = MemoryView.of(scratchMemory, offBatchAttnOut, DataType.FP32, Layout.rowMajor(Shape.of(cfg.dim(), maxBatch)));
    ...
    private static final class RopeTables {
        ...
        static RopeTables precompute(int ctx, int headDim, float theta, float[] ropeScales, int maxHeads) {
            int half = headDim / 2;
    ...
    private static final class LlamaModel {
        ...
        private static final long PARALLEL_MIN_WORK = 4096L;  // Lowered threshold for better parallelization
        private static final int THREADS = Math.max(1, Integer.getInteger("ai.qxotic.llama.kernel.threads", Runtime.getRuntime().availableProcessors()));
        private static final int GEMV_THREADS = Math.max(1, Integer.getInteger("ai.qxotic.llama.gemv.threads", Math.min(THREADS, 8)));
        private static final int GEMV_PARALLEL_MIN_ROWS =
                Math.max(1, Integer.getInteger("ai.qxotic.llama.gemv.parallel.minRows", Math.max(64, GEMV_THREADS * 8)));
        private static final int LEGACY_BATCH_SIZE = Integer.getInteger("ai.qxotic.llama.batch.size", 512);
        private static final int PREFILL_BATCH_SIZE = Integer.getInteger("ai.qxotic.llama.prefill.batch", LEGACY_BATCH_SIZE);
        private static final int PREFILL_UBATCH_SIZE =
                Integer.getInteger("ai.qxotic.llama.prefill.ubatch", Math.min(128, PREFILL_BATCH_SIZE));
        private static final boolean USE_JAVA_WEIGHT_STATIONARY_GEMM =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.javawsgemm", "false"));
        private static final int JAVA_WS_TILE_BATCH =
                Integer.getInteger("ai.qxotic.llama.javawsgemm.tileBatch", 64);
        private static final int JAVA_WS_TILE_M =
                Integer.getInteger("ai.qxotic.llama.javawsgemm.tileM", 32);
        private static final int JAVA_WS_TILE_N =
                Integer.getInteger("ai.qxotic.llama.javawsgemm.tileN", 256);
        private static final boolean USE_C_BATCH_GEMM =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.cgemm", "true"));
        private static final boolean ENABLE_TIMINGS =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.timing", "true"));
        private static final boolean ATTN_VECTOR_SOFTMAX =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.attention.softmax.vectorized", "false"));
        private static final boolean SWIGLU_VECTOR =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.swiglu.vectorized", "false"));
        private static final boolean ATTN_DECODE_TWO_PASS =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.attention.decode.twopass", "false"));
        private static final boolean ATTN_DECODE_BLOCKED =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.attention.decode.blocked", "true"));
        private static final int ATTN_DECODE_BLOCK_SIZE =
                Math.max(16, Integer.getInteger("ai.qxotic.llama.attention.decode.blockSize", 128));
        private static final boolean USE_PERSISTENT_PARALLEL =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.parallel.pool", "false"));
        private static final long C_BATCH_GEMM_MIN_WORK =
                Long.getLong("ai.qxotic.llama.cgemm.minWork", 262_144L);
        private static final int C_BATCH_GEMM_OMP_MIN_WORK =
                Integer.getInteger("ai.qxotic.llama.cgemm.omp.minWork", 16_384);
        private static final int C_BATCH_GEMM_OMP_CHUNK =
                Integer.getInteger("ai.qxotic.llama.cgemm.omp.chunk", 2);
        private static final int C_BATCH_GEMM_TILE_M =
                Integer.getInteger("ai.qxotic.llama.cgemm.tileM", 32);
        private static final int C_BATCH_GEMM_PREFETCH_FLOATS =
                Integer.getInteger("ai.qxotic.llama.cgemm.prefetch", 128);
        private static final boolean C_BATCH_GEMM_USE_BLAS =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.cgemm.blas", "true"));
        private static final boolean C_BATCH_GEMM_BLAS_REQUIRED =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.cgemm.blas.required", "false"));
        private static final long C_BATCH_GEMM_BLAS_MIN_WORK =
                Long.getLong("ai.qxotic.llama.cgemm.blas.minWork", 65_536L);
        private static final boolean C_GEMV_USE_BLAS =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.cgemv.blas", "false"));
        private static final boolean C_GEMV_BLAS_REQUIRED =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.cgemv.blas.required", "false"));
        private static final long C_GEMV_BLAS_MIN_WORK =
                Long.getLong("ai.qxotic.llama.cgemv.blas.minWork", 16_384L);
        ...
        private static final String C_GEMV_KERNEL_SOURCE =
                buildCGemvKernelSource(C_GEMV_USE_BLAS, C_GEMV_BLAS_REQUIRED, C_GEMV_BLAS_MIN_WORK, C_GEMV_KERNEL_NAME);
        ...
        private static final java.util.concurrent.ExecutorService EXEC = java.util.concurrent.Executors.newFixedThreadPool(THREADS, r -> {
        ...
        private final class DecodeEngine {
            ...
            private void attentionLayerStep(LlamaState state, DecodeScratch scratch, int layer, LayerPlan plan, int kvDim) {
                ...
                try (TimingProfiler.Scope ignored = timings.scope("decode.rope")) {
                    q = applyRoPETensor(scratch.qPreTensor, cfg.nHeads(), state.position, w.rope(), scratch.qRotBuf, scratch.qRotTensor);
                    k = applyRoPETensor(scratch.kPreTensor, cfg.nKvHeads(), state.position, w.rope(), scratch.kRotBuf, scratch.kRotTensor);
                }
                ...
                try (TimingProfiler.Scope ignored = timings.scope("decode.proj.wo")) {
                    projectInto(scratch.attentionOutBuf, plan.wo, scratch.ffnDownBuf, cfg.dim(), cfg.dim());
                }
            ...
            private void ffnLayerStep(DecodeScratch scratch, LayerPlan plan) {
                ...
                try (TimingProfiler.Scope ignored = timings.scope("decode.ffn.down")) {
                    projectDownInto(scratch.ffnHiddenBuf, plan.wDown, scratch.ffnDownBuf, cfg.dim(), cfg.ffnDim());
                }
        ...
        private final class PrefillEngine {
            void ingestTokens(int[] tokens, LlamaState state) {
                ...
                for (int chunkStart = 0; chunkStart < tokens.length; chunkStart += prefillBatch) {
                    ...
                    for (int ubatchStart = 0; ubatchStart < chunkSize; ubatchStart += prefillUbatch) {
        ...
        static {
            System.err.println("[LlamaModel] Using " + THREADS + " threads, prefill batch: " + PREFILL_BATCH_SIZE +
                             ", prefill ubatch: " + PREFILL_UBATCH_SIZE +
                             ", cgemm mode: " + (C_BATCH_GEMM_BLAS_REQUIRED ? "BLAS_REQUIRED" : "BEST_EFFORT") +
                              ", vector species: " + SPECIES.length() + "x" + SPECIES.elementType().getSimpleName());
        }
        ...
        private static KernelExecutable registerCBatchGemmKernel(DeviceRuntime runtime) {
            ...
            try {
                return runtime
                        .loadRegisteredExecutable(C_BATCH_GEMM_KERNEL_NAME)
                        .orElseGet(
                                () ->
                ...
                if (C_BATCH_GEMM_BLAS_REQUIRED) {
                    throw new IllegalStateException(
                            "BLAS-required mode enabled but C GEMM kernel registration failed",
                            ex);
                }
                return null;
            }
        ...
        private static KernelExecutable registerCGemvKernel(DeviceRuntime runtime) {
            ...
            try {
                return runtime
                        .loadRegisteredExecutable(C_GEMV_KERNEL_NAME)
                        .orElseGet(
                                () ->
                                        runtime.registerKernel(
                                                C_GEMV_KERNEL_NAME,
                ...
                if (C_GEMV_BLAS_REQUIRED) {
                    throw new IllegalStateException(
                            "BLAS-required mode enabled but C GEMV kernel registration failed",
                            ex);
                }
                return null;
            }
        ...
        private void ingestTokensBatched(int[] tokens, int tokenOffset, int batchSize, LlamaState state) {
            DecodeScratch scratch = state.scratch;
            int kvDim = cfg.nKvHeads() * cfg.headDim();
            ...
            try (TimingProfiler.Scope ignored = timings.scope("op.embedding.batch")) {
                parallelForBatch(batchSize, i -> {
                    loadEmbeddingIntoBatchBuffer(tokens[tokenOffset + i], scratch.batchXBuf, i);
                });
            }
            ...
            for (int layer = 0; layer < cfg.nLayers(); layer++) {
                ...
                try (TimingProfiler.Scope ignored = timings.scope("gemm.q")) {
                    batchGemm(scratch.batchXbBuf, plan.wq, scratch.batchQPreBuf, batchSize, cfg.dim(), cfg.dim());
                }
                try (TimingProfiler.Scope ignored = timings.scope("gemm.k")) {
                    batchGemm(scratch.batchXbBuf, plan.wk, scratch.batchKPreBuf, batchSize, kvDim, cfg.dim());
                }
                try (TimingProfiler.Scope ignored = timings.scope("gemm.v")) {
                    batchGemm(scratch.batchXbBuf, plan.wv, scratch.batchVPreBuf, batchSize, kvDim, cfg.dim());
                }
                ...
                try (TimingProfiler.Scope ignored = timings.scope("op.rope.kv")) {
                    parallelForBatch(batchSize, i -> {
                        ...
                        applyRoPEInPlace(scratch.batchQPreBuf, i, cfg.nHeads(), position, w.rope(), cfg.dim());
                        applyRoPEInPlace(scratch.batchKPreBuf, i, cfg.nKvHeads(), position, w.rope(), kvDim);

                        // Write to KV cache directly from batch buffer
                        writeKeyValueCacheFromBatch(state, currentLayer, kvDim, scratch.batchKPreBuf, scratch.batchVPreBuf, i, position);
                    });
                }
                ...
                try (TimingProfiler.Scope ignored = timings.scope("gemm.wo")) {
                    batchGemm(scratch.batchAttnOutBuf, plan.wo, scratch.batchFfnDownBuf, batchSize, cfg.dim(), cfg.dim());
                }

                // 2f: Add residual in parallel
                try (TimingProfiler.Scope ignored = timings.scope("op.residual.attn")) {
                    parallelForBatch(batchSize, i -> {
                        addIntoBatch(scratch.batchXBuf, scratch.batchFfnDownBuf, scratch.batchXTmpBuf, i, cfg.dim());
                    });
                }
                ...
                try (TimingProfiler.Scope ignored = timings.scope("op.rmsnorm.ffn")) {
                    rmsNormBatch(scratch.batchXTmpBuf, plan.ffnNorm, scratch.batchFFInBuf, batchSize);
                }

                // 2h: Batch FFN projections (Gate and Up)
                try (TimingProfiler.Scope ignored = timings.scope("gemm.gate")) {
                    batchGemm(scratch.batchFFInBuf, plan.wGate, scratch.batchFfnGateBuf, batchSize, cfg.ffnDim(), cfg.dim());
                }
                try (TimingProfiler.Scope ignored = timings.scope("gemm.up")) {
                    batchGemm(scratch.batchFFInBuf, plan.wUp, scratch.batchFfnUpBuf, batchSize, cfg.ffnDim(), cfg.dim());
                }

                // 2i: SwiGLU activation on batch
                try (TimingProfiler.Scope ignored = timings.scope("op.swiglu")) {
                    swigluBatch(scratch.batchFfnGateBuf, scratch.batchFfnUpBuf, scratch.batchFfnHiddenBuf, batchSize, cfg.ffnDim());
                }

                // 2j: Batch FFN down projection
                try (TimingProfiler.Scope ignored = timings.scope("gemm.down")) {
                    batchGemm(scratch.batchFfnHiddenBuf, plan.wDown, scratch.batchFfnDownBuf, batchSize, cfg.dim(), cfg.ffnDim());
                }

                // 2k: Add residual in parallel
                try (TimingProfiler.Scope ignored = timings.scope("op.residual.ffn")) {
                    parallelForBatch(batchSize, i -> {
                        addIntoBatch(scratch.batchXTmpBuf, scratch.batchFfnDownBuf, scratch.batchXBuf, i, cfg.dim());
                    });
                }
            }
        ...
        private void loadEmbeddingIntoBatchBuffer(int token, MemoryView<?> batchBuf, int index) {
            ...
            MemoryAccess access = Environment.current().runtimeFor(tokenTable.memory().device()).memoryDomain().directAccess();
            int dim = cfg.dim();
            ...
            for (int i = 0; i < dim; i++) {
                float v = access.readFloat(tokenTable.memory(), tokenOffset + (long) i * Float.BYTES);
                long batchOff = batchBase + ((long) i * batchStride + index) * Float.BYTES;
                access.writeFloat(batchBuf.memory(), batchOff, v);
            }
        }

        private void copyFromBatch(MemoryView<?> batchBuf, int index, MemoryView<?> singleBuf, int n) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(batchBuf.memory().device()).memoryDomain().directAccess();
            int batchStride = PREFILL_BATCH_SIZE;
            long batchBase = batchBuf.byteOffset();
            long singleOffset = singleBuf.byteOffset();
            for (int i = 0; i < n; i++) {
                float v = access.readFloat(batchBuf.memory(), batchBase + ((long) i * batchStride + index) * Float.BYTES);
                access.writeFloat(singleBuf.memory(), singleOffset + (long) i * Float.BYTES, v);
            }
        }

        private void copyToBatch(MemoryView<?> singleBuf, int index, MemoryView<?> batchBuf, int n) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(singleBuf.memory().device()).memoryDomain().directAccess();
            long singleOffset = singleBuf.byteOffset();
            long batchOffset = batchBuf.byteOffset() + (long) index * n * Float.BYTES;
            for (int i = 0; i < n; i++) {
                float v = access.readFloat(singleBuf.memory(), singleOffset + (long) i * Float.BYTES);
                access.writeFloat(batchBuf.memory(), batchOffset + (long) i * Float.BYTES, v);
            }
        }

        private void rmsNormBatch(MemoryView<?> input, Tensor weight, MemoryView<?> output, int batchSize) {
            int dim = cfg.dim();
        ...
        private void rmsNormBatchElement(MemoryView<?> input, Tensor weight, MemoryView<?> output, int batchIndex, int dim) {
            MemoryView<?> wv = weight.materialize();
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(input.memory().device()).memoryDomain().directAccess();
            int batchStride = PREFILL_BATCH_SIZE;
        ...
        private void swigluBatch(MemoryView<?> gateBuf, MemoryView<?> upBuf, MemoryView<?> outBuf, int batchSize, int ffnDim) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(gateBuf.memory().device()).memoryDomain().directAccess();
        ...
        private void batchGemm(MemoryView<?> A, MemoryView<?> B, MemoryView<?> C, int batchSize, int m, int n) {
            // Prefill convention: X is column-batch [n, batchSize], W is [m, n], output is [m, batchSize].
            // This computes Y = W * X.

            if (C_BATCH_GEMM_BLAS_REQUIRED && (cBatchGemmKernel == null || cStream == null)) {
                throw new IllegalStateException("BLAS-required mode enabled but C GEMM kernel is unavailable");
            }
            ...
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(A.memory().device()).memoryDomain().directAccess();
            ...
            parallelForBatch(batchSize, b -> {
                ...
                for (int row = 0; row < m; row++) {
                    ...
                    for (; col < unrollBound; col += vecLen * 4) {
                        ...
                        FloatVector bv1 = FloatVector.fromMemorySegment(SPECIES, bSeg, bOff1, ByteOrder.nativeOrder());
                        acc1 = av1.fma(bv1, acc1);

                        long aOff2 = aOff1 + (long) vecLen * Float.BYTES;
                        long bOff2 = bOff1 + (long) vecLen * Float.BYTES;
                        FloatVector av2 = FloatVector.fromMemorySegment(SPECIES, aSeg, aOff2, ByteOrder.nativeOrder());
                        FloatVector bv2 = FloatVector.fromMemorySegment(SPECIES, bSeg, bOff2, ByteOrder.nativeOrder());
                        acc2 = av2.fma(bv2, acc2);

                        long aOff3 = aOff2 + (long) vecLen * Float.BYTES;
                        long bOff3 = bOff2 + (long) vecLen * Float.BYTES;
                        FloatVector av3 = FloatVector.fromMemorySegment(SPECIES, aSeg, aOff3, ByteOrder.nativeOrder());
                        FloatVector bv3 = FloatVector.fromMemorySegment(SPECIES, bSeg, bOff3, ByteOrder.nativeOrder());
                        acc3 = av3.fma(bv3, acc3);

                        long aOff4 = aOff3 + (long) vecLen * Float.BYTES;
                        long bOff4 = bOff3 + (long) vecLen * Float.BYTES;
                        FloatVector av4 = FloatVector.fromMemorySegment(SPECIES, aSeg, aOff4, ByteOrder.nativeOrder());
                        FloatVector bv4 = FloatVector.fromMemorySegment(SPECIES, bSeg, bOff4, ByteOrder.nativeOrder());
                    ...
                    for (; col < vecBound; col += vecLen) {
                        ...
                        FloatVector bv = FloatVector.fromMemorySegment(SPECIES, bSeg, bOff, ByteOrder.nativeOrder());
                        acc1 = av.fma(bv, acc1);
                    }
                    ...
                    for (; col < n; col++) {
                        float av = access.readFloat(A.memory(), aRowBase + (long) col * Float.BYTES);
                        float bv = access.readFloat(B.memory(), bRowBase + (long) col * Float.BYTES);
                        dot += av * bv;
                    }
                            ...
                            for (int k = kStart; k < kEnd; k++) {
                                ...
                                for (; b < vecBatchBound; b += vecLen) {
                                    ...
                                    for (int rowLocal = 0; rowLocal < mCount; rowLocal++) {
                                        ...
                                        FloatVector accVec = FloatVector.fromArray(SPECIES, acc, accBase);
                                        FloatVector wVec = FloatVector.broadcast(SPECIES, wVals[rowLocal]);
        ...
        private void batchGemmTiled(
                MemoryView<?> A, MemoryView<?> B, MemoryView<?> C,
                MemorySegment aSeg, MemorySegment bSeg, MemorySegment cSeg,
                long aBase, long bBase, long cBase,
                MemoryAccess access,
                int batchSize, int m, int n,
                int bStart, int bEnd,
                int tileM, int tileN, int tileB) {
        ...
        private void parallelForBatched(int batchSize, int rows, java.util.function.BiConsumer<Integer, Integer> body) {
            ...
            if (USE_PERSISTENT_PARALLEL) {
                PARALLEL_POOL.forRange(total, 16, idx -> {
                    int b = idx / rows;
                    int r = idx % rows;
                    body.accept(b, r);
                });
            ...
            java.util.List<java.util.concurrent.Future<?>> futures = new java.util.ArrayList<>(workers);
        ...
        private void applyRoPEInPlace(MemoryView<?> batchBuf, int batchIndex, int nHeads, int position, RopeTables rope, int dim) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(batchBuf.memory().device()).memoryDomain().directAccess();
            ...
            for (int head = 0; head < nHeads; head++) {
                ...
                for (int i = 0; i < half; i++) {
                    int even = base + (i << 1);
                    int odd = even + 1;
                    long evenOff = baseOffset + ((long) even * batchStride + batchIndex) * Float.BYTES;
                    long oddOff = baseOffset + ((long) odd * batchStride + batchIndex) * Float.BYTES;
        ...
        private void writeKeyValueCacheFromBatch(LlamaState state, int layer, int kvDim,
                                                  MemoryView<?> batchKBuf, MemoryView<?> batchVBuf,
                                                  int batchIndex, int position) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(batchKBuf.memory().device()).memoryDomain().directAccess();
            ...
            for (int i = 0; i < kvDim; i++) {
                float kVal = access.readFloat(batchKBuf.memory(), kBatchBase + ((long) i * batchStride + batchIndex) * Float.BYTES);
                float vVal = access.readFloat(batchVBuf.memory(), vBatchBase + ((long) i * batchStride + batchIndex) * Float.BYTES);

                access.writeFloat(keyCacheView.memory(), keyCacheBase + keyRowOffset + (long) i * Float.BYTES, kVal);
                access.writeFloat(valueCacheView.memory(), valueCacheBase + (long) i * cfg.contextLength() * Float.BYTES + (long) position * Float.BYTES, vVal);
            }
        }

        private void batchAttention(LlamaState state, int layer, DecodeScratch scratch, int batchSize) {
            ...
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(qBuf.memory().device()).memoryDomain().directAccess();
            ...
            parallelForBatched(batchSize, cfg.nHeads(), (b, head) -> {
                ...
                long qHeadBase = qBase + ((long) headOffset * batchStride + b) * Float.BYTES;
                long outHeadBase = outBase + ((long) headOffset * batchStride + b) * Float.BYTES;
                ...
                for (int d = 0; d < headDim; d++) {
                    qHead[d] = access.readFloat(qBuf.memory(), qHeadBase + (long) d * batchStride * Float.BYTES);
                }

                // Compute attention scores: Q @ K^T
                float maxScore = Float.NEGATIVE_INFINITY;
                for (int t = 0; t < length; t++) {
                    long kHeadBase = keyBase + (long) t * keyRowStrideBytes + (long) kvHeadOffset * Float.BYTES;
                    ...
                    for (; d < headDim; d++) {
                        float kv = access.readFloat(keyCache.memory(), kHeadBase + (long) d * Float.BYTES);
                        dot += qHead[d] * kv;
                    }
                ...
                if (ATTN_VECTOR_SOFTMAX) {
                    ...
                    for (; t < vecBoundLen; t += vecLen) {
                        FloatVector sVec = FloatVector.fromArray(SPECIES, scores, t);
                        FloatVector eVec = sVec.sub(maxScoreVec).lanewise(VectorOperators.EXP);
                ...
                for (int d = 0; d < headDim; d++) {
                    long valueRowBase = valueBase + (long) (kvHeadOffset + d) * valueRowStrideBytes;
                    ...
                    for (; t < length; t++) {
                        float p = scores[t];
                        float vv = access.readFloat(valueCache.memory(), valueRowBase + (long) t * Float.BYTES);
                        accVal += p * vv;
                    }
                    access.writeFloat(outBuf.memory(), outHeadBase + (long) d * batchStride * Float.BYTES, accVal);
                }
            });
        }

        private void addIntoBatch(MemoryView<?> aBatch, MemoryView<?> bBatch, MemoryView<?> outBatch, int batchIndex, int n) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(aBatch.memory().device()).memoryDomain().directAccess();
        ...
        private void loadEmbeddingIntoScratch(int token, DecodeScratch scratch) {
            ...
            for (int i = 0; i < dim; i++) {
                float v = access.readFloat(tokenTable.memory(), tokenOffset + (long) i * Float.BYTES);
                access.writeFloat(scratch.xBuf.memory(), scratch.xBuf.byteOffset() + (long) i * Float.BYTES, v);
            }
        }
        ...
        private void rmsNormInto(Tensor x, Tensor weight, MemoryView<?> out) {
            ...
            MemoryView<?> wv = weight.materialize();
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(xv.memory().device()).memoryDomain().directAccess();
            int dim = cfg.dim();
            ...
            for (int i = 0; i < dim; i++) {
                ...
                float wVal = access.readFloat(wv.memory(), wBase + (long) i * Float.BYTES);
                access.writeFloat(out.memory(), outBase + (long) i * Float.BYTES, xVal * invRms * wVal);
            }
        }

        private void projectInto(LlamaState state, Tensor x, Tensor weight, MemoryView<?> outBuffer) {
        ...
        private void projectQkvInto(LlamaState state, Tensor x, Tensor wq, Tensor wk, Tensor wv, DecodeScratch scratch) {
            MemoryView<?> xView = x.materialize();
        ...
        private void projectInto(MemoryView<?> xView, MemoryView<?> weightView, MemoryView<?> outBuffer, int m, int n) {
            gemv(weightView, xView, outBuffer, m, n);
        }

        private void projectDownInto(LlamaState state, Tensor x, Tensor wDown, MemoryView<?> outBuffer) {
        ...
        private void projectDownInto(MemoryView<?> xView, MemoryView<?> wDownView, MemoryView<?> outBuffer, int m, int n) {
            gemv(wDownView, xView, outBuffer, m, n);
        }

        private void projectPairSwigluInto(LlamaState state, Tensor x, Tensor wGate, Tensor wUp, DecodeScratch scratch) {
            MemoryView<?> xView = x.materialize();
        ...
        private void projectPairSwigluInto(MemoryView<?> xView, LayerPlan plan, DecodeScratch scratch) {
            int m = cfg.ffnDim();
            int n = cfg.dim();
        ...
        private void applySwigluFromScratch(DecodeScratch scratch, int m) {
            ...
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(scratch.ffnGateBuf.memory().device()).memoryDomain().directAccess();
            ...
            MemorySegment gateSeg = ((MemoryView<MemorySegment>) scratch.ffnGateBuf).memory().base();
            MemorySegment upSeg = ((MemoryView<MemorySegment>) scratch.ffnUpBuf).memory().base();
            MemorySegment outSeg = ((MemoryView<MemorySegment>) scratch.ffnHiddenBuf).memory().base();
            ...
            for (; i < upper; i += SPECIES.length()) {
                ...
                FloatVector gate = FloatVector.fromMemorySegment(SPECIES, gateSeg, gateOff, ByteOrder.nativeOrder());
                FloatVector up = FloatVector.fromMemorySegment(SPECIES, upSeg, upOff, ByteOrder.nativeOrder());
            ...
            for (; i < m; i++) {
                float gate = access.readFloat(scratch.ffnGateBuf.memory(), gateBase + (long) i * Float.BYTES);
                float up = access.readFloat(scratch.ffnUpBuf.memory(), upBase + (long) i * Float.BYTES);
                float silu = gate / (1.0f + (float) Math.exp(-gate));
                access.writeFloat(scratch.ffnHiddenBuf.memory(), outBase + (long) i * Float.BYTES, silu * up);
            }
        }
        ...
        private void gemv(MemoryView<?> A, MemoryView<?> x, MemoryView<?> y, int m, int n) {
            ...
            if (C_GEMV_BLAS_REQUIRED && (cGemvKernel == null || cStream == null)) {
                throw new IllegalStateException("BLAS-required mode enabled but C GEMV kernel is unavailable");
            }
            if (cGemvKernel != null && cStream != null && shouldUseCGemv(m, n)) {
                cGemvKernel.launch(
                        LaunchConfig.auto(),
                        KernelArgs.fromVarargs(A, x, y, m, n),
                        cStream);
                return;
            }
            ...
            if (work < PARALLEL_MIN_WORK || m < GEMV_PARALLEL_MIN_ROWS) {
                ...
                for (int row = 0; row < m; row++) {
                    ...
                    for (; col < unrollBound; col += vecLen * 4) {
                        ...
                        FloatVector xv2 = FloatVector.fromMemorySegment(SPECIES, xSeg, xOff2, ByteOrder.nativeOrder());
                        FloatVector av2 = FloatVector.fromMemorySegment(SPECIES, aSeg, aOff2, ByteOrder.nativeOrder());
                        acc2 = av2.fma(xv2, acc2);

                        long aOff3 = aOff2 + (long) vecLen * Float.BYTES;
                        long xOff3 = xOff2 + (long) vecLen * Float.BYTES;
                        FloatVector xv3 = FloatVector.fromMemorySegment(SPECIES, xSeg, xOff3, ByteOrder.nativeOrder());
                        FloatVector av3 = FloatVector.fromMemorySegment(SPECIES, aSeg, aOff3, ByteOrder.nativeOrder());
                        acc3 = av3.fma(xv3, acc3);

                        long aOff4 = aOff3 + (long) vecLen * Float.BYTES;
                        long xOff4 = xOff3 + (long) vecLen * Float.BYTES;
                        FloatVector xv4 = FloatVector.fromMemorySegment(SPECIES, xSeg, xOff4, ByteOrder.nativeOrder());
                        FloatVector av4 = FloatVector.fromMemorySegment(SPECIES, aSeg, aOff4, ByteOrder.nativeOrder());
            ...
            parallelForRows(m, GEMV_THREADS, row -> {
                long rowBase = aBase + (long) row * n * Float.BYTES;
        ...
        private static void parallelForRows(int rows, int maxWorkers, java.util.function.IntConsumer body) {
            if (rows <= 1) {
        ...
        private Tensor attention(LlamaState state, Tensor q, int layer, DecodeScratch scratch, int position) {
            ...
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(qView.memory().device()).memoryDomain().directAccess();
            ...
            java.util.function.IntConsumer headKernel = head -> {
                int kvHead = head / kvMul;
                int kvHeadOffset = kvHead * headDim;
                int headOffset = head * headDim;
                ...
                for (int d = 0; d < headDim; d++) {
                    qHeadScratch[d] = access.readFloat(qView.memory(), qHeadBase + (long) d * Float.BYTES);
                }
                if (ATTN_DECODE_TWO_PASS) {
                    for (int t = 0; t < length; t++) {
                        ...
                        int d = 0;
                        for (; d < vecBoundHead; d += vecLen) {
                            FloatVector qVec = FloatVector.fromArray(SPECIES, qHeadScratch, d);
                            FloatVector kVec =
                                    FloatVector.fromMemorySegment(
                                            SPECIES,
                                            keySeg,
                        ...
                        float dot = accVec.reduceLanes(VectorOperators.ADD);
                        for (; d < headDim; d++) {
                            float kv = access.readFloat(key.memory(), kHeadBase + (long) d * Float.BYTES);
                    ...
                    for (int d2 = 0; d2 < headDim; d2++) {
                        ...
                        for (; t < vecBoundTime; t += vecLen) {
                            FloatVector scoreVec = FloatVector.fromArray(SPECIES, scoreScratch, t);
                    ...
                    if (ATTN_DECODE_BLOCKED && length >= (ATTN_DECODE_BLOCK_SIZE << 1)) {
                        ...
                        for (int blockStart = 0; blockStart < length; blockStart += blockSize) {
                            ...
                            float prevTerm =
                                    runningNorm == 0.0f
                                            ? 0.0f
                                            : runningNorm * (float) Math.exp(runningMax - newMax);
                            ...
                            int d = 0;
                            for (; d < vecBoundHead; d += vecLen) {
                                FloatVector outVec = FloatVector.fromArray(SPECIES, outHeadScratch, d);
                            ...
                            for (int d2 = 0; d2 < headDim; d2++) {
                                long vHeadBase =
                                        valueBase + (long) (kvHeadOffset + d2) * valueRowStrideBytes;
                                FloatVector accV = FloatVector.zero(SPECIES);
                                int bi = 0;
                                ...
                                for (; bi < blockLen; bi++) {
                                    int t = blockStart + bi;
                                    float vv = access.readFloat(value.memory(), vHeadBase + (long) t * Float.BYTES);
                            ...
                            for (; d < vecBoundHead; d += vecLen) {
                                FloatVector outVec = FloatVector.fromArray(SPECIES, outHeadScratch, d);
                                FloatVector tmpVec = FloatVector.fromArray(SPECIES, tmpHeadScratch, d);
                        ...
                        for (int t = 0; t < length; t++) {
                            ...
                            float oldScale = runningNorm == 0.0f ? 0.0f : (float) Math.exp(runningMax - newMax);
                            float newWeight = (float) Math.exp(score - newMax);
                            float newNorm = runningNorm * oldScale + newWeight;
                            float coeffOld = newNorm == 0.0f ? 0.0f : (runningNorm * oldScale) / newNorm;
                                ...
                                float vv =
                                        access.readFloat(
                                                ...
                                                valueBase
                                                        + (long) (kvHeadOffset + d2) * valueRowStrideBytes
                                                        + (long) t * Float.BYTES);
                                outHeadScratch[d2] = outHeadScratch[d2] * coeffOld + vv * coeffNew;
                            }
                ...
                for (int d = 0; d < headDim; d++) {
                    access.writeFloat(out.memory(), outHeadBase + (long) d * Float.BYTES, outHeadScratch[d]);
                }
            };
        ...
        private Tensor applyRoPETensor(Tensor input, int nHeads, int position, RopeTables rope, MemoryView<?> outBuf, Tensor outTensor) {
            MemoryView<?> in = input.materialize();
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(in.memory().device()).memoryDomain().directAccess();
            int headDim = cfg.headDim();
            ...
            for (int head = 0; head < nHeads; head++) {
                ...
                for (int i = 0; i < half; i++) {
                    ...
                    float c = cos[i];
                    float s = sin[i];
                    access.writeFloat(outBuf.memory(), outBase + (long) even * Float.BYTES, x0 * c - x1 * s);
                    access.writeFloat(outBuf.memory(), outBase + (long) odd * Float.BYTES, x0 * s + x1 * c);
                }
            }
            return outTensor;
        }

        private void writeKeyValueCache(LlamaState state, int layer, int kvDim, Tensor k, Tensor v) {
            writeKeyValueCache(state, layer, kvDim, k, v, state.position);
        }

        private void writeKeyValueCache(LlamaState state, int layer, int kvDim, Tensor k, Tensor v, int position) {
            MemoryView<?> kv = k.materialize();
            MemoryView<?> vv = v.materialize();
            MemoryView<?> keyCacheView = state.keyCache[layer];
            MemoryView<?> valueCacheView = state.valueCache[layer];
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(kv.memory().device()).memoryDomain().directAccess();
    ...
    private static final class Sampler {
        ...
        int sample(MemoryView<?> logits) {
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access = Environment.current().runtimeFor(logits.memory().device()).memoryDomain().directAccess();
    ...
    private static final class LoadedModel implements AutoCloseable {
        ...
        private LoadedModel(LlamaConfig config, Tokenizer tokenizer, Llama3ChatFormat chatFormat, LlamaModel model, Arena arena, FileChannel channel) {
            this.config = config;
        ...
        static LoadedModel load(Path modelPath) throws IOException {
            ...
            return new LoadedModel(config, tokenizer, format, new LlamaModel(config, weights), arena, channel);
        }
    ...
    private static Tokenizer loadTokenizer(GGUF gguf) {
        ...
        int[] tokenTypes = gguf.containsKey("tokenizer.ggml.token_type") ? gguf.getValue(int[].class, "tokenizer.ggml.token_type") : null;
        ...
        List<IntPair> pairs = Arrays.stream(merges).map(s -> s.split(" ")).map(p -> new IntPair(vocabulary.id(p[0]), vocabulary.id(p[1]))).toList();
        TextSplitter splitter = RegexSplitter.create(LLAMA3_PATTERN);
    ...
    private static LlamaConfig loadConfig(GGUF gguf, int vocab) {
        ...
        return new LlamaConfig(dim, gguf.getValue(int.class, a + ".feed_forward_length"), gguf.getValue(int.class, a + ".block_count"), heads, kvHeads, headDim, gguf.getValue(int.class, a + ".context_length"), vocab, gguf.getValue(float.class, a + ".attention.layer_norm_rms_epsilon"), gguf.getValue(float.class, a + ".rope.freq_base"));
    }

    private static LlamaWeights loadWeights(GGUF gguf, LlamaConfig cfg, long tensorBase, FileChannel channel, Arena arena) {
        Tensor tokenTable = mapTensorAsArrayTensor(gguf, "token_embd.weight", tensorBase, channel, arena);
        Tensor output = gguf.containsTensor("output.weight") ? mapTensorAsArrayTensor(gguf, "output.weight", tensorBase, channel, arena) : tokenTable;

        LlamaLayer[] layers = new LlamaLayer[cfg.nLayers()];
        for (int i = 0; i < cfg.nLayers(); i++) {
            layers[i] = new LlamaLayer(
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".attn_norm.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".attn_q.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".attn_k.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".attn_v.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".attn_output.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".ffn_norm.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".ffn_gate.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".ffn_down.weight", tensorBase, channel, arena),
                    mapTensorAsArrayTensor(gguf, "blk." + i + ".ffn_up.weight", tensorBase, channel, arena));
        }

        Tensor outNorm = mapTensorAsArrayTensor(gguf, "output_norm.weight", tensorBase, channel, arena);
        float[] ropeScales = gguf.containsTensor("rope_freqs.weight") ? toFloatArray(mapTensorAsArrayTensor(gguf, "rope_freqs.weight", tensorBase, channel, arena)) : null;
        RopeTables rope = RopeTables.precompute(cfg.contextLength, cfg.headDim, cfg.ropeTheta, ropeScales, cfg.nHeads);
        return new LlamaWeights(tokenTable, output, outNorm, layers, rope);
    }

    private static Tensor mapTensorAsArrayTensor(GGUF gguf, String name, long tensorBase, FileChannel channel, Arena arena) {
        TensorEntry e = gguf.getTensor(name);
        if (e == null) {
            throw new IllegalArgumentException("Missing tensor: " + name);
        }
        if (e.ggmlType() != GGMLType.F32) {
            throw new UnsupportedOperationException("Only F32 is supported; " + name + " is " + e.ggmlType());
        }
        long bytes = e.ggmlType().byteSizeForShape(e.shape());
        try {
            MemorySegment seg = channel.map(FileChannel.MapMode.READ_ONLY, tensorBase + e.offset(), bytes, arena);
            Memory<MemorySegment> mem = MemoryFactory.ofMemorySegment(seg);
            Shape shape = e.shape().length == 2 ? Shape.of(e.shape()[1], e.shape()[0]) : Shape.flat(e.shape());
            MemoryView<MemorySegment> view = MemoryView.of(mem, 0, DataType.FP32, Layout.rowMajor(shape));
            int size = Math.toIntExact(shape.size());
            float[] out = new float[size];
            MemoryAccess<MemorySegment> access = Environment.current().nativeMemoryDomain().directAccess();
