package ai.qxotic.jota.examples.demos;
...
public final class NBodySwingDemo {
    ...
    public static void main(String[] args) {
        ...
        ExecutionMode executionMode = device.equals(Device.HIP) ? ExecutionMode.LAZY : ExecutionMode.EAGER;
        try {
    ...
    private static void createAndShow(int width, int height, int bodies, long seed, Device device) {
        ...
        Timer timer =
                new Timer(
                        16,
                        e -> {
                            simulation.step(panel.mouseWorldX(), panel.mouseWorldY(), panel.mouseInside());
                            panel.repaint();
                        });
        timer.start();
    }
    ...
    private static final class NBodySimulation {
        ...
        private static float[] toHostVector(Tensor tensor) {
            ...
            MemoryView<float[]> dstView =
                    MemoryView.of(MemoryFactory.ofFloats(out), DataType.FP32, Layout.rowMajor(src.shape()));
