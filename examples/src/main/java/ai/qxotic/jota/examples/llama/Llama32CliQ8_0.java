 ...
 *
 * <p>This version extends the base Llama32Cli to support Q8_0 quantized weights,
 * which use 8-bit integer quantization with per-block scaling factors. This reduces
 * memory usage while maintaining good inference quality.
 *
 * <p>Q8_0 format: Each block contains 32 elements stored as:
 * - 2 bytes: Float16 scale factor
 * - 32 bytes: Quantized int8 values
 *
 * <p>The GEMV kernel uses the Vector API for efficient vectorized computation of
 * Q8_0 matrix-vector products.
 *
 * <p>Usage: Run with a Q8_0 quantized GGUF model file.
 */
public final class Llama32CliQ8_0 {
    ...
    private static void run(Options options) throws IOException {
        if (options.trace) {
            System.setProperty("ai.qxotic.trace", "true");
        }
    ...
    private static LlamaWeights loadWeights(
        ...
        // Output projection - keep Q8_0 for efficient gemv
        TensorWithQuantInfo outputInfo =
                gguf.containsTensor("output.weight")
                        ? mapTensorAsArrayTensor(gguf, "output.weight", tensorBase, channel, arena)
                        : new TensorWithQuantInfo(tokenTable, false);
        Tensor output = outputInfo.tensor();
        if (outputInfo.isQ8_0()) q8_0Tensors.add(output);
        ...
        for (int i = 0; i < cfg.nLayers; i++) {
            // Norm weights - dequantize for element access
            attnNorm[i] = mapTensorAsF32(
                    gguf, "blk." + i + ".attn_norm.weight", tensorBase, channel, arena);

            TensorWithQuantInfo t;

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".attn_q.weight", tensorBase, channel, arena);
            wq[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wq[i]);

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".attn_k.weight", tensorBase, channel, arena);
            wk[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wk[i]);

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".attn_v.weight", tensorBase, channel, arena);
            wv[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wv[i]);

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".attn_output.weight", tensorBase, channel, arena);
            wo[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wo[i]);

            // FFN norm - dequantize for element access
            ffnNorm[i] = mapTensorAsF32(
                    gguf, "blk." + i + ".ffn_norm.weight", tensorBase, channel, arena);

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".ffn_gate.weight", tensorBase, channel, arena);
            wGate[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wGate[i]);

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".ffn_down.weight", tensorBase, channel, arena);
            wDown[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wDown[i]);

            t = mapTensorAsArrayTensor(
                    gguf, "blk." + i + ".ffn_up.weight", tensorBase, channel, arena);
            wUp[i] = t.tensor();
            if (t.isQ8_0()) q8_0Tensors.add(wUp[i]);
        }

        // Output norm - dequantize for element access
        Tensor outNorm = mapTensorAsF32(gguf, "output_norm.weight", tensorBase, channel, arena);
        ...
        System.err.println("DEBUG: Loaded " + q8_0Tensors.size() + " Q8_0 tensors");
    ...
    private record TensorWithQuantInfo(Tensor tensor, boolean isQ8_0) {}

    /** Map tensor and force F32 by dequantizing Q8_0 if needed */
    private static Tensor mapTensorAsF32(
            GGUF gguf, String name, long tensorBase, FileChannel channel, Arena arena) {
        TensorWithQuantInfo info = mapTensorAsArrayTensor(gguf, name, tensorBase, channel, arena);
        if (!info.isQ8_0()) {
            return info.tensor();
        }
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                long blockIndex = i / blockSize;
                long withinBlockIndex = i % blockSize;
                long blockOffset = blockIndex * blockBytes;

                short scaleShort = seg.get(java.lang.foreign.ValueLayout.JAVA_SHORT_UNALIGNED, blockOffset);
                float scale = Float.float16ToFloat(scaleShort);
                byte quantVal = seg.get(java.lang.foreign.ValueLayout.JAVA_BYTE, blockOffset + 2 + withinBlockIndex);

                out[i] = quantVal * scale;
            }
            return Tensor.of(out, shape);
        } catch (IOException ex) {
            throw new RuntimeException("Failed to dequantize tensor: " + name, ex);
        }
    }


    private static TensorWithQuantInfo mapTensorAsArrayTensor(
            GGUF gguf, String name, long tensorBase, FileChannel channel, Arena arena) {
        TensorEntry e = gguf.getTensor(name);
        if (e == null) {
        ...
        try {
            ...
            if (e.ggmlType() == GGMLType.Q8_0) {
                // Keep Q8_0 data in raw quantized format with proper Q8_0 DataType
                // Shape must represent blocks, not elements (32 elements per block)
                Shape blockShape;
                if (e.shape().length == 2) {
                    long rows = e.shape()[1];
                    long cols = e.shape()[0];
                    if (cols % 32 != 0) {
                        throw new IllegalArgumentException("Q8_0 column dimension must be multiple of 32");
                    }
                    blockShape = Shape.of(rows, cols / 32);
                } else if (e.shape().length == 1) {
                    long size = e.shape()[0];
                    if (size % 32 != 0) {
                        throw new IllegalArgumentException("Q8_0 size must be multiple of 32");
                    }
                    blockShape = Shape.flat(size / 32);
                } else {
                    throw new UnsupportedOperationException("Unsupported Q8_0 shape rank: " + e.shape().length);
                }
                MemoryView<MemorySegment> view =
                        MemoryView.of(mem, 0, DataType.Q8_0, Layout.rowMajor(blockShape));
                // Return the original element shape for application logic, but tensor internally uses block layout
                return new TensorWithQuantInfo(Tensor.of(view), true);
            }

            // Handle F32 tensors (copy to float array for compatibility)
            if (e.ggmlType() == GGMLType.F32) {
                ...
                return new TensorWithQuantInfo(Tensor.of(out, shape), false);
            }

            throw new UnsupportedOperationException(
                    "Only F32 and Q8_0 are supported; " + name + " is " + e.ggmlType());
        } catch (IOException ex) {
    ...
    private static final class Llama32Model {
        ...
        private static final String GEMV_Q8_0_KERNEL_NAME = "llama.gemv.q8_0";
        ...
        private static final String GEMV_KERNEL_SOURCE =
                ...
                public final class LlamaGemvKernel implements JavaKernel {
                    ...
                    private static final int THREADS =
                            Math.max(
                                    1,
                                    Integer.getInteger(
                                            "ai.qxotic.llama.kernel.threads",
                ...
                """;
        // language=java
        private static final String GEMV_Q8_0_KERNEL_SOURCE =
                """
                package ai.qxotic.jota.tensor.jit;
                ...
                public final class LlamaGemvQ8_0Kernel implements JavaKernel {
                    private static final VectorSpecies<Float> F_SPECIES = FloatVector.SPECIES_PREFERRED;
                    private static final int FLOAT16_BYTES = 2;
                    private static final int Q8_0_BLOCK_SIZE = 32;
                    private static final int Q8_0_BLOCK_BYTES = FLOAT16_BYTES + Q8_0_BLOCK_SIZE;
                    private static final long PARALLEL_MIN_WORK = 131072L;
                    private static final int THREADS =
                            Math.max(
                                    1,
                    ...
                    public void execute(MemoryDomain<MemorySegment> domain, KernelArgs args) {
                        ...
                        if (work < PARALLEL_MIN_WORK) {
                            for (int row = 0; row < m; row++) {
                                long rowBase = aBase + (long) row * ((long)n / Q8_0_BLOCK_SIZE) * Q8_0_BLOCK_BYTES;
                                float dot = vectorDotQ8_0(aBaseSegment, rowBase, xBaseSegment, xBase, n, access);
                                access.writeFloat(y.memory(), yBase + (long) row * Float.BYTES, dot);
                            }
                            return;
                        }
                    ...
                    private static float vectorDotQ8_0(
                            MemorySegment aSegment, long aOffset,
                            MemorySegment xSegment, long xBase,
                            int size, MemoryAccess<MemorySegment> access) {
                        FloatVector val0 = FloatVector.zero(F_SPECIES);
                        FloatVector val1 = FloatVector.zero(F_SPECIES);
                        long blockOffset = aOffset;
                        int j = 0;
                        int upperBound = size / Q8_0_BLOCK_SIZE * Q8_0_BLOCK_SIZE;

                        // Unroll by 2 blocks for better ILP
                        int unrollBound = (upperBound / (2 * Q8_0_BLOCK_SIZE)) * (2 * Q8_0_BLOCK_SIZE);
                            ...
                            case 512 -> {
                                ...
                                for (; j < unrollBound; j += 2 * Q8_0_BLOCK_SIZE, blockOffset += 2 * Q8_0_BLOCK_BYTES) {
                                    ...
                                    var wBytes1 = ByteVector.fromMemorySegment(
                                            ByteVector.SPECIES_256, aSegment,
                                            blockOffset + Q8_0_BLOCK_BYTES + FLOAT16_BYTES, ByteOrder.LITTLE_ENDIAN);

                                    // Interleave computations for better scheduling
                                    var x0_0 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 0 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x0_1 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 1 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x1_0 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 32 + 0 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x1_1 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 32 + 1 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());

                                    var sum0_0 = x0_0.mul(wBytes0.castShape(F_SPECIES, 0));
                                    var sum0_1 = x0_1.mul(wBytes0.castShape(F_SPECIES, 1));
                                    var sum1_0 = x1_0.mul(wBytes1.castShape(F_SPECIES, 0));
                                    var sum1_1 = x1_1.mul(wBytes1.castShape(F_SPECIES, 1));

                                    val0 = sum0_0.add(sum0_1).fma(FloatVector.broadcast(F_SPECIES, wScale0), val0);
                                    val1 = sum1_0.add(sum1_1).fma(FloatVector.broadcast(F_SPECIES, wScale1), val1);
                                }
                            }
                            case 256 -> {
                                // Process 2 blocks at once
                                for (; j < unrollBound; j += 2 * Q8_0_BLOCK_SIZE, blockOffset += 2 * Q8_0_BLOCK_BYTES) {
                                    ...
                                    var x0_0 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 0 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x0_1 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 1 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x0_2 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 2 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x0_3 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 3 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());

                                    var sum0 = x0_0.mul(wBytes0.castShape(F_SPECIES, 0))
                                            .add(x0_1.mul(wBytes0.castShape(F_SPECIES, 1)))
                                            .add(x0_2.mul(wBytes0.castShape(F_SPECIES, 2)))
                                            .add(x0_3.mul(wBytes0.castShape(F_SPECIES, 3)));

                                    val0 = sum0.fma(FloatVector.broadcast(F_SPECIES, wScale0), val0);

                                    var x1_0 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 32 + 0 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x1_1 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 32 + 1 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x1_2 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 32 + 2 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());
                                    var x1_3 = FloatVector.fromMemorySegment(F_SPECIES, xSegment, xBase + (long)(j + 32 + 3 * F_SPECIES.length()) * Float.BYTES, ByteOrder.nativeOrder());

                                    var sum1 = x1_0.mul(wBytes1.castShape(F_SPECIES, 0))
                                            .add(x1_1.mul(wBytes1.castShape(F_SPECIES, 1)))
                                            .add(x1_2.mul(wBytes1.castShape(F_SPECIES, 2)))
                                            .add(x1_3.mul(wBytes1.castShape(F_SPECIES, 3)));

                                    val1 = sum1.fma(FloatVector.broadcast(F_SPECIES, wScale1), val1);
                                }
                            }
                            case 128 -> {
                                ...
                                for (; j < upperBound; j += Q8_0_BLOCK_SIZE, blockOffset += Q8_0_BLOCK_BYTES) {
                                    ...
                                    for (int i = 0; i < 2; ++i) {
                                        var wBytes = ByteVector.fromMemorySegment(
                                                B_128, aSegment,
                                                blockOffset + FLOAT16_BYTES + i * B_128.vectorByteSize(),
                                                ByteOrder.LITTLE_ENDIAN);
                                        var sum0 = FloatVector.fromMemorySegment(
                                                F_SPECIES, xSegment,
                                                xBase + (long)(j + i * 16 + 0 * F_SPECIES.length()) * Float.BYTES,
                                                ByteOrder.nativeOrder())
                                                .mul(wBytes.castShape(F_SPECIES, 0));
                                        var sum1 = FloatVector.fromMemorySegment(
                                                F_SPECIES, xSegment,
                                                xBase + (long)(j + i * 16 + 1 * F_SPECIES.length()) * Float.BYTES,
                                                ByteOrder.nativeOrder())
                                                .mul(wBytes.castShape(F_SPECIES, 1));
                                        var sum2 = FloatVector.fromMemorySegment(
                                                F_SPECIES, xSegment,
                                                xBase + (long)(j + i * 16 + 2 * F_SPECIES.length()) * Float.BYTES,
                                                ByteOrder.nativeOrder())
                                                .mul(wBytes.castShape(F_SPECIES, 2));
                                        var sum3 = FloatVector.fromMemorySegment(
                                                F_SPECIES, xSegment,
                                                xBase + (long)(j + i * 16 + 3 * F_SPECIES.length()) * Float.BYTES,
                                                ByteOrder.nativeOrder())
                                                .mul(wBytes.castShape(F_SPECIES, 3));
                                        val0 = sum0.add(sum1).add(sum2).add(sum3).fma(wScale, val0);
                                    }
                                }
                            }
                        ...
                        for (; j < upperBound; j += Q8_0_BLOCK_SIZE, blockOffset += Q8_0_BLOCK_BYTES) {
                            ...
                            switch (F_SPECIES.vectorBitSize()) {
                                case 512 -> {
                                    ...
                                    var sum0 = FloatVector.fromMemorySegment(
                                            F_SPECIES, xSegment,
                                            xBase + (long)(j + 0 * F_SPECIES.length()) * Float.BYTES,
                                            ByteOrder.nativeOrder())
                                            .mul(wBytes.castShape(F_SPECIES, 0));
                                    var sum1 = FloatVector.fromMemorySegment(
                                            F_SPECIES, xSegment,
                                            xBase + (long)(j + 1 * F_SPECIES.length()) * Float.BYTES,
                                            ByteOrder.nativeOrder())
                                            .mul(wBytes.castShape(F_SPECIES, 1));
                                    val0 = sum0.add(sum1).fma(wScale, val0);
                                }
                                case 256 -> {
                                    var wBytes = ByteVector.fromMemorySegment(
                                            ByteVector.SPECIES_256, aSegment,
                                    ...
                                    var sum2 = FloatVector.fromMemorySegment(
                                            F_SPECIES, xSegment,
                                            xBase + (long)(j + 2 * F_SPECIES.length()) * Float.BYTES,
                                            ByteOrder.nativeOrder())
                                            .mul(wBytes.castShape(F_SPECIES, 2));
                                    var sum3 = FloatVector.fromMemorySegment(
                                            F_SPECIES, xSegment,
                                            xBase + (long)(j + 3 * F_SPECIES.length()) * Float.BYTES,
                                            ByteOrder.nativeOrder())
                                            .mul(wBytes.castShape(F_SPECIES, 3));
                                    val0 = sum0.add(sum1).add(sum2).add(sum3).fma(wScale, val0);
                                }
                        ...
                        float result = val0.add(val1).reduceLanes(VectorOperators.ADD);
        ...
        private static final int C_GEMV_OMP_MIN_ROWS_DOWN =
                Integer.getInteger("ai.qxotic.llama.cgemv.omp.minRows.down", 64);
        private static final int C_GEMV_OMP_MIN_ROWS_LOGITS =
                Integer.getInteger("ai.qxotic.llama.cgemv.omp.minRows.logits", 32);
        private static final int C_GEMV_OMP_MIN_ROWS_PAIR =
                Integer.getInteger("ai.qxotic.llama.cgemv.omp.minRows.pair", 64);
        private static final int C_GEMV_OMP_CHUNK_DOWN =
                Integer.getInteger("ai.qxotic.llama.cgemv.omp.chunk.down", 4);
        private static final int C_GEMV_OMP_CHUNK_LOGITS =
                Integer.getInteger("ai.qxotic.llama.cgemv.omp.chunk.logits", 4);
        private static final int C_GEMV_OMP_CHUNK_PAIR =
                Integer.getInteger("ai.qxotic.llama.cgemv.omp.chunk.pair", 4);
        private static final int C_GEMV_PREFETCH_FLOATS_DOWN =
                Integer.getInteger("ai.qxotic.llama.cgemv.prefetch.down", 64);
        private static final int C_GEMV_PREFETCH_FLOATS_LOGITS =
                Integer.getInteger("ai.qxotic.llama.cgemv.prefetch.logits", 64);
        private static final int C_GEMV_PREFETCH_FLOATS_PAIR =
                Integer.getInteger("ai.qxotic.llama.cgemv.prefetch.pair", 128);
        ...
        private static final boolean USE_C_GEMV =
                Boolean.parseBoolean(System.getProperty("ai.qxotic.llama.cgemv", "true"));
        private static final long C_GEMV_MIN_WORK_GENERAL =
                Long.getLong("ai.qxotic.llama.cgemv.minWork.general", Long.MAX_VALUE);
        private static final long C_GEMV_MIN_WORK_DOWN =
                Long.getLong("ai.qxotic.llama.cgemv.minWork.down", 262_144L);
        private static final long C_GEMV_MIN_WORK_LOGITS =
                Long.getLong("ai.qxotic.llama.cgemv.minWork.logits", 131_072L);
        private static final long C_GEMV_MIN_WORK_PAIR =
                Long.getLong("ai.qxotic.llama.cgemv.minWork.pair", 262_144L);
        ...
        private final KernelExecutable gemvQ8_0Kernel;
        ...
        Llama32Model(LlamaConfig cfg, LlamaWeights w) {
            ...
            this.gemvQ8_0Kernel = registerGemvQ8_0Kernel(runtime);
        ...
        private Tensor attention(Tensor q, int layer, int length, DecodeScratch scratch) {
            ...
            MemoryView<?> key = keyCache[layer];
            MemoryView<?> value = valueCache[layer];
            MemoryView<?> out = scratch.attentionOutBuf;
            MemoryView<?> scoreRow = scratch.attnScoresBuf;
            if (qView.dataType() != DataType.FP32
                    || key.dataType() != DataType.FP32
                    || value.dataType() != DataType.FP32
                    || out.dataType() != DataType.FP32
                    || scoreRow.dataType() != DataType.FP32
                    || !qView.layout().isSuffixContiguous(0)
                    || !key.layout().isSuffixContiguous(0)
            ...
            long outBase = out.byteOffset();
            long scoreBase = scoreRow.byteOffset();
            ...
            for (int kvHead = 0; kvHead < cfg.nKvHeads; kvHead++) {
                ...
                for (int g = 0; g < kvMul; g++) {
                    ...
                    for (int t = 0; t < length; t++) {
                        ...
                        access.writeFloat(
                                scoreRow.memory(), scoreBase + (long) t * Float.BYTES, scaled);
                        if (scaled > maxScore) {
                            maxScore = scaled;
                        }
                    }
        ...
        private boolean projectWithGemvKernelInto(
                Tensor x, Tensor weightOutIn, MemoryView<?> outBuffer, GemvRoute route) {
            // Check if weight tensor is Q8_0 quantized
            boolean isQ8_0 = w.q8_0Tensors().contains(weightOutIn);
            ...
            // For Q8_0, weight is still reported as FP32 but stored differently
            if (!isQ8_0 && weightOutIn.dataType() != DataType.FP32) {
                gemvMissDtype++;
                return false;
            }
            ...
            // For Q8_0, shape is in blocks (32 elements per block), convert to elements
            long nElements = isQ8_0 ? n * 32 : n;
            if (x.shape().size(1) != nElements) {
                gemvMissShapeMismatch++;
                return false;
            }
            ...
            // Select the appropriate kernel based on quantization
            KernelExecutable kernelToUse;
            if (isQ8_0) {
                if (gemvQ8_0Kernel == null) {
                    gemvMissKernelUnavailable++;
                    return false;
                }
                kernelToUse = gemvQ8_0Kernel;
            } else {
                ...
                kernelToUse = (shouldUseCGemv(route, m, n) && cKernel != null && cStream != null)
                        ? null : gemvKernel;
            }
            ...
            if (isQ8_0) {
                // Use Q8_0 kernel (pass element count, not block count)
                kernelToUse.launch(
                        LaunchConfig.auto(),
                        KernelArgs.fromVarargs(
                                a, xv, outBuffer, Math.toIntExact(m), Math.toIntExact(nElements)),
                        stream);
            } else {
                // Use regular FP32 kernels
                KernelExecutable cKernel = cKernelForRoute(route);
                if (shouldUseCGemv(route, m, n) && cKernel != null && cStream != null) {
                    cKernel.launch(
                            LaunchConfig.auto(),
        ...
        private static KernelExecutable registerGemvQ8_0Kernel(DeviceRuntime runtime) {
            if (!runtime.supportsKernels()) {
                return null;
            }
            try {
                return runtime.loadRegisteredExecutable(GEMV_Q8_0_KERNEL_NAME)
                        .orElseGet(
                                () ->
                                        runtime.registerKernel(
        ...
        private void loadEmbeddingIntoScratch(int token, DecodeScratch scratch) {
            MemoryView<?> table = w.tokenTable.materialize();
            if (table.dataType() != DataType.FP32
                    || !table.layout().isSuffixContiguous(0)
                    || scratch.xBuf.dataType() != DataType.FP32
                    || !scratch.xBuf.layout().isSuffixContiguous(0)) {
                throw new IllegalStateException("embedding path requires suffix-contiguous FP32");
            }
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access =
                    Environment.current()
            ...
            long srcBase = table.byteOffset() + (long) token * cfg.dim * Float.BYTES;
            long dstBase = scratch.xBuf.byteOffset();
            for (int i = 0; i < cfg.dim; i++) {
                float v = access.readFloat(table.memory(), srcBase + (long) i * Float.BYTES);
                access.writeFloat(scratch.xBuf.memory(), dstBase + (long) i * Float.BYTES, v);
            }
        }

        private void rmsNormInto(Tensor x, Tensor weight, MemoryView<?> out) {
            MemoryView<?> inView = x.materialize();
            MemoryView<?> wView = weight.materialize();
            if (inView.dataType() != DataType.FP32
                    || wView.dataType() != DataType.FP32
                    || out.dataType() != DataType.FP32
                    || !inView.layout().isSuffixContiguous(0)
                    || !wView.layout().isSuffixContiguous(0)
                    || !out.layout().isSuffixContiguous(0)) {
                throw new IllegalStateException("rmsNormInto requires suffix-contiguous FP32");
            }
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access =
                    Environment.current()
            ...
            long xBase = inView.byteOffset();
            long wBase = wView.byteOffset();
            long outBase = out.byteOffset();
            float ss = 0f;
            for (int i = 0; i < cfg.dim; i++) {
                float v = access.readFloat(inView.memory(), xBase + (long) i * Float.BYTES);
                ss += v * v;
            }
            float inv = 1f / (float) Math.sqrt((ss / cfg.dim) + cfg.rmsEps);
            for (int i = 0; i < cfg.dim; i++) {
                float xv = access.readFloat(inView.memory(), xBase + (long) i * Float.BYTES);
                float wv = access.readFloat(wView.memory(), wBase + (long) i * Float.BYTES);
                access.writeFloat(out.memory(), outBase + (long) i * Float.BYTES, xv * inv * wv);
            }
        }
        ...
        private static void addInto(
            ...
            if (a.dataType() != DataType.FP32
                    ...
                    || out.dataType() != DataType.FP32
                    || !a.layout().isSuffixContiguous(0)
                    || !b.layout().isSuffixContiguous(0)
                    || !out.layout().isSuffixContiguous(0)) {
                throw new IllegalStateException("addInto requires suffix-contiguous FP32");
            }
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access =
                    Environment.current()
        ...
        private void projectWithDirectLoopInto(
            ...
            if (!isQ8_0 && (a.dataType() != DataType.FP32
                    || xv.dataType() != DataType.FP32
                    || outBuffer.dataType() != DataType.FP32
                    || !a.layout().isSuffixContiguous(0)
                    || !xv.layout().isSuffixContiguous(0)
                    || !outBuffer.layout().isSuffixContiguous(0))) {
                throw new IllegalStateException("project fallback requires suffix-contiguous FP32");
            }

            int m = Math.toIntExact(weightOutIn.shape().size(0));
            ...
            long yBase = outBuffer.byteOffset();

            if (isQ8_0) {
                // Q8_0 fallback: dequantize on-the-fly
                ...
                for (int row = 0; row < m; row++) {
                    long rowBase = aBase + (long) row * ((long)n / blockSize) * blockBytes;
                    float dot = 0f;
                    for (int col = 0; col < n; col++) {
                        int blockIndex = col / blockSize;
                        int withinBlockIndex = col % blockSize;
                        long blockOffset = rowBase + (long)blockIndex * blockBytes;

                        short scaleShort = aSeg.get(java.lang.foreign.ValueLayout.JAVA_SHORT_UNALIGNED, blockOffset);
                        float scale = Float.float16ToFloat(scaleShort);
                        byte quantVal = aSeg.get(java.lang.foreign.ValueLayout.JAVA_BYTE, blockOffset + 2 + withinBlockIndex);

                        float av = quantVal * scale;
                        float xvCol = access.readFloat(xv.memory(), xBase + (long) col * Float.BYTES);
                        dot += av * xvCol;
                    }
                    access.writeFloat(outBuffer.memory(), yBase + (long) row * Float.BYTES, dot);
                }
            } else {
                // Regular FP32 fallback
                long aBase = a.byteOffset();
                for (int row = 0; row < m; row++) {
                    long rowBase = aBase + (long) row * n * Float.BYTES;
                    float dot = 0f;
        ...
        private void applySwigluDirectInto(
            ...
            if (gate.dataType() != DataType.FP32
                    ...
                    || out.dataType() != DataType.FP32
                    || !gate.layout().isSuffixContiguous(0)
                    || !up.layout().isSuffixContiguous(0)
                    || !out.layout().isSuffixContiguous(0)) {
                throw new IllegalStateException("swiglu fallback requires suffix-contiguous FP32");
            }
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access =
                    Environment.current()
        ...
        private void writeKeyValueCache(
            ...
            MemoryView<?> valueDst = valueCache[layer];
            if (keyView.dataType() != DataType.FP32
                    || valueView.dataType() != DataType.FP32
                    || keyDst.dataType() != DataType.FP32
                    || valueDst.dataType() != DataType.FP32
                    || !keyView.layout().isSuffixContiguous(0)
                    || !valueView.layout().isSuffixContiguous(0)
                    || !keyDst.layout().isSuffixContiguous(0)
                    || !valueDst.layout().isSuffixContiguous(0)) {
                throw new IllegalStateException("KV cache write requires suffix-contiguous FP32");
            }
            ...
            long keySrcBase = keyView.byteOffset();
            long valueSrcBase = valueView.byteOffset();
            long keyDstBase = keyDst.byteOffset() + (long) position * kvDim * Float.BYTES;
            long valueColBase = valueDst.byteOffset() + (long) position * Float.BYTES;
            long valueRowStrideBytes = (long) cfg.contextLength * Float.BYTES;
        ...
        private Tensor applyRoPETensor(
                ...
                Tensor outTensor) {
            MemoryView<?> inView = x.materialize();
            if (inView.dataType() != DataType.FP32 || outBuffer.dataType() != DataType.FP32) {
                return x;
            }
            @SuppressWarnings({"rawtypes", "unchecked"})
            MemoryAccess access =
            ...
            for (int h = 0; h < heads; h++) {
                ...
                for (int i = 0; i < half; i++) {
                    ...
                    access.writeFloat(
                            outBuffer.memory(),
                            outBuffer.byteOffset() + (long) even * Float.BYTES,
                            re * c - im * s);
                    access.writeFloat(
                            outBuffer.memory(),
                            outBuffer.byteOffset() + (long) odd * Float.BYTES,
                            re * s + im * c);
                }
            }
            return outTensor;
        ...
        private void touchTensorArray(Tensor[] tensors) {
            for (Tensor tensor : tensors) {
                touchTensor(tensor);
            }
        }

        private void touchTensor(Tensor tensor) {
            // Skip touching Q8_0 tensors - they have different memory layout
            if (w.q8_0Tensors().contains(tensor)) {
                return;
            }
