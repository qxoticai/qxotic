 ...
 *
 * <p>This version extends the base Llama32Cli to support Q8_0 quantized weights, which use 8-bit
 * integer quantization with per-block scaling factors. This reduces memory usage while maintaining
 * good inference quality.
 *
 * <p>Q8_0 format: Each block contains 32 elements stored as: - 2 bytes: Float16 scale factor - 32
 * bytes: Quantized int8 values
 *
 * <p>The GEMV kernel uses the Vector API for efficient vectorized computation of Q8_0 matrix-vector
 * products.
 *
...
public final class Llama32CliQ8_0 {
                ...
                if (e.shape().length == 2) {
                    ...
                    if (cols % 32 != 0) {
                        throw new IllegalArgumentException(
                                "Q8_0 column dimension must be multiple of 32");
                    }
                    ...
                    throw new UnsupportedOperationException(
                            "Unsupported Q8_0 shape rank: " + e.shape().length);
                }
                MemoryView<MemorySegment> view =
                        MemoryView.of(mem, 0, DataType.Q8_0, Layout.rowMajor(blockShape));
                // Return the original element shape for application logic, but tensor internally
                // uses block layout
    ...
    private static final class Llama32Model {
        ...
        private static final boolean USE_C_GEMV =
                Boolean.parseBoolean(System.getProperty("com.qxotic.llama.cgemv", "true"));
        ...
        private boolean projectWithGemvKernelInto(
            ...
            if (isQ8_0) {
                ...
                kernelToUse =
                        (shouldUseCGemv(route, m, n) && cKernel != null && cStream != null)
                                ? null
                                : gemvKernel;
            }
            ...
            if (isQ8_0) {
                ...
                for (int row = 0; row < m; row++) {
                    ...
                    for (int col = 0; col < n; col++) {
                        ...
                        float xvCol =
                                access.readFloat(xv.memory(), xBase + (long) col * Float.BYTES);
                        dot += av * xvCol;
                    }
