...
public final class RayTracerFusedDemo {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.printf("Wrote %s [%dx%d] on %s in %d ms%n", OUTPUT, WIDTH, HEIGHT, device.name(), elapsedMs);
    }
    ...
    private static RenderResult render(int w, int h, int bounces) {
        ...
        Tensor rox = broadcast(0f, shape), roy = broadcast(-0.4f, shape), roz = broadcast(2.2f, shape);
        ...
        Tensor one = broadcast(1f, shape), zero = broadcast(0f, shape), half = broadcast(0.5f, shape);
        Tensor eps = broadcast(EPS, shape), huge = broadcast(HUGE, shape), hugeHalf = broadcast(HUGE * 0.5f, shape);
        Tensor accR = zero, accG = zero, accB = zero, thrR = one, thrG = one, thrB = one, alive = one;
        for (int i = 0; i < bounces; i++) {
            Hit hit = trace(spheres, rox, roy, roz, rdx, rdy, rdz, one, zero, eps, huge, hugeHalf, shape);
            ...
            Tensor ldx = hit.x.subtract(zero), ldy = hit.y.subtract(broadcast(5f, shape)), ldz = hit.z.subtract(zero);
            Tensor dist = ldx.square().add(ldy.square()).add(ldz.square()).add(0.1f).sqrt();
            ldx = ldx.divide(dist); ldy = ldy.divide(dist); ldz = ldz.divide(dist);
            Tensor ndotl = hit.nx.multiply(ldx).add(hit.ny.multiply(ldy)).add(hit.nz.multiply(ldz)).max(zero).min(one);
            ...
            Tensor norm = rdx.square().add(rdy.square()).add(rdz.square()).add(1e-6f).sqrt().reciprocal();
            rdx = rdx.multiply(norm); rdy = rdy.multiply(norm); rdz = rdz.multiply(norm);
            rox = hit.x.add(hit.nx.multiply(EPS * 4f));
            roy = hit.y.add(hit.ny.multiply(EPS * 4f));
            roz = hit.z.add(hit.nz.multiply(EPS * 4f));
        }
        return new RenderResult(toArray(accR.divide(accR.add(1f)).sqrt().max(zero).min(one)),
    ...
    private static Hit trace(Sphere[] spheres, Tensor ox, Tensor oy, Tensor oz, Tensor dx, Tensor dy, Tensor dz,
                             Tensor one, Tensor zero, Tensor eps, Tensor huge, Tensor hugeHalf, Shape shape) {
        Tensor bestT = huge, bx = zero, by = zero, bz = zero, bnx = zero, bny = zero, bnz = zero;
        Tensor bar = zero, bag = zero, bab = zero, brefl = zero;
        for (Sphere s : spheres) {
            Tensor cx = broadcast(s.cx, shape), cy = broadcast(s.cy, shape), cz = broadcast(s.cz, shape), r = broadcast(s.r, shape);
            ...
            t = tNear.multiply(near.cast(DataType.FP32)).add(t.multiply(one.subtract(near.cast(DataType.FP32))));
            Tensor far = disc.greaterThan(zero).logicalAnd(tFar.greaterThan(eps)).logicalAnd(tFar.lessThan(t));
            t = tFar.multiply(far.cast(DataType.FP32)).add(t.multiply(one.subtract(far.cast(DataType.FP32))));
            ...
            Tensor hx = ox.add(dx.multiply(t)), hy = oy.add(dy.multiply(t)), hz = oz.add(dz.multiply(t));
            Tensor nx = hx.subtract(cx).divide(r), ny = hy.subtract(cy).divide(r), nz = hz.subtract(cz).divide(r);
            bx = hx.multiply(better).add(bx.multiply(worse)); by = hy.multiply(better).add(by.multiply(worse)); bz = hz.multiply(better).add(bz.multiply(worse));
            bnx = nx.multiply(better).add(bnx.multiply(worse)); bny = ny.multiply(better).add(bny.multiply(worse)); bnz = nz.multiply(better).add(bnz.multiply(worse));
    ...
    private static void writePpm(String file, int w, int h, RenderResult r) throws IOException {
        ...
        for (int i = 0; i < w * h; i++) {
            rgb[i * 3] = toByte(r.r[i]); rgb[i * 3 + 1] = toByte(r.g[i]); rgb[i * 3 + 2] = toByte(r.b[i]);
        }
    ...
    private static byte toByte(float v) { return (byte) Math.round(Math.max(0, Math.min(1, v)) * 255); }

    private static float[] toArray(Tensor t) {
        MemoryView<?> view = t.materialize();
        float[] arr = new float[(int) view.shape().size()];
        @SuppressWarnings("unchecked") MemoryView<Object> src = (MemoryView<Object>) view;
        MemoryDomain.copy((MemoryDomain<Object>) Environment.current().runtimeFor(view.memory().device()).memoryDomain(),
            src, DomainFactory.ofFloats(), MemoryView.of(MemoryFactory.ofFloats(arr), DataType.FP32, Layout.rowMajor(view.shape())));
        return arr;
    }

    private static Tensor broadcast(float v, Shape s) { return Tensor.broadcasted(v, s); }
    private record Sphere(float cx, float cy, float cz, float r, float ar, float ag, float ab, float refl) {}
    private record Hit(Tensor mask, Tensor x, Tensor y, Tensor z, Tensor nx, Tensor ny, Tensor nz,
                       Tensor ar, Tensor ag, Tensor ab, Tensor refl) {}
    ...
    private static Device parseDevice(String value) {
        String n = value == null ? "panama" : value.trim().toLowerCase();
        return switch (n) {
            case "panama", "cpu", "native" -> Device.PANAMA;
            case "c" -> Device.C;
            case "hip" -> Device.HIP;
            case "opencl", "cl" -> Device.OPENCL;
            default -> throw new IllegalArgumentException("Unsupported --device: " + value + " (use panama|c|hip|opencl)");
        };
    }
    ...
    private static void printRuntimeDiagnostics(Environment env) {
        ...
        for (RuntimeDiagnostic d : env.runtimeDiagnostics()) {
            System.out.println("- " + d.device().leafName() + " [" + d.probe().status().name().toLowerCase() + "] " + d.probe().message());
    ...
    private static String unavailableDeviceMessage(Environment env, Device device) {
        StringBuilder sb = new StringBuilder("Requested device runtime is not available: " + device.name() + "\n");
        for (RuntimeDiagnostic d : env.runtimes().diagnosticsFor(device)) {
            sb.append("- ").append(d.probe().status().name().toLowerCase()).append(": ").append(d.probe().message()).append("\n");
            if (d.probe().hint() != null) sb.append("  hint: ").append(d.probe().hint()).append("\n");
        }
        sb.append("Try: --device panama or run with --list-devices for diagnostics.");
        return sb.toString();
    }
}
