...
/**
 * Core computational interface for tensor operations in the LLaMA model implementation.
 * This interface defines the fundamental operations required for transformer model inference,
 * including attention mechanisms, matrix operations, and various tensor manipulations.
 * <p>
 * A key design principle of this interface is the "potato tensor" approach - tensors are treated
 * as opaque data structures that cannot be read from or inspected. This deliberate design choice:
 * - Forces all operations to be self-contained within the implementation
 * - Enables efficient zero-copy implementations on accelerators (GPU, etc.)
 * - Prevents accidental data transfers between host and device memory
 * - Maintains clean abstraction boundaries between compute and data management
 *
...
public interface KernelOps<V extends FloatSpan, M extends FloatMatrixView> {
    ...
    /**
     * Copies content from one span to another.
     * This operation maintains the "potato tensor" principle by not exposing the actual data.
     *
    ...
    /**
     * Copies content from input span to output span with strided access pattern.
     * Allows for non-contiguous memory access patterns in the output tensor.
     *
    ...
    /**
     * Computes the softmax function over the input span.
    ...
    /**
     * Applies RMSNorm (Root Mean Square Layer Normalization).
    ...
    /**
     * Multiplies a matrix by a vector: out = matrix @ vector
     * The operation assumes compatible dimensions: [rows, cols] @ [cols] = [rows]
     *
    ...
    /**
     * Performs matrix multiplication between two matrices: out = a @ b^T
     * The operation assumes row-major order for all matrices.
     *
     * <p>
     * {@code A=[R, K] B=[K, C]^T C=[R, C]}
     *
    ...
    default void matrixMultiply(long R, long C, long K, M a, M b, M out) {
        ...
        gemmRowMajor(R, C, K,
                a.innerSpan(), a.startOffset(), a.rowStride(),
                b.innerSpan(), b.startOffset(), b.rowStride(),
                out.innerSpan(), out.startOffset(), out.rowStride()
        );
    }

    /**
     * Performs general matrix multiplication (GEMM) in row-major order.
     * This is equivalent to the BLAS GEMM operation in column-major order with parameters:
     * {@code GEMM("T", "N", C, R, K, 1f, B, B_offset, ldb, A, A_offset, lda, 0f, C, C_offset, ldc)}
     * <p>
     * Rather use {@link #matrixMultiply(long, long, long, FloatMatrixView, FloatMatrixView, FloatMatrixView)} which provide more simple interface.
     */
     ...
     * Supports both original and NeoX-style implementations.
     *
