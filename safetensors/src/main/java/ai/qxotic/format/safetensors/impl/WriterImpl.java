...
final class WriterImpl {
    ...
    static long computeTensorDataOffset(Map<String, String> metadata, Collection<TensorEntry> tensorEntries, int alignment) {
    ...
    static void writeImpl(Safetensors safetensors, WritableByteChannel byteChannel) throws IOException {
        ...
        int paddingSpaces = Math.toIntExact(padding(8 + headerBytes.length, safetensors.getAlignment()));
    ...
    private static byte[] getHeaderBytes(Map<String, String> metadata, Collection<TensorEntry> tensorEntries) {
    ...
    private void writePaddingWithSpaces(WritableByteChannel byteChannel, int padding) throws IOException {
        if (padding == 0) {
            return;
        }
    ...
    private static Map<String, Object> toJSON(Map<String, String> metadata, Collection<TensorEntry> tensorEntries) {
        Map<String, Object> json = new LinkedHashMap<>();
        if (!metadata.isEmpty()) {
            json.put(ReaderImpl.METADATA_KEY, metadata);
        }
        for (TensorEntry tensorEntry : tensorEntries) {
            List<Long> shape = Arrays.stream(tensorEntry.shape()).boxed().collect(Collectors.toList());
            long startOffset = tensorEntry.byteOffset();
            long endOffset = startOffset + tensorEntry.byteSize();
            json.put(tensorEntry.name(),
                    Map.of(
