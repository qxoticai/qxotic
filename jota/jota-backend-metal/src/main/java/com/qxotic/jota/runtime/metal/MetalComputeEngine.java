...
public final class MetalComputeEngine implements ComputeEngine {
    ...
    private final boolean interpreterFallback =
            Boolean.parseBoolean(System.getProperty("jota.metal.interpreter.fallback", "true"));

    MetalComputeEngine(Device device) {
        this.device = device;
    }
    ...
    private MemoryView<?> executeCompiled(TIRGraph graph, List<Tensor> inputs) {
        ...
        MetalMemoryDomain metalDomain = MetalMemoryDomain.instance();
        List<MemoryView<?>> outputs = allocateOutputs(lirGraph, metalDomain);
        ...
        if (shouldReturnHostOutput()) {
            return toHost(metalDomain, castDevice(outputs.getFirst()));
        }
    ...
    private MemoryView<?> executeWithInterpreterFallback(TIRGraph graph, List<Tensor> inputs) {
        ...
        return toDevice(MetalMemoryDomain.instance(), hostOutputs.getFirst());
    }
    ...
    private static MemoryView<MetalDevicePtr> allocateScratchBuffer(
    ...
    private static List<Tensor> resolveInputs(
        ...
        for (int i = 0; i < graph.inputs().size(); i++) {
            ...
            resolved.add(Tensor.of(toDevice(metalDomain, runtimeView)));
        }
        return resolved;
    }

    private static MemoryView<MetalDevicePtr> toDevice(
            MetalMemoryDomain metalDomain, MemoryView<?> view) {
        if (view.memory().device().belongsTo(DeviceType.METAL)) {
            @SuppressWarnings("unchecked")
            MemoryView<MetalDevicePtr> metalView = (MemoryView<MetalDevicePtr>) view;
            return metalView;
        }
        @SuppressWarnings("unchecked")
        ...
        MemoryView<MetalDevicePtr> dst =
                MemoryView.of(
                        metalDomain.memoryAllocator().allocateMemory(spec.byteSize()),
                        spec.byteOffset(),
                        view.dataType(),
                        view.layout());
        MemoryDomain.copy(srcDomain, srcView, metalDomain, dst);
        return dst;
    }
    ...
    private static MemoryView<MemorySegment> toHost(
        ...
        MemoryDomain.copy(metalDomain, view, hostDomain, hostView);
        return hostView;
    }
    ...
    private static MemoryView<MetalDevicePtr> castDevice(MemoryView<?> view) {
        @SuppressWarnings("unchecked")
        MemoryView<MetalDevicePtr> metalView = (MemoryView<MetalDevicePtr>) view;
        return metalView;
    }
    ...
    private static List<MemoryView<?>> allocateOutputs(
            LIRGraph graph, MetalMemoryDomain metalDomain) {
        List<MemoryView<?>> outputs = new ArrayList<>(graph.outputs().size());
        for (var output : graph.outputs()) {
            Memory<MetalDevicePtr> memory =
                    metalDomain.memoryAllocator().allocateMemory(output.dataType(), output.shape());
