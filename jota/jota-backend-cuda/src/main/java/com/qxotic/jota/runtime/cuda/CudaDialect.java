...
final class CudaDialect implements CLikeDialect {
    ...
    public String renderSource(LIRGraph graph, ScratchLayout scratchLayout, String kernelName) {
        return new CudaEmitter(graph, scratchLayout, kernelName).generate();
    }

    private static final class CudaEmitter extends CLikeLirSourceGenerator {
        private CudaEmitter(LIRGraph graph, ScratchLayout scratchLayout, String kernelName) {
            super(graph, scratchLayout, kernelName);
        }

        @Override
        protected void appendPreamble(StringBuilder source) {
            source.append("#include <cuda_runtime.h>\n");
            source.append("#include <cuda_fp16.h>\n");
            source.append("#include <cuda_bf16.h>\n");
