...
public final class TIRValidationPass implements TIRPass {
    ...
    private void validateNode(TIRNode node, Set<TIRNode> visited) {
        ...
        switch (node) {
            ...
            case GatherOp op -> {
                validateGatherOp(op);
                validateNode(op.input(), visited);
                validateNode(op.indices(), visited);
            }
    ...
    private void validateGatherOp(GatherOp node) {
        ...
        if (normalizedAxis < 0 || normalizedAxis >= rank) {
            throw new IllegalArgumentException(
                    "Gather axis out of bounds: " + axis + " for rank " + rank);
        }

        // Validate indices is integral type
        DataType indicesType = node.indices().dataType();
        ...
        Shape expected =
                GatherOp.computeOutputShape(inputShape, node.indices().shape(), node.axis());
        requireShapeEquals("GatherOp", node.shape(), expected);
    }
