...
import static ai.qxotic.jota.Shape.shape;
...
class ShapeTest {
    ...
    @ParameterizedTest
    @MethodSource("shapeProvider")
    void flatAtWithWrapAround(Shape shape, long[] expectedDims) {
        for (int i = 0; i < expectedDims.length; i++) {
            assertEquals(expectedDims[i], shape.flatAt(i));
            // Test negative indices
            assertEquals(expectedDims[i], shape.flatAt(i - expectedDims.length));
        }
    }

    @Test
    void flatAtThrowsForInvalidIndex() {
        Shape shape = Shape.of(2, 3);
        assertThrows(IllegalArgumentException.class, () -> shape.flatAt(2));
        assertThrows(IllegalArgumentException.class, () -> shape.flatAt(-3));
    }
    ...
    void testModeAt() {
        ...
        Shape[] modes =
                IntStream.range(0, shape.rank()).mapToObj(shape::modeAt).toArray(Shape[]::new);
    ...
    //        assertArrayEquals(new long[]{1, 1, 2, 1, 3},
    // alreadyUnsqueezed.unsqueeze(1).toArray());
    //    }
    //
    //    @Test
    ...
    //        assertEquals(original.size() * 4, original.insert(1, 4).size());
    //        assertEquals(original.size(), original.unsqueeze(1).size());
    //    }
}
