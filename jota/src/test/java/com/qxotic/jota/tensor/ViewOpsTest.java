...
class ViewOpsTest {
    ...
    void nestedUnsqueezeSqueezeRoundTripShape() {
        Tensor input = Tensor.iota(30, DataType.FP32).view(Shape.of(2, Shape.of(3, 5)));
        MemoryView<?> original = input.materialize();
    ...
    /** Tests that reshape returns a result with the correct shape for contiguous input. */
    @Test
    ...
    /** Tests that reshape works when view is not possible (strided slice with holes). */
    @Test
    ...
    /** Tests that reshape works for transposed tensor. */
    @Test
    void reshapeTransposedTensor() {
        ...
        MemoryView<?> materialized = flatView.materialize();
        assertTrue(materialized.isContiguous());
    ...
    /** Tests that reshape works for sliced tensor with gaps. */
    @Test
