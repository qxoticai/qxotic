...
class MaxReductionOpTest extends AbstractMemoryTest {
    ...
    void reducesMaxAlongAxisForAllTypes() {
        ...
        for (DataType dataType : PRIMITIVE_TYPES) {
            ...
            assertValueEquals(
                    dataType,
                    expectedMax(dataType, 0, 3),
                    TensorTestReads.readValue(reduced, 0, dataType));
            assertValueEquals(
                    dataType,
                    expectedMax(dataType, 3, 3),
    ...
    void reducesExpressionInputs() {
        ...
        assertValueEquals(
                DataType.FP32, 3.0f, TensorTestReads.readValue(reduced, 0, DataType.FP32));
        assertValueEquals(
                DataType.FP32, 6.0f, TensorTestReads.readValue(reduced, 1, DataType.FP32));
    }

    @Test
    void reducesFullShapeToScalarForMax() {
        Tensor input = range(DataType.FP32, Shape.of(2, 3));
        Tensor reduced = input.max();
        MemoryView<?> output = reduced.materialize();
        assertEquals(Shape.scalar(), output.shape());
        assertValueEquals(
                DataType.FP32, 5.0f, TensorTestReads.readValue(reduced, 0, DataType.FP32));
    }

    @Test
    void wrapsNegativeAxisForMax() {
        Tensor input = range(DataType.FP32, Shape.of(2, 2, 2));
        Tensor reduced = input.max(-1);
        MemoryView<?> output = reduced.materialize();
        assertEquals(Shape.of(2, 2), output.shape());
        assertValueEquals(
                DataType.FP32, 1.0f, TensorTestReads.readValue(reduced, 0, DataType.FP32));
        assertValueEquals(
                DataType.FP32, 3.0f, TensorTestReads.readValue(reduced, 1, DataType.FP32));
        assertValueEquals(
                DataType.FP32, 5.0f, TensorTestReads.readValue(reduced, 2, DataType.FP32));
        assertValueEquals(
                DataType.FP32, 7.0f, TensorTestReads.readValue(reduced, 3, DataType.FP32));
    }
    ...
    void normalizesDuplicateAxesForMax() {
        ...
        assertEquals(Shape.of(2, 1), output.shape());
        assertValueEquals(
                DataType.FP32, 2.0f, TensorTestReads.readValue(reduced, 0, DataType.FP32));
        assertValueEquals(
                DataType.FP32, 5.0f, TensorTestReads.readValue(reduced, 1, DataType.FP32));
    }
