...
class TensorScheduleIntegrationTest {
    ...
    void schedulesNonTrivialTracedTensorProgram() {
        ...
        boolean hasGather =
                schedule.steps().stream()
                        .anyMatch(
                                step ->
                                        containsNodeType(
                                                step.graph().outputs().getFirst(), GatherOp.class));
        boolean hasReduction =
                schedule.steps().stream()
                        .anyMatch(
                                step ->
                                        containsNodeType(
                                                step.graph().outputs().getFirst(),
                                                ReductionOp.class));
    ...
    private static boolean containsNodeType(TIRNode node, Class<? extends TIRNode> type) {
        ...
        return switch (node) {
            ...
            case BinaryOp op ->
                    containsNodeType(op.left(), type) || containsNodeType(op.right(), type);
            case TernaryOp op ->
            ...
            case GatherOp op ->
                    containsNodeType(op.input(), type) || containsNodeType(op.indices(), type);
