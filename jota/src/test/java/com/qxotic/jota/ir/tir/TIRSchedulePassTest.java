...
class TIRSchedulePassTest {
    ...
    void splitsKernelWhenGraphContainsTwoReductions() {
        ...
        ReductionOp first =
                new ReductionOp(ReductionOperator.SUM, input, new int[] {1}, false, DataType.FP32);
        ReductionOp second =
                new ReductionOp(ReductionOperator.SUM, first, new int[] {0}, false, DataType.FP32);
        ...
        assertEquals(
                2,
                schedule.steps().size(),
                "Expected split because each kernel supports at most one reduction");
        KernelStep firstStep = schedule.steps().get(0);
        KernelStep secondStep = schedule.steps().get(1);
        assertTrue(
                secondStep
                        .inputs()
                        .contains(new ScheduleInputRef.ProducedValueRef(firstStep.output())));
    }
}
