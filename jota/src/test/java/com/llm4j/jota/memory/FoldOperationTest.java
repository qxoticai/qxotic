...
class FoldOperationTest {
    ...
    public static MemoryView<float[]> of(float... floats) {
        return MemoryViewFactory.of(Shape.of(floats.length), DataType.F32, MemoryFactory.ofFloats(floats));
    }

    public static MemoryView<float[]> full(float value, Shape shape) {
        var floats = new float[Math.toIntExact(shape.totalNumberOfElements())];
    ...
    private float[] extractValues(MemoryView<float[]> view) {
        long totalElements = view.shape().totalNumberOfElements();
