...
import static com.llm4j.jota.memory.AbstractMemoryTest.calculateByteOffset;
import static com.llm4j.jota.memory.AbstractMemoryTest.readFloat;
...
class BroadcastTest {
    ...
    void testBroadcastVectorToMatrix() {
        ...
        for (int i = 0; i < 2; i++) {
            assertEquals(1.0f, memoryAccess.readFloat(result.memory(), calculateByteOffset(result, i, 0)));
            assertEquals(2.0f, memoryAccess.readFloat(result.memory(), calculateByteOffset(result, i, 1)));
            assertEquals(3.0f, memoryAccess.readFloat(result.memory(), calculateByteOffset(result, i, 2)));
        }
    }
    ...
    void testBroadcastWithExistingSingletons() {
        ...
        MemoryView<float[]> vec = MemoryViewFactory.of(
                Shape.of(2, 1),  // Already has singleton dimension
                DataType.F32,
                MemoryFactory.ofFloats(data)
        );
        ...
        assertEquals(5.0f, memoryAccess.readFloat(result.memory(), calculateByteOffset(result, 0, 0)));
        assertEquals(5.0f, memoryAccess.readFloat(result.memory(), calculateByteOffset(result, 0, 3))); // Broadcasted
        assertEquals(10.0f, memoryAccess.readFloat(result.memory(), calculateByteOffset(result, 1, 2)));
    }
    ...
    void testBroadcastNonContiguousSource() {
        ...
        MemoryView<float[]> strided = MemoryViewFactory.of(
                ...
                strides,
                DataType.F32,
                4, // Start at index 1 (4 bytes offset)
                MemoryFactory.ofFloats(underlying)
        );
        ...
        assertArrayEquals(new long[]{0, 8}, result.byteStrides()); // outer stride=0
        MemoryAccess<float[]> memoryAccess = MemoryAccessFactory.ofFloats();
    ...
    void testBroadcastWithOffsetMemory() {
        ...
        MemoryView<float[]> view = MemoryViewFactory.of(
                ...
                new long[]{4},
                DataType.F32,
                50 * 4, // byte offset to [50]
                MemoryFactory.ofFloats(largeBuffer)
        );
    ...
    void testBroadcastWithNegativeStride() {
        ...
        MemoryView<float[]> reversed = MemoryViewFactory.of(
                ...
                DataType.F32,
                3 * 4, // start at last element (offset=12)
                MemoryFactory.ofFloats(underlyingData)
        );
        ...
        assertEquals(4.0f, readFloat(memoryAccess, broadcasted, 0, 0)); // first row, first col
    ...
    void testBroadcastEmptyTensor() {
        ...
        assertEquals(0, result.shape().totalNumberOfElements());
    ...
    void testBroadcastAlreadyBroadcasted() {
        // Tensor of shape [1,3] that is actually a broadcasted view over [3] with stride0 in the first dimension.
        float[] data = {1.0f, 2.0f, 3.0f};
        ...
        assertArrayEquals(new long[]{0, 0, 4}, result.byteStrides());
        MemoryAccess<float[]> memoryAccess = MemoryAccessFactory.ofFloats();
    ...
    void freshViewIsNotBroadcasted() {
        MemoryView<float[]> v = MemoryViewFactory.of(Shape.of(2, 3), DataType.F32, MemoryFactory.ofFloats(new float[6]));
        assertFalse(v.isBroadcasted(), "Fresh compact view should not be broadcasted (no zero strides)");
    }
    ...
    void scalarBroadcastedTo2DHasZeroStrides() {
        ...
        assertTrue(b.isBroadcasted(), "Broadcasting scalar to (2,3) should produce zero strides");
    ...
    void expandAlongSingletonAxisIsBroadcasted() {
        ...
        MemoryView<float[]> v = MemoryViewFactory.of(Shape.of(4, 1), DataType.F32, MemoryFactory.ofFloats(new float[4]));
        assertFalse(v.isBroadcasted());
        MemoryView<float[]> expanded = v.expand(Shape.of(4, 3));
        assertTrue(expanded.isBroadcasted(), "Expanding a singleton axis must create zero stride");
    ...
    void expandAfterPermuteIsBroadcastedOnThatAxis() {
        ...
        MemoryView<float[]> v = MemoryViewFactory.of(Shape.of(3, 1, 4), DataType.F32, MemoryFactory.ofFloats(new float[12]));
        assertFalse(v.isBroadcasted());
        // Expand the singleton middle axis from 1 -> 7
        MemoryView<float[]> expanded = v.expand(Shape.of(3, 7, 4));
        assertTrue(expanded.isBroadcasted(), "Expanded singleton middle axis should have zero stride");
    }

    @Test
    void broadcastAddsLeadingAxisWhenNeeded() {
        // Starting from 1D view (size along last axis), broadcast requires aligning by prepending singleton dims
        MemoryView<float[]> v = MemoryViewFactory.of(Shape.of(1, 5), DataType.F32, MemoryFactory.ofFloats(new float[5]));
        assertFalse(v.isBroadcasted());
        // Broadcast last axis 5 stays 5, first axis 1 -> 3
        MemoryView<float[]> b = v.broadcast(Shape.of(3, 5));
        assertTrue(b.isBroadcasted(), "Broadcasting along first axis should introduce a zero stride axis");
    }
    ...
    void sliceWithIndexStrideIsNotBroadcasted() {
        ...
        assertFalse(colStride2.isBroadcasted(), "Strided slicing is not broadcasting (no zero strides)");
    ...
    void scalarBroadcastedTo3DIsBroadcasted() {
        MemoryView<float[]> scalar = MemoryViewFactory.of(Shape.scalar(), DataType.F32, MemoryFactory.ofFloats(7f));
    ...
    void testBroadcastZeroDim() {
        MemoryView<float[]> empty = MemoryViewFactory.of(Shape.of(0), DataType.F32, MemoryFactory.ofFloats(new float[0]));
        MemoryView<float[]> result = empty.broadcast(Shape.of(3, 0));
        assertEquals(Shape.of(3, 0), result.shape());
        assertEquals(0, result.shape().totalNumberOfElements());
    }
    ...
    void testBroadcastScalarToZero() {
        MemoryView<float[]> scalar = MemoryViewFactory.of(Shape.scalar(), DataType.F32, MemoryFactory.ofFloats(1f));
    ...
    void testBroadcastToZeroDim() {
        ...
        assertEquals(0, r.shape().totalNumberOfElements());
    ...
    void testBroadcastScalarToZero2() {
        MemoryView<float[]> s = MemoryViewFactory.of(Shape.scalar(), DataType.F32, MemoryFactory.ofFloats(1f));
