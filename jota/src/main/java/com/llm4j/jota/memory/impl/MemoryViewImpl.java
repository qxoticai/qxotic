...
final class MemoryViewImpl<T> implements MemoryView<T> {
    ...
    private MemoryViewImpl(Shape shape, long[] byteStrides, DataType dataType, long byteOffset, Memory<T> memory) {
        ...
        if (this.isContiguous) {
            if (byteOffset + dataType.byteSizeFor(shape.totalNumberOfElements()) > memory.byteSize()) {
    ...
    static long[] contiguousByteStrides(Shape shape, DataType dataType) {
        long[] byteStrides = new long[shape.rank()];
        long curStride = dataType.byteSize();

        for (int i = shape.rank() - 1; i >= 0; --i) {
            byteStrides[i] = curStride;
            curStride *= shape.dimension(i);
        }
...
//    }
    ...
    public MemoryView<T> reshape(Shape newShape) {
        if (shape().totalNumberOfElements() != newShape.totalNumberOfElements()) {
