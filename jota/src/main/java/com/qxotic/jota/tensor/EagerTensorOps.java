...
public class EagerTensorOps implements TensorOps {
    ...
    private final Optional<DeviceRuntime.EagerKernels> eagerKernels;
    private final boolean requireKernels;

    public EagerTensorOps(MemoryDomain<?> domain) {
        this.domain = Objects.requireNonNull(domain);
        this.eagerKernels = Optional.empty();
        this.requireKernels = false;
    }

    public EagerTensorOps(DeviceRuntime runtime) {
        Objects.requireNonNull(runtime, "runtime");
        this.domain = runtime.memoryDomain();
        this.eagerKernels = runtime.eagerKernels();
        this.requireKernels = true;
    }
    ...
    public Tensor logicalAnd(Tensor a, Tensor b) {
        Optional<Tensor> kernelResult = runLogicalKernel(BinaryOp.LOGICAL_AND, a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
    ...
    public Tensor logicalOr(Tensor a, Tensor b) {
        Optional<Tensor> kernelResult = runLogicalKernel(BinaryOp.LOGICAL_OR, a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        return logicalBinaryOp(a, b, (va, vb) -> va || vb);
    ...
    public Tensor logicalXor(Tensor a, Tensor b) {
        Optional<Tensor> kernelResult = runLogicalKernel(BinaryOp.LOGICAL_XOR, a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        return logicalBinaryOp(a, b, (va, vb) -> va ^ vb);
    ...
    private Tensor compareOp(Tensor a, Tensor b, BinaryOp op) {
        Optional<Tensor> kernelResult = runCompareKernel(op, a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        MemoryView<?> viewA = a.materialize();
    ...
    public Tensor where(Tensor condition, Tensor trueValue, Tensor falseValue) {
        Optional<Tensor> kernelResult = runWhereKernel(condition, trueValue, falseValue);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
    ...
    public Tensor gather(Tensor input, Tensor indices, int axis) {
        Optional<Tensor> kernelResult = runGatherKernel(input, indices, axis);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        ...
        Shape outputShape =
                GatherOp.computeOutputShape(inputView.shape(), indicesView.shape(), normalizedAxis);
    ...
    public Tensor matmul(Tensor a, Tensor b) {
        Optional<Tensor> kernelResult = runMatmulKernel(a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
    ...
    public Tensor batchedMatmul(Tensor a, Tensor b) {
        Optional<Tensor> kernelResult = runBatchedMatmulKernel(a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        MemoryView<?> leftView = a.materialize();
    ...
    public Tensor cast(Tensor x, DataType targetType) {
        Optional<Tensor> kernelResult = runCastKernel(x, targetType);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
    ...
    private Tensor unaryOp(Tensor x, UnaryOp op) {
        Optional<Tensor> kernelResult = runUnaryKernel(op, x);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        MemoryView<?> view = x.materialize();
    ...
    private Optional<Tensor> runUnaryKernel(UnaryOp op, Tensor x) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("unary " + op);
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().unary(op, x)));
    }

    private Optional<Tensor> runBinaryKernel(BinaryOp op, Tensor a, Tensor b) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("binary " + op);
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().binary(op, a, b)));
    }

    private Optional<Tensor> runCompareKernel(BinaryOp op, Tensor a, Tensor b) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("compare " + op);
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().compare(op, a, b)));
    }

    private Optional<Tensor> runLogicalKernel(BinaryOp op, Tensor a, Tensor b) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("logical " + op);
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().logical(op, a, b)));
    }

    private Optional<Tensor> runCastKernel(Tensor x, DataType targetType) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("cast");
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().cast(x, targetType)));
    }

    private Optional<Tensor> runWhereKernel(Tensor condition, Tensor trueValue, Tensor falseValue) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("where");
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(
                materializeResult(kernels.orElseThrow().where(condition, trueValue, falseValue)));
    }

    private Optional<Tensor> runReduceKernel(
            ...
            int axis,
            int... axes) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("reduce " + kind);
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        DeviceRuntime.ReductionOp reductionOp =
                switch (kind) {
                    case SUM -> DeviceRuntime.ReductionOp.SUM;
                    case PRODUCT -> DeviceRuntime.ReductionOp.PRODUCT;
                    case MIN -> DeviceRuntime.ReductionOp.MIN;
                    case MAX -> DeviceRuntime.ReductionOp.MAX;
                };
        return Optional.of(
                materializeResult(
                        kernels.orElseThrow()
                                .reduce(reductionOp, x, accumulatorType, keepDims, axis, axes)));
    }

    private Optional<Tensor> runMatmulKernel(Tensor a, Tensor b) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("matmul");
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().matmul(a, b)));
    }

    private Optional<Tensor> runBatchedMatmulKernel(Tensor a, Tensor b) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("batchedMatmul");
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().batchedMatmul(a, b)));
    }

    private Optional<Tensor> runGatherKernel(Tensor input, Tensor indices, int axis) {
        Optional<DeviceRuntime.EagerKernels> kernels = eagerKernels("gather");
        if (kernels.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(materializeResult(kernels.orElseThrow().gather(input, indices, axis)));
    }

    private Optional<DeviceRuntime.EagerKernels> eagerKernels(String operationName) {
        if (eagerKernels.isPresent()) {
            return eagerKernels;
        }
        if (requireKernels) {
            throw new UnsupportedOperationException(
                    "No eager kernels are registered for runtime operation: " + operationName);
        }
        return Optional.empty();
    }

    private static Tensor materializeResult(Tensor tensor) {
        return tensor.isMaterialized() ? tensor : Tensor.of(tensor.materialize());
    }
    ...
    private Tensor reduce(
            ...
            int _axis,
            int... _axes) {
        Optional<Tensor> kernelResult =
                runReduceKernel(kind, x, accumulatorType, keepDims, _axis, _axes);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
    ...
    private double readAsDoubleFromView(
            MemoryAccess<Object> access, MemoryView<?> view, long offset) {
        @SuppressWarnings("unchecked")
    ...
    private Tensor binaryOp(Tensor a, Tensor b, BinaryOp op) {
        Optional<Tensor> kernelResult = runBinaryKernel(op, a, b);
        if (kernelResult.isPresent()) {
            return kernelResult.orElseThrow();
        }
        MemoryView<?> viewA = a.materialize();
    ...
    private void applyBinaryOp(
        ...
        if (dtype == DataType.I8 || dtype == DataType.BOOL) {
            ...
            float b = Float.float16ToFloat(access.readShort(memB, offB));
            access.writeShort(
                    memOut, offOut, Float.floatToFloat16(ConstantFolder.evalFloat(a, b, op)));
        } else if (dtype == DataType.BF16) {
            float a = BFloat16.toFloat(access.readShort(memA, offA));
            float b = BFloat16.toFloat(access.readShort(memB, offB));
            access.writeShort(
                    memOut, offOut, BFloat16.fromFloat(ConstantFolder.evalFloat(a, b, op)));
        } else if (dtype == DataType.FP64) {
            double a = access.readDouble(memA, offA);
            double b = access.readDouble(memB, offB);
