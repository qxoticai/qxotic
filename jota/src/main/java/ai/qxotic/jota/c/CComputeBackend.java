...
public final class CComputeBackend implements ai.qxotic.jota.tensor.ComputeBackend {
    ...
    public CComputeBackend(CMemoryContext context) {
    ...
    public MemoryView<?> execute(TIRGraph graph, List<Tensor> inputs) {
        ...
        List<MemoryView<?>> outputViews = allocateOutputs(lirGraph, context);
        Memory<MemorySegment> scratch = null;
        if (scratchLayout.requiresScratch()) {
            scratch = context.memoryAllocator().allocateMemory(scratchLayout.alignedTotalByteSize(), 64L);
        }
        ...
        exec.launch(new LaunchConfig(1, 1, 1, 1, 1, 1, 0, false), args, new ExecutionStream(Device.C, 0L, true));
    ...
    private List<Tensor> resolveInputs(LIRGraph graph, List<Tensor> inputs) {
        ...
        for (int i = 0; i < graph.inputs().size(); i++) {
            ...
            MemoryContext.copy(srcContext, srcView, srcContext, contig);
            view = contig;
            srcView = contig;
            if (view.memory().device().equals(Device.C) && view.memory().base() instanceof MemorySegment) {
                resolved.add(Tensor.of(view));
                continue;
            }
            MemoryView<MemorySegment> copy =
                    MemoryView.of(
                            context.memoryAllocator().allocateMemory(view.dataType(), view.shape()),
                            view.dataType(),
                            Layout.rowMajor(view.shape()));
            MemoryContext.copy(srcContext, srcView, context, copy);
            resolved.add(Tensor.of(copy));
        }
        return resolved;
    }

    private static List<MemoryView<?>> allocateOutputs(LIRGraph graph, CMemoryContext context) {
