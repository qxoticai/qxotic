...
public final class IRConstantFolder {
    ...
    public static ScalarConstant foldUnary(UnaryOperator op, ScalarConstant input) {
        ...
        if (dtype == DataType.FP32) {
            ...
            if (result != null) {
                return ScalarConstant.broadcast(Float.floatToIntBits(result), dtype, shape);
            }
        } else if (dtype == DataType.FP64) {
            double value = Double.longBitsToDouble(input.rawBits());
            Double result = applyUnaryDouble(op, value);
            if (result != null) {
                return ScalarConstant.broadcast(Double.doubleToLongBits(result), dtype, shape);
            }
        } else if (dtype == DataType.I32) {
            int value = (int) input.rawBits();
            Integer result = applyUnaryInt(op, value);
            if (result != null) {
                return ScalarConstant.broadcast(result, dtype, shape);
            }
    ...
    public static ScalarConstant foldBinary(
        ...
        // Both must have same dtype (caller should have promoted)
