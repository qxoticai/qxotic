...
public final class IRLInterpreter {
    ...
    public void execute(IRLGraph graph) {
        executeNode(graph.body());
    }

    /** Executes a single IR-L node. */
    public void executeNode(IRLNode node) {
        switch (node) {
    ...
    private void executeNestedLoops(java.util.List<Loop> loops, int depth, IRLNode body) {
        if (depth >= loops.size()) {
            executeNode(body);
            return;
        }
    ...
    private void executeBlock(Block block) {
        for (IRLNode stmt : block.statements()) {
            executeNode(stmt);
        }
    }
    ...
    private void writeValue(MemorySegment segment, long offset, long bits, DataType dtype) {
        if (dtype == DataType.FP32) {
            segment.set(ValueLayout.JAVA_FLOAT_UNALIGNED, offset, Float.intBitsToFloat((int) bits));
        } else if (dtype == DataType.FP64) {
            segment.set(ValueLayout.JAVA_DOUBLE_UNALIGNED, offset, Double.longBitsToDouble(bits));
        } else if (dtype == DataType.FP16 || dtype == DataType.BF16) {
            segment.set(ValueLayout.JAVA_SHORT_UNALIGNED, offset, (short) bits);
        } else if (dtype == DataType.I8 || dtype == DataType.BOOL) {
            segment.set(ValueLayout.JAVA_BYTE, offset, (byte) bits);
        } else if (dtype == DataType.I16) {
            segment.set(ValueLayout.JAVA_SHORT_UNALIGNED, offset, (short) bits);
        } else if (dtype == DataType.I32) {
            segment.set(ValueLayout.JAVA_INT_UNALIGNED, offset, (int) bits);
        } else if (dtype == DataType.I64) {
            segment.set(ValueLayout.JAVA_LONG_UNALIGNED, offset, bits);
        } else {
