...
import ai.qxotic.jota.tensor.BinaryNode;
import ai.qxotic.jota.tensor.BinaryOp;
import ai.qxotic.jota.tensor.CastNode;
import ai.qxotic.jota.tensor.ContiguousNode;
import ai.qxotic.jota.tensor.ExprNode;
...
import ai.qxotic.jota.tensor.ReductionNode;
import ai.qxotic.jota.tensor.ReductionOp;
import ai.qxotic.jota.tensor.RangeNode;
import ai.qxotic.jota.tensor.ScalarNode;
import ai.qxotic.jota.tensor.TernaryNode;
import ai.qxotic.jota.tensor.TernaryOp;
import ai.qxotic.jota.tensor.TransferNode;
import ai.qxotic.jota.tensor.UnaryNode;
import ai.qxotic.jota.tensor.UnaryOp;
import ai.qxotic.jota.tensor.ViewTransformOp;
...
final class JavaKernelCompiler {
    ...
    private static final class KernelSourceGenerator {
        ...
        private String literal(ScalarNode scalar) {
            ...
            if (scalar.dataType() == DataType.FP64) {
                double value = scalar.value().doubleValue();
                return Double.toString(value);
            }
            if (scalar.dataType() == DataType.I32) {
                return Integer.toString(scalar.value().intValue());
            }
            if (scalar.dataType() == DataType.I64) {
                return Long.toString(scalar.value().longValue()) + "L";
            }
