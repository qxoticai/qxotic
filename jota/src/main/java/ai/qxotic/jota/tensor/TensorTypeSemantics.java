...
final class TensorTypeSemantics {
    ...
    static void requireSameIntegralType(DataType left, DataType right, String opName) {
        if (left != right) {
            throw new IllegalArgumentException(
                    opName + " requires operands with the same data type, got " + left + " and " + right);
        }
    ...
    static void requireBool(DataType dataType, String opName) {
        if (dataType != DataType.BOOL) {
            throw new IllegalArgumentException(opName + " requires BOOL data type, got " + dataType);
        }
    }
    ...
    static DataType resolveReductionAccumulator(
        ...
        if (accumulatorType == DataType.BOOL
            ...
            throw new IllegalArgumentException(
                    opName
                            + " accumulator must be numeric (non-BOOL), got "
                            + accumulatorType);
        }
        if (inputType == DataType.BOOL) {
            return accumulatorType;
        }
        DataType promoted = promoteForArithmetic(inputType, accumulatorType, opName + " accumulator");
