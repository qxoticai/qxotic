...
final class LongsMemoryOperations implements MemoryOperations<long[]> {
    ...
    @Override
    public void copy(Memory<long[]> src, long srcByteOffset, Memory<long[]> dst, long dstByteOffset, long byteSize) {
        ...
        System.arraycopy(
                src.base(), Math.toIntExact(srcByteOffset / Long.BYTES),
                dst.base(), Math.toIntExact(dstByteOffset / Long.BYTES),
                Math.toIntExact(byteSize / Long.BYTES)
        );
    }

    @Override
    public void copyFromNative(Memory<MemorySegment> src, long srcByteOffset, Memory<long[]> dst, long dstByteOffset, long byteSize) {
        MemoryAccessChecks.checkBounds(src, srcByteOffset, byteSize);
        MemoryAccessChecks.checkBounds(dst, dstByteOffset, byteSize);
        MemoryAccessChecks.checkAlignedValue(dstByteOffset, Long.BYTES);
        MemoryAccessChecks.checkAlignedValue(byteSize, Long.BYTES);
    ...
    @Override
    public void copyToNative(Memory<long[]> src, long srcByteOffset, Memory<MemorySegment> dst, long dstByteOffset, long byteSize) {
        MemoryAccessChecks.checkBounds(src, srcByteOffset, byteSize);
        MemoryAccessChecks.checkBounds(dst, dstByteOffset, byteSize);
        MemoryAccessChecks.checkAlignedValue(srcByteOffset, Long.BYTES);
        MemoryAccessChecks.checkAlignedValue(byteSize, Long.BYTES);
    ...
    public void fillByte(Memory<long[]> memory, long byteOffset, long byteSize, byte byteValue) {
        ...
        Arrays.fill(
                memory.base(), Math.toIntExact(byteOffset / Long.BYTES),
