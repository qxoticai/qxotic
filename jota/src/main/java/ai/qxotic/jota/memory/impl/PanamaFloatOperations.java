...
final class PanamaFloatOperations implements FloatOperations<MemorySegment> {
    ...
    @Override
    public void elementWise(
            float scalar, FloatUnaryOperator unaryOperator, MemoryView<MemorySegment> out) {
        if (out.dataType() != DataType.FP32) {
            throw new UnsupportedOperationException(
                    "Unsupported (out) data type: " + out.dataType());
        }
    ...
    public float foldAll(
            MemoryView<MemorySegment> in, float initialValue, FloatBinaryOperator binaryOperator) {
        if (in.dataType() != DataType.FP32) {
            throw new UnsupportedOperationException("Unsupported data type: " + in.dataType());
        }
