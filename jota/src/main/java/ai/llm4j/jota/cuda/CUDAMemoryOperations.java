...
public final class CUDAMemoryOperations implements MemoryOperations<CUdeviceptr> {
    ...
    @Override
    public void copy(
            Memory<CUdeviceptr> src,
            long srcByteOffset,
            Memory<CUdeviceptr> dst,
            long dstByteOffset,
            long byteSize) {
        if (byteSize == 0) {
            return;
    ...
    public void fillByte(
            Memory<CUdeviceptr> memory, long byteOffset, long byteSize, byte byteValue) {
        if (byteSize == 0) {
            return;
        }
        if (byteSize < 0) {
        ...
        int result = cuMemsetD8(memory.base().withByteOffset(byteOffset), byteValue, byteSize);
        if (result != CUresult.CUDA_SUCCESS) {
    ...
    public void fillShort(
            Memory<CUdeviceptr> memory, long byteOffset, long byteSize, short shortValue) {
        if (byteSize == 0) {
            return;
        }
        if (byteSize < 0) {
        ...
        int result =
                cuMemsetD16(memory.base().withByteOffset(byteOffset), shortValue, byteSize / 2);
        if (result != CUresult.CUDA_SUCCESS) {
            throw new RuntimeException("CUDA memset failed: " + result);
        }
    }
    ...
    public void fillInt(Memory<CUdeviceptr> memory, long byteOffset, long byteSize, int intValue) {
        ...
        int result = cuMemsetD32(memory.base().withByteOffset(byteOffset), intValue, byteSize / 4);
        if (result != CUresult.CUDA_SUCCESS) {
            throw new RuntimeException("CUDA memset failed: " + result);
        }
    }
    ...
    public void fillLong(
            Memory<CUdeviceptr> memory, long byteOffset, long byteSize, long longValue) {
        throw new UnsupportedOperationException(
                "CUDA does not support 64-bit fill. A custom kernel is required.");
    }
}
