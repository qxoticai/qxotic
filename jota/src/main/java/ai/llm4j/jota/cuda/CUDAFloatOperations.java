...
public class CUDAFloatOperations { // implements FloatOperations<CUdeviceptr> {
    ...
    //        this.unaryContiguousKernel = new CUDAKernel("/kernels.ptx",
    // "elementwise_unary_contiguous");
    //        this.unaryStridedKernel = new CUDAKernel("/kernels.ptx", "elementwise_unary_strided");
    //        this.binaryContiguousKernel = new CUDAKernel("/kernels.ptx",
    // "elementwise2_contiguous");
    //        this.binaryStridedKernel = new CUDAKernel("/kernels.ptx", "elementwise2_strided");
    //        this.scalarContiguousKernel = new CUDAKernel("/kernels.ptx",
    // "elementwise2_scalar_contiguous");
    //        this.scalarStridedKernel = new CUDAKernel("/kernels.ptx",
    // "elementwise2_scalar_strided");
    ...
    //        this.fillWithUnaryScalarContiguousKernel = new CUDAKernel("/kernels.ptx",
    // "fill_with_unary_scalar_contiguous");
    //        this.fillWithUnaryScalarStridedKernel = new CUDAKernel("/kernels.ptx",
    // "fill_with_unary_scalar_strided");
    //
    ...
    //
    //    @Override
    //    public void elementWise(MemoryView<CUdeviceptr> in, FloatUnaryOperator unaryOperator,
    // MemoryView<CUdeviceptr> out) {
    //        int n = (int) in.shape().size();
    ...
    //
    //    @Override
    //    public void elementWise(float in, FloatUnaryOperator unaryOperator,
    // MemoryView<CUdeviceptr> out) {
    //        int n = (int) out.shape().size();
    //        int opId = getUnaryOpId(unaryOperator);
    //        if (out.isContiguous()) {
    ...
    //    }
    //
    //    private void launchFillWithUnaryScalarContiguousKernel(float scalarIn,
    // MemoryView<CUdeviceptr> out, int n, int opId) {
    ...
    //        fillWithUnaryScalarContiguousKernel.launch(gridSize, 1, 1, blockSize, 1, 1, 0,
    // kernelParameters);
    //    }
    //
    //    private void launchFillWithUnaryScalarStridedKernel(float scalarIn,
    // MemoryView<CUdeviceptr> out, int n, int opId) {
    ...
    //            Pointer.to(out.byteStride().toArray()),
    //            Pointer.to(new int[]{n}),
    ...
    //        int blockSize = 256;
    //        int gridSize = (n + blockSize - 1) / blockSize;
    //        fillWithUnaryScalarStridedKernel.launch(gridSize, 1, 1, blockSize, 1, 1, 0,
    // kernelParameters);
    //    }
    //
    //    @Override
    //    public void elementWise2(MemoryView<CUdeviceptr> left, FloatBinaryOperator binaryOperator,
    // MemoryView<CUdeviceptr> right, MemoryView<CUdeviceptr> out) {
    //        int n = (int) left.shape().size();
    ...
    //    @Override
    //    public void elementWise2(MemoryView<CUdeviceptr> left, FloatBinaryOperator binaryOperator,
    // float right, MemoryView<CUdeviceptr> out) {
    //        int n = (int) left.shape().size();
    //        int opId = getBinaryOpId(binaryOperator);
    ...
    //    // Kernel Launchers
    //
    //    private void launchUnaryContiguousKernel(MemoryView<CUdeviceptr> in,
    // MemoryView<CUdeviceptr> out, int n, int opId) {
    //        Pointer kernelParameters = Pointer.to(
    ...
    //    }
    //
    //    private void launchUnaryStridedKernel(MemoryView<CUdeviceptr> in, MemoryView<CUdeviceptr>
    // out, int n, int opId) {
    //        int rank = in.shape().rank();
    //        Pointer kernelParameters = Pointer.to(
    //            Pointer.to(in.memory().base().withByteOffset(in.byteOffset())),
    //            Pointer.to(in.shape().toArray()),
    //            Pointer.to(in.byteStride()),
    //            Pointer.to(out.memory().base().withByteOffset(out.byteOffset())),
    //            Pointer.to(out.shape().toArray()),
    //            Pointer.to(out.byteStride()),
    //            Pointer.to(new int[]{n}),
    //            Pointer.to(new int[]{rank}),
    //            Pointer.to(new int[]{opId})
    //        );
    ...
    //    }
    //
    //    private void launchBinaryContiguousKernel(MemoryView<CUdeviceptr> left,
    // MemoryView<CUdeviceptr> right, MemoryView<CUdeviceptr> out, int n, int opId) {
    //        Pointer kernelParameters = Pointer.to(
    ...
    //    }
    //
    //    private void launchBinaryStridedKernel(MemoryView<CUdeviceptr> left,
    // MemoryView<CUdeviceptr> right, MemoryView<CUdeviceptr> out, int n, int opId) {
    //        int rank = left.shape().rank();
    //        Pointer kernelParameters = Pointer.to(
    //            Pointer.to(left.memory().base().withByteOffset(left.byteOffset())),
    //            Pointer.to(left.shape().toArray()),
    //            Pointer.to(left.byteStride()),
    //            Pointer.to(right.memory().base().withByteOffset(right.byteOffset())),
    //            Pointer.to(right.shape().toArray()),
    //            Pointer.to(right.byteStride()),
    //            Pointer.to(out.memory().base().withByteOffset(out.byteOffset())),
    //            Pointer.to(out.shape().toArray()),
    //            Pointer.to(out.byteStride()),
    //            Pointer.to(new int[]{n}),
    ...
    //    }
    //
    //    private void launchScalarContiguousKernel(MemoryView<CUdeviceptr> in, float scalar,
    // MemoryView<CUdeviceptr> out, int n, int opId) {
    //        Pointer kernelParameters = Pointer.to(
    //            Pointer.to(in.memory().base().withByteOffset(in.byteOffset())),
    ...
    //    }
    //
    //    private void launchScalarStridedKernel(MemoryView<CUdeviceptr> in, float scalar,
    // MemoryView<CUdeviceptr> out, int n, int opId) {
    //        int rank = in.shape().rank();
    //        Pointer kernelParameters = Pointer.to(
    //            Pointer.to(in.memory().base().withByteOffset(in.byteOffset())),
    ...
    //
    //    @Override
    //    public void fold(MemoryView<CUdeviceptr> in, FloatBinaryOperator binaryOperator, float
    // initialValue, MemoryView<CUdeviceptr> out, int axis) {
    //        int rank = in.shape().rank();
    //        if (axis < 0 || axis >= rank) {
    ...
    //    }
    //
    //    private void launchFoldContiguousKernel(MemoryView<CUdeviceptr> in, FloatBinaryOperator
    // binaryOperator, float initialValue, MemoryView<CUdeviceptr> out, int axis) {
    //        int opId = getBinaryOpId(binaryOperator);
    //        int nOut = (int) out.shape().size();
    //        int rank = in.shape().rank();
    //
    //        Pointer kernelParameters = Pointer.to(
    //            Pointer.to(in.memory().base().withByteOffset(in.byteOffset())),
    ...
    //    }
    //
    //    private void launchFoldStridedKernel(MemoryView<CUdeviceptr> in, FloatBinaryOperator
    // binaryOperator, float initialValue, MemoryView<CUdeviceptr> out, int axis) {
    //        int opId = getBinaryOpId(binaryOperator);
    //        int nOut = (int) out.shape().size();
    //        int rank = in.shape().rank();
    ...
    //
    //    @Override
    //    public void reduce(MemoryView<CUdeviceptr> in, FloatBinaryOperator binaryOperator,
    // MemoryView<CUdeviceptr> out, int axis) {
    //        int rank = in.shape().rank();
    //        if (axis < 0 || axis >= rank) {
    //            throw new IllegalArgumentException("Invalid axis: " + axis);
    //        }
    ...
    //    }
    //
    //    private void launchReduceContiguousKernel(MemoryView<CUdeviceptr> in, FloatBinaryOperator
    // binaryOperator, MemoryView<CUdeviceptr> out, int axis) {
    //        int opId = getBinaryOpId(binaryOperator);
    //        int nOut = (int) out.shape().size();
    //        int rank = in.shape().rank();
    //
    ...
    //    }
    //
    //    private void launchReduceStridedKernel(MemoryView<CUdeviceptr> in, FloatBinaryOperator
    // binaryOperator, MemoryView<CUdeviceptr> out, int axis) {
    //        int opId = getBinaryOpId(binaryOperator);
    //        int nOut = (int) out.shape().size();
    //        int rank = in.shape().rank();
    ...
    //        if (!in.isContiguous()) {
    //            throw new UnsupportedOperationException("Strided reduceAll is not yet implemented
    // for CUDA.");
    //        }
    //        int n = (int) in.shape().size();
    //        if (n == 0) {
    ...
    //        float initialValue = CUDAMemoryAccess.instance().readFloat(in.memory(),
    // in.byteOffset());
    //        return foldAll(in.slice(0, 1, n), initialValue, binaryOperator);
    //    }
    //
    //    @Override
    //    public float foldAll(MemoryView<CUdeviceptr> in, float initialValue, FloatBinaryOperator
    // binaryOperator) {
    //        if (!in.isContiguous()) {
    //            throw new UnsupportedOperationException("Strided foldAll is not yet implemented
    // for CUDA.");
    //        }
    //
    //        int n = (int) in.shape().size();
    ...
    //        try (ScopedMemory<CUdeviceptr> partialResultsDev =
    // CUDAScopedMemoryAllocator.instance().allocateMemory(gridSize * 4L, 4)) {
    //
    ...
    //            foldAllKernel.launch(gridSize, 1, 1, blockSize, 1, 1, sharedMemBytes,
    // kernelParams1);
    //
    ...
    //                Pointer.to(partialResultsDev.base()), // Output to the start of the same
    // buffer
    //                Pointer.to(new int[]{gridSize}),      // n is now the number of partial
    // results
    //                Pointer.to(new float[]{initialValue}),
    //                Pointer.to(new int[]{opId})
    //            );
    ...
    //
    //    @Override
    //    public void matrixMultiply(MemoryView<CUdeviceptr> left, MemoryView<CUdeviceptr> right,
    // MemoryView<CUdeviceptr> out) {
    //        // Validate shapes
    ...
    //            throw new IllegalArgumentException("Inner dimensions must match for matrix
    // multiplication: " +
    //                                               left.shape().dimension(1) + " != " +
    // right.shape().dimension(0));
    //        }
    ...
    //                                               left.shape().dimension(0) + ", " +
    // right.shape().dimension(1) +
    //                                               "), got " + out.shape());
    //        }
    //
    //        // cuBLAS requires contiguous, column-major matrices.
    //        // Since Jota uses row-major, we conceptually transpose the operation: C = A @ B
    // becomes C^T = B^T @ A^T
    //        // So, we call SGEMM with B as the first matrix and A as the second, both transposed.
    //        if (!left.isContiguous() || !right.isContiguous() || !out.isContiguous()) {
    //            throw new UnsupportedOperationException("Strided matrix multiplication is not yet
    // supported. " +
    //                                                    "Please ensure all input and output views
    // are contiguous.");
    //        }
    //
    ...
    //        // Parameters for cublasSgemm(handle, transa, transb, m, n, k, alpha, A, lda, B, ldb,
    // beta, C, ldc)
