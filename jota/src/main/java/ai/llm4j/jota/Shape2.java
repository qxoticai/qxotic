...
public interface Shape2 {
    ...
    //        Set<Integer> toRemoveSet =
    // Arrays.stream(_axes).map(this::wrapAround).boxed().collect(Collectors.toSet());
    ...
    //        int[] removeAxes = IntStream.range(0, rank()).filter(i -> dimension(i) ==
    // 1).toArray();
    ...
    //        if (dimension < 0) {
    //            throw new IllegalArgumentException("dimension must be non-negative, got: " +
    // dimension);
    //        }
