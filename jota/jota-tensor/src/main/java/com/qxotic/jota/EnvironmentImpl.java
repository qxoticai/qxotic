...
record EnvironmentImpl(
    ...
    private static Environment buildDefaultEnvironment() {
        ...
        return new EnvironmentImpl(nativeBackend, nativeBackend, defaultFloatValue(), registry);
    }

    private static DataType defaultFloatValue() {
        ...
        for (DataType dataType :
            ...
            if (dataType.name().equals(name) || dataType.aliases().contains(name)) {
                return dataType;
            }
        }
        throw new IllegalArgumentException("unknown floating-point data type: " + name);
    }
