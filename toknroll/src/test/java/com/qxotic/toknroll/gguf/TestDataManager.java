...
public class TestDataManager {
    ...
    public GGUF getOrDownloadMetadata(TestModel model) throws IOException, InterruptedException {
        ...
        return getOrDownloadMetadata(cacheKey, model.getHuggingFaceUrl());
    }

    /**
     * Gets or downloads GGUF metadata from an arbitrary URL using the provided cache key.
     *
     * @param cacheKey unique cache key
     * @param url direct URL to GGUF artifact
     */
    ...
    /** Downloads and caches an arbitrary remote file and returns its local path. */
    public Path getOrDownloadFile(String cacheKey, String url, String extension)
            throws IOException, InterruptedException {
        String ext = extension == null ? "" : extension;
        Path cachedFile = cachePath.resolve(cacheKey + ext);
        if (Files.exists(cachedFile)) {
            return cachedFile;
        }
     ...
     */
    private void downloadAndCache(String url, Path targetFile)
            throws IOException, InterruptedException {
    ...
    public static TokenizerMetadata extractTokenizerMetadata(GGUF gguf) {
        ...
        Integer padTokenId =
                firstPresentInt(
                        gguf, "tokenizer.ggml.pad_token_id", "tokenizer.ggml.padding_token_id");
        Integer unkTokenId =
                firstPresentInt(
                        gguf, "tokenizer.ggml.unk_token_id", "tokenizer.ggml.unknown_token_id");
    ...
    private static Integer firstPresentInt(GGUF gguf, String primaryKey, String fallbackKey) {
        Integer value = gguf.getValueOrDefault(int.class, primaryKey, null);
        if (value != null) {
            return value;
        }
        return gguf.getValueOrDefault(int.class, fallbackKey, null);
    }
}
