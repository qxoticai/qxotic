...
public class GPT2Tokenizer extends AbstractTokenizer {
    ...
    public GPT2Tokenizer(Vocabulary vocabulary, Normalizer normalizer, TextSplitter preTokenizer, Map<IntPair, MergeRank> mergeRanks) {
        super(vocabulary, normalizer, preTokenizer);
        this.merges = mergeRanks;
    }

    public GPT2Tokenizer(Vocabulary vocabulary, Normalizer normalizer, TextSplitter preTokenizer, List<IntPair> mergeRanks) {
        super(vocabulary, normalizer, preTokenizer);
    ...
    private IntSequence encode(String text, Set<String> allowedSpecial) {
        ...
        assert allowedSpecial.stream()
                .allMatch(vocabulary::contains);

        // Allowed tokens should be all special.
        assert allowedSpecial.stream().allMatch(tokenString -> {
            int tokenIndex = vocabulary.id(tokenString);
            return !vocabulary().isTokenOfType(tokenIndex, StandardTokenType.NORMAL);
        });
    ...
    private IntSequence encodeChunk(CharSequence chunk) {
        ...
        while (size >= 2) {
            ...
            for (int i = 0; i + 1 < size; ++i) {
                ...
                if (candidateMergeRank != null && (bestMergeRank == null || candidateMergeRank.rank() < bestMergeRank.rank())) {
            ...
            System.arraycopy(tokens, startPosition + 2, tokens, startPosition + 1, size - startPosition - 2);
            --size;
        }
    ...
    protected IntSequence encodeImpl(CharSequence text) {
        byte[] rawBytes = useJavaReplacement
                ? text.toString().getBytes(StandardCharsets.UTF_8) // use ?
                : getBytesWithReplacement(text, REPLACEMENT_BYTES); // use \uFFFD (EF BF BD in UTF8)
    ...
    public byte[] decodeBytes(IntSequence tokens) {
        int totalBytes = tokens.stream()
                .map(tokenIndex -> vocabulary().token(tokenIndex).length())
                .sum();
