 ...
 * 
 * This benchmark class provides:
 * - Operations per second (ops/s)
 * - Characters per second (chars/s) - calculated as ops/s × text_length
 * - Tokens per second (tokens/s) - calculated as ops/s × token_count
 * 
 * Usage:
 *   mvn exec:java -Dexec.mainClass="ai.qxotic.tokenizers.benchmarks.TokenizerBenchmarkRunner"
 */
...
public class TokenizerBenchmark {
    ...
    private String generateText(String size, String type) {
        ...
        return switch (size) {
            ...
            case "large" -> baseText.repeat(10).substring(0, Math.min(10000, baseText.length() * 10));
