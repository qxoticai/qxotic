...
public final class RoPE {
    ...
    public static float[][] precomputeFreqsCis(
            int contextLength,
            int headSize,
            double theta,
            float scaleFactor,
    ...
    public static float[][] precomputeFreqsCis(
            int contextLength, int headSize, double theta, float[] ropeFreqs) {
        assert headSize % 2 == 0;
        ...
        for (int pos = 0; pos < contextLength; ++pos) {
            for (int i = 0; i < headSize; i += 2) {
                float freq =
                        (float) (1.0 / Math.pow(theta, i / (double) headSize)) / ropeFreqs[i / 2];
                float val = pos * freq;
    ...
    private static float[][] precomputeFreqsCis(
            int contextLength,
            int headSize,
            double theta,
            boolean ropeScaling,
