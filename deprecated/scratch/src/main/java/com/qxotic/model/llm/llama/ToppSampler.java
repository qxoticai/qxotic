...
final class ToppSampler implements Sampler2 {
    ...
    public int applyAsInt(FloatSpan logits) {
        ...
        for (int i = 0; i < indices.length; i++) {
            if (logitsOps.getElementAt(logits, i) >= cutoff) {
                indices[head++] = i;
            } else {
                indices[tail--] = i;
            }
        ...
        for (int i = n0 - 1; i >= 0; i--) {
            swap(indices, 0, i);
            cumulativeProb += logitsOps.getElementAt(logits, indices[i]);
        ...
        for (int i = n0 - 1; i >= lastIndex; i--) {
            cdf += logitsOps.getElementAt(logits, indices[i]);
            if (r < cdf) {
                return indices[i];
            }
        }
